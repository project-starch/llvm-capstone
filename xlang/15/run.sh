...
echo ""
echo "=== Running under RISC-V QEMU (observed: exit 139) ==="
# Observed: SIGSEGV, exit 139 -- but HEAP-LAYOUT SENSITIVE. It faults when
# invoked with absolute paths (as this script does) and exits 0 with short
# relative paths; each is individually deterministic across runs.
...
QEMU_EXIT_CODE=$?
set -e
echo "RISC-V QEMU run exited with: $QEMU_EXIT_CODE (expected 139)"
...
if [ "$QEMU_EXIT_CODE" -ne 139 ]; then
    echo "FAIL: RISC-V QEMU expected exit 139, got $QEMU_EXIT_CODE." >&2
    FAILED=1
fi
if [ "$FAILED" -eq 0 ]; then
    echo ""
