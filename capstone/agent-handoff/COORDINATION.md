...
| 2026-07-09 | Agent-B | `capstone/capstone-qemu` (add `tests/capstone-revoke-probes/` — task-003 revoke probes) | 2e6a67d1 → e0cd45de | (superproject commit on `capstone-bootstrap-b`; needs operator push of submodule branch) |
...
| 2026-07-09 | Agent-B task-004 (durability + probes) | Landed the four task-003 revoke probes in the submodule's own tree `capstone/capstone-qemu/tests/capstone-revoke-probes/` (in-lane; not `capstone/tests`). Submodule commit `e0cd45de` (on `2e6a67d1`); superproject gitlink bumped `2e6a67d1`→`e0cd45de` + docs on `capstone-bootstrap-b`. Driver reuses the runtime-qemu harness; GREEN 4/4 on binary `2e6a67d1`. **Durability:** csdrop `2e6a67d1` already pushed to `project-starch/capstone-qemu` (reflog `update by push`); `e0cd45de` needs an operator push (no push creds in this session). **Integration for A:** after the operator pushes the submodule branch, `git submodule update` resolves the gitlink from `project-starch/capstone-qemu` origin — no `.gitmodules`/origin change was made. |
