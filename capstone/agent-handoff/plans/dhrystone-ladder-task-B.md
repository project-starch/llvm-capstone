# B-lane task: broaden the silicon ladder — Dhrystone (headline) + integer breadth
...
Broaden the silicon ladder with **Dhrystone** as the next headline rung: it compiles
in the silicon config and, run in a pure-cap domain on QEMU, returns a
**deterministic result == a native `cc -O0` oracle**, with the static gate showing
**`cjalr=0`**. That's the bar — a second recognized headline benchmark (alongside
CoreMark matrix, rung 7) proven in the silicon config on QEMU.

Dhrystone reports *time*, not a checksum, so there is no natural return value. Fold
the benchmark's **final deterministic state** into one `unsigned` and return that:
after a fixed run count, Dhrystone's `Int_Glob`, `Bool_Glob`, `Ch_1_Glob`,
`Ch_2_Glob`, the two record chains' int/enum/char fields, and `Arr_1_Glob` /
`Arr_2_Glob` all settle to standard, documented values. Combine those into an
FNV-style checksum (same contract the crc32 / coremark rungs use). These are all
`int` / `char` / `enum` values — **pointer-size-independent**, so the domain and a
native host fold the identical number by construction. (Dhrystone *uses* pointers
`Rec_Pointer`, but do **not** fold any pointer value into the checksum — that would
diverge at `sizeof(void*)=16`.)
...
- **Do NOT modify** `gen-gp-captable-glue.py`, `link-gpfree.ld`, `sbi_capstone.c`,
  `build-ladder-domain.sh`, or `run-ladder-qemu.sh`. If you think a rung *needs* a
  change there, **stop and coordinate** — that surface is A's this week.
- **The large-`.rodata` path is A's and is NOT yet landed.** If a benchmark has a
  **large initialized `const` table** — concretely, a **file-scope `const` array
  > 256 B**, or **any** `static` / function-local (`.L…`) large `const` — it will
  either fail the build (the generator dies on an initialized global that overflows
  the 12-bit store offset and isn't copy-eligible) or hit the half-built copy path.
  **Do not build or extend a delivery mechanism for it.** Keep the data small /
  runtime-computed, or pivot (below). Dhrystone's data is small (a couple of ~31-byte
  `char` arrays, short string literals, two records) so this most likely won't bite;
  its `const` strings are well under 256 B.
- **Single-TU only (RISK A).** `getGpCaptableIndex` numbers globals *per module*, so
  a multi-`.c` domain collides on the one gp cap-table. Amalgamate Dhrystone's TUs
  into one `_kernel.h` (like rung 7 did for CoreMark). Provide tiny **freestanding**
  `strcpy`/`strcmp` in the kernel — do not pull libc.
...
## Fallback (keeps the task guaranteed-shippable)
If Dhrystone turns out to need the large-RO path (it shouldn't), **don't fight it** —
pivot to breadth instead: add **2–3 more integer BEEBS / RV8 kernels** that fit the
existing small-global / immediate-init path (in the spirit of the existing
`beebs_insertsort` / `beebs_prime` / `beebs_recursion` / `rv8_primes` rungs). Same
`retval == oracle`, `cjalr=0` bar. Either outcome — Dhrystone *or* a breadth batch —
is a complete, valuable deliverable.

## Already solved — reuse, don't re-derive
- Large **`.bss`** is handled (generator zeroes it with a runtime loop, `160a7613`) —
  a big zero scratch array is fine.
...
is at `2e3e6a52`). Then work only under
