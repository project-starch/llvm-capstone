...
| `cincoffset` untagged rs1 on 2D float arrays | Runtime tag fault | `ludcmp`, likely `minver` | **DEFERRED** — compiles/links with soft-float runtime but faults at `helper_cscincoffset` on `a[i][j]`; needs cincoffset canonicalization extended (Bug #1 variant) or invasive source workaround. |
