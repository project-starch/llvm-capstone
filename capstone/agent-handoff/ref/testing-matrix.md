...
| MicroPython resumable census | every selected direct Python file either returns and is scored, faults, hangs, or is explicitly unscored; current standard baseline is 598 PASS / 86 FAIL / 0 FAULT / 233 UNSCORED | MicroPython port, interpreter patches, cap-init, or suite-runner changes | `capstone/benchmarks/micropython/tools/run-resumable-suite.py --capture-output`; full chunk recipe in `plans/micropython-domain-compilation.md` |
