...
| MicroPython resumable census | every selected direct Python file either returns and is scored, faults, hangs, or is explicitly unscored; current standard baseline is 576 PASS / 339 FAIL / 0 FAULT / 2 UNSCORED | MicroPython port, interpreter patches, cap-init, or suite-runner changes | `capstone/benchmarks/micropython/tools/run-resumable-suite.py`; full chunk recipe in `plans/micropython-domain-compilation.md` |
