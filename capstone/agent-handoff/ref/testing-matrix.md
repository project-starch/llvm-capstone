...
# MicroPython chunk generation/scoring/resume tools (full recipe is in the active plan)
python3 "$CAPSTONE_REPO_ROOT/capstone/benchmarks/micropython/tools/run-resumable-suite.py" --help
...
| MicroPython resumable census | every selected direct Python file either returns and is scored, faults, hangs, or is explicitly unscored; current standard baseline is 592 PASS / 93 FAIL / 0 FAULT / 232 UNSCORED | MicroPython port, interpreter patches, cap-init, or suite-runner changes | `capstone/benchmarks/micropython/tools/run-resumable-suite.py --capture-output`; full chunk recipe in `plans/micropython-domain-compilation.md` |
