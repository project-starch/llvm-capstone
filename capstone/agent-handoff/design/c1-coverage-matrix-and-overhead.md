...
| **Stack — dynamic `alloca` (runtime-sized)** | **Yes (gated)** | `SHRINK` in `lowerDYNAMIC_STACKALLOC`: returned pointer narrowed to `[cursor, cursor+alignedSize)`, `sp`/X2 kept broad | **off** (`-capstone-shrink-stack`) | lit `cap-shrink-dynalloca.ll` on/off; reuses the runtime-proven SHRINK primitive (`stack_oob`) |
