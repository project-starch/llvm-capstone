...
static void start_impl(void) {
  ...
  open_req->flags = 0;
  open_req->mode = 0;
  copy_bytes(open_req->path, HOSTCALL_FILE_OPEN_CLOSE_PROBE_INPUT_PATH,
             HOSTCALL_FILE_OPEN_CLOSE_PROBE_INPUT_PATH_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_OPEN;
  metadata->offset = HC_FILE_OPEN_REQ_V0_PATH_OFFSET;
  metadata->length = HOSTCALL_FILE_OPEN_CLOSE_PROBE_INPUT_PATH_LEN;
  metadata->result = 0;
  metadata->error = 0;
