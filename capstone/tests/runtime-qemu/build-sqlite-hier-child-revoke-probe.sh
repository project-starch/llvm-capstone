...
# Build the SQLite hierarchical derived-child (parent-close cascade) Stage-2 probe.
# One engine controller (.user) + one host borrower (.smode), Buildroot guest cc.
# The .user links libcapstone.c, which now provides share_child_region().
...
PROBE_DIR="$SCRIPT_DIR/sqlite-hier-child-revoke-probe"
...
"$GUEST_CC" -O2 \
  ...
  -o "$OUT_DIR/sqlite_hier_child_revoke_probe.user" \
  "$PROBE_DIR/sqlite_hier_child_revoke_probe_guest.c" "$LIBCAPSTONE_C"

"$GUEST_CC" -static -nostdlib -fPIC \
  -I"$PROBE_DIR" \
  -o "$OUT_DIR/sqlite_hier_child_revoke_probe.smode" \
  "$PROBE_DIR/sqlite_hier_child_revoke_probe.smode.c"

echo "Built sqlite_hier_child_revoke_probe(.user/.smode) in $OUT_DIR"
