...
#                      prints "MKDIR-RMDIR-DONE failures=0" with a PASS status
#                      line, AND the control fails its first mkdir
#
# The control is the same test linked against the hostcall.c of 2877b2d4cc9a
# (the runtime before this; pinned, so it stays a control after this lands),
# which has no mkdirat: mkdir comes back ENOSYS there. It differs from the
# test in that one file only -- the host serves PATH_MKDIR in both arms -- so a
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/mkdir-rmdir}
PYTHON=${PYTHON:-python3}
CONTROL=2877b2d4cc9a
...
# The control's one difference: $CONTROL's hostcall.c, without mkdirat. Its
...
if grep -q 'SYS_mkdirat' "$O/hostcall-control.c"; then
  echo "$CONTROL's hostcall.c serves mkdirat; this control no longer controls anything" >&2
  exit 2
fi
...
"$CAPSTONE_CLANG" "${CF[@]}" -std=c11 -O2 -c "$HERE/mkdir_rmdir.c" -o "$O/mkdir_rmdir.o"
COMMON=("$O/start-musl.o" "$O/tls.o" "$O/set_thread_area.o" "$O/setjmp.o"
        "${MUSL_OVERRIDE_OBJS[@]}"
        "$O/level0.o" "${softfloat_objs[@]}" "$O/entry.o" "$O/mkdir_rmdir.o")
rm -rf "$OUT/share"/*.dom "$OUT/share/mkdirtest" "$OUT/share/mkdir-rel"
for arm in test control; do
  ...
  "$CAPSTONE_LD_LLD" --gc-sections -T "$REPO/capstone/my_first_domain/link.ld" \
    -o "$OUT/share/mk-$arm.dom" "${COMMON[@]}" "$hc" "$ARCHIVE"
done

# The host: libc-test's, built from this tree, so it serves PATH_MKDIR in both arms.
...
GUEST='echo __BOOT_OK__; cp /mnt/host/lt.user /tmp/lt.user && chmod 0755 /tmp/lt.user; for d in mk-test mk-control; do echo RUN-BEGIN $d; /tmp/lt.user /mnt/host/$d.dom 60; echo RUN-END $d rc=$?; done; echo __ALL_DONE__'
set +e
...
arm_log() { sed -n "/RUN-BEGIN mk-$1/,/RUN-END mk-$1/p" "$LOG"; }
verdict=0
if grep -aq "MKDIR-RMDIR-DONE failures=0" <(arm_log test) &&
   grep -aq "LT-RESULT mk-test.dom status=0 .*PASS" "$LOG"; then
  echo "  test:    PASS"
else
  echo "  test:    FAIL"; verdict=1
fi
arm_log test | grep -aE "^MKDIR-RMDIR|halted|LT-RESULT" | sed 's/^/    /' || true
if grep -aq "MKDIR-RMDIR FAIL mkdir " <(arm_log control) &&
   grep -aq "MKDIR-RMDIR-DONE" <(arm_log control); then
  echo "  control: FAILS mkdir, as it must without the opcode"
else
  echo "  control: did NOT fail mkdir -- the test cannot tell PATH_MKDIR from its absence"; verdict=1
fi
arm_log control | grep -aE "^MKDIR-RMDIR|halted|LT-RESULT" | sed 's/^/    /' || true
