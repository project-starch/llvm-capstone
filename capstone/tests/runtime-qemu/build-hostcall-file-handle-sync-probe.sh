...
# Build the first handle-based FILE_SYNC proof: open, write, sync, and close.
...
PROBE_DIR="$SCRIPT_DIR/hostcall-file-handle-sync-probe"
...
"$GUEST_CC" \
  ...
  -o "$OUT_DIR/hostcall_file_handle_sync_probe.user" \
  "$PROBE_DIR/hostcall_file_handle_sync_probe_guest.c" \
  "$LIBCAPSTONE_C"

"$GUEST_CC" \
  -static -nostdlib -fPIC \
  -I"$PROBE_DIR" \
  -o "$OUT_DIR/hostcall_file_handle_sync_probe.smode" \
  "$PROBE_DIR/hostcall_file_handle_sync_probe.smode.c"

printf 'Built %s\n' "$OUT_DIR/hostcall_file_handle_sync_probe.user"
printf 'Built %s\n' "$OUT_DIR/hostcall_file_handle_sync_probe.smode"
