#!/usr/bin/env bash
# pread, pwrite, preadv and pwritev in a musl domain, under QEMU.
#
#   bash run.sh        exit 0 only if the domain built with this tree's runtime
#                      prints "PREAD-PWRITE-DONE failures=0" with a PASS status
#                      line, AND the control fails its first pwrite
#
# The control is the same test linked against the hostcall.c of 54012556b1f1
# (the runtime before this; pinned, so it stays a control after this lands),
# which has no pread64/pwrite64/preadv/pwritev: they come back ENOSYS there. It
# differs from the test in that one file only, so a run that passes both ways is
# not being served by the new cases at all, and the run says so.
#
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/pread-pwrite}
PYTHON=${PYTHON:-python3}
CONTROL=54012556b1f1
...
# The control's one difference: $CONTROL's hostcall.c, without the four cases.
...
if grep -qE 'SYS_pread64|SYS_pwritev' "$O/hostcall-control.c"; then
  echo "$CONTROL's hostcall.c serves pread/pwrite; this control no longer controls anything" >&2
  exit 2
fi
...
"$CAPSTONE_CLANG" "${CF[@]}" -std=c11 -O2 -c "$HERE/pread_pwrite.c" -o "$O/pread_pwrite.o"
COMMON=("$O/start-musl.o" "$O/tls.o" "$O/set_thread_area.o" "$O/setjmp.o"
        "${MUSL_OVERRIDE_OBJS[@]}"
        "$O/level0.o" "${softfloat_objs[@]}" "$O/entry.o" "$O/pread_pwrite.o")
rm -f "$OUT/share"/*.dom "$OUT/share/pread-pwrite.dat"
for arm in test control; do
  ...
  "$CAPSTONE_LD_LLD" --gc-sections -T "$REPO/capstone/my_first_domain/link.ld" \
    -o "$OUT/share/pp-$arm.dom" "${COMMON[@]}" "$hc" "$ARCHIVE"
done

# The host: libc-test's, built from this tree; FILE_READ/FILE_WRITE have always
# taken an offset, so the host is the same in both arms and needs no change.
...
GUEST='echo __BOOT_OK__; cp /mnt/host/lt.user /tmp/lt.user && chmod 0755 /tmp/lt.user; for d in pp-test pp-control; do echo RUN-BEGIN $d; /tmp/lt.user /mnt/host/$d.dom 60; echo RUN-END $d rc=$?; done; echo __ALL_DONE__'
set +e
...
arm_log() { sed -n "/RUN-BEGIN pp-$1/,/RUN-END pp-$1/p" "$LOG"; }
verdict=0
if grep -aq "PREAD-PWRITE-DONE failures=0" <(arm_log test) &&
   grep -aq "LT-RESULT pp-test.dom status=0 .*PASS" "$LOG"; then
  echo "  test:    PASS"
else
  echo "  test:    FAIL"; verdict=1
fi
arm_log test | grep -aE "^PREAD-PWRITE|halted|LT-RESULT" | sed 's/^/    /'
if grep -aq "PREAD-PWRITE FAIL pwrite " <(arm_log control) &&
   grep -aq "PREAD-PWRITE-DONE" <(arm_log control); then
  echo "  control: FAILS pwrite, as it must without the cases"
else
  echo "  control: did NOT fail pwrite -- the test cannot tell the cases from their absence"; verdict=1
fi
arm_log control | grep -aE "^PREAD-PWRITE|halted|LT-RESULT" | sed 's/^/    /'
