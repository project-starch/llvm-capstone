...
#define S_IFMT_LOCAL 0170000UL
#define S_IFREG_LOCAL 0100000UL
...
static void start_impl(void) {
  ...
  struct hc_file_truncate_req_v0 *truncate_req;
  struct hc_file_stat_basic_req_v0 *stat_req;
  struct hc_file_stat_basic_resp_v0 *stat_resp;
  ...
  open_req->mode = 0;
  copy_bytes(open_req->path, HOSTCALL_FILE_HANDLE_TRUNCATE_PROBE_INPUT_PATH,
             HOSTCALL_FILE_HANDLE_TRUNCATE_PROBE_INPUT_PATH_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_OPEN;
  metadata->offset = HC_FILE_OPEN_REQ_V0_PATH_OFFSET;
  metadata->length = HOSTCALL_FILE_HANDLE_TRUNCATE_PROBE_INPUT_PATH_LEN;
  metadata->result = 0;
  metadata->error = 0;
  ...
  truncate_req = (struct hc_file_truncate_req_v0 *)payload;
  truncate_req->handle = handle_token;
  truncate_req->size = HOSTCALL_FILE_HANDLE_TRUNCATE_PROBE_TARGET_SIZE;
  truncate_req->flags = 0;
  truncate_req->reserved0 = 0;
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_TRUNCATE;
  metadata->offset = 0;
  metadata->length = 0;
  metadata->result = 0;
  metadata->error = 0;
  ...
  stat_req = (struct hc_file_stat_basic_req_v0 *)payload;
  stat_req->handle = handle_token;
  stat_req->flags = 0;
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_STAT_BASIC;
  metadata->offset = 0;
  metadata->length = 0;
  metadata->result = 0;
  metadata->error = 0;
  ...
  if (metadata->phase != HC_V0_PHASE_RESP || metadata->error != 0 ||
      metadata->result != 0 || metadata->offset != 0 ||
      metadata->length != HC_FILE_STAT_BASIC_RESP_V0_SIZE) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }

  stat_resp = (struct hc_file_stat_basic_resp_v0 *)payload;
  if (stat_resp->file_size != HOSTCALL_FILE_HANDLE_TRUNCATE_PROBE_TARGET_SIZE) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }
  if ((stat_resp->mode & S_IFMT_LOCAL) != S_IFREG_LOCAL) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }
