#!/usr/bin/env bash
# Build the large-region probe (bigregion/bigregion_host.c) with the guest toolchain, linking the
...
mkdir -p "$OUT_DIR"
"$GUEST_CC" -O2 -I"$MODCAPSTONE_INCLUDE" -o "$OUT_DIR/bigregion.user" \
  "$SCRIPT_DIR/bigregion/bigregion_host.c" "$LIBCAPSTONE_C"
printf 'Built %s\n' "$OUT_DIR/bigregion.user"
