...
static void stage_path_delete(const char *path, hostcall_u64_t path_len) {
  struct hc_path_delete_req_v0 *delete_req = (struct hc_path_delete_req_v0 *)payload;

  delete_req->flags = HC_PATH_DELETE_FLAG_NONE;
  copy_bytes(delete_req->path, path, path_len);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_PATH_DELETE;
  metadata->offset = HC_PATH_DELETE_REQ_V0_PATH_OFFSET;
  metadata->length = path_len;
  metadata->result = 0;
  metadata->error = 0;
}

static void stage_path_access(const char *path, hostcall_u64_t path_len) {
  struct hc_path_access_req_v0 *access_req = (struct hc_path_access_req_v0 *)payload;

  access_req->flags = HC_PATH_ACCESS_FLAG_EXISTS;
  copy_bytes(access_req->path, path, path_len);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_PATH_ACCESS;
  metadata->offset = HC_PATH_ACCESS_REQ_V0_PATH_OFFSET;
  metadata->length = path_len;
  metadata->result = 0;
  metadata->error = 0;
}
...
static void start_impl(void) {
  ...
  stage_path_delete(HOSTCALL_PATH_DELETE_PROBE_PATH,
                    HOSTCALL_PATH_DELETE_PROBE_PATH_LEN);
  dom_return(HC_V0_RET_PENDING);

  if (metadata->phase != HC_V0_PHASE_RESP || metadata->error != 0 ||
      metadata->result != 0 || metadata->offset != 0 || metadata->length != 0) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }

  stage_path_access(HOSTCALL_PATH_DELETE_PROBE_PATH,
                    HOSTCALL_PATH_DELETE_PROBE_PATH_LEN);
  dom_return(HC_V0_RET_PENDING);
