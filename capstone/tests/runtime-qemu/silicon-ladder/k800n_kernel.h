...
static unsigned k800n_len(const char *s){unsigned n=0;while(s&&s[n])n++;return n;}
static unsigned k800n_compute(void)
{
  ...
  for (i = 0; i < 4; i++)
    if (a[i].z && a[i].y && k800n_len(a[i].z) > 0 && k800n_len(a[i].y) > 0) ok++;
