#ifndef LOCFN_KERNEL_H
#define LOCFN_KERNEL_H
 ...
 * pointer-bearing aggregate. LOCFN_N is the only thing that changes between variants; grow it
...
static volatile unsigned locfn_gate = 1u;  /* satisfies the ldc gp[i] build gate */
static int locfn_f0(void) { return 3; }
static int locfn_f1(void) { return 5; }
static int locfn_f2(void) { return 7; }
static int locfn_f3(void) { return 11; }

struct locfn_ent { int (*fn)(void); int tag; };  /* fn ptr only, NO string ptr */

static unsigned locfn_compute(void) {
  ...
  struct locfn_ent a[LOCFN_N];
  unsigned i, s = 0;
  for (i = 0; i < LOCFN_N; i++) {
    a[i].fn   = (i & 2) ? ((i & 1) ? locfn_f0 : locfn_f1)
                        : ((i & 1) ? locfn_f2 : locfn_f3);
    a[i].tag  = (int)(i * 3u + 1u);
  }
  for (i = 0; i < LOCFN_N; i++) {
    s += (unsigned)a[i].fn() + (unsigned)a[i].tag;
  }
  return s + locfn_gate - 1u;
}
#endif
