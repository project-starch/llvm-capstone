...
#ifdef S06AGGF_WITH_FIXED_MEMCPY
...
struct s06aggf_s { void *p; unsigned long x; unsigned long y; };

__attribute__((aligned(16))) static struct s06aggf_s s06aggf_src;
__attribute__((aligned(16))) static struct s06aggf_s s06aggf_dst;
__attribute__((aligned(16))) static unsigned long   s06aggf_target[4];

#define S06AGGF_X 0x1111222233334444UL
#define S06AGGF_Y 0x5555666677778888UL

static unsigned s06aggf_compute(void)
{
  volatile unsigned long *v;
  unsigned r = 0;

  s06aggf_target[0] = 0xABCDEFUL;
  s06aggf_src.p = (void *)s06aggf_target;      /* chunk 0: a real capability */
  s06aggf_src.x = S06AGGF_X;                   /* chunk 1 low  half */
  s06aggf_src.y = S06AGGF_Y;                   /* chunk 1 high half */

  s06aggf_dst = s06aggf_src;                   /* THE CONSTRUCT: aggregate assignment */

  v = (volatile unsigned long *)&s06aggf_dst.x;
  if (*v != S06AGGF_X) r |= 1u;
  v = (volatile unsigned long *)&s06aggf_dst.y;
  if (*v != S06AGGF_Y) r |= 2u;
  return 64u + r;
}
#endif
