#ifndef CLP4_KERNEL_H
#define CLP4_KERNEL_H
/* SIMPLEST R-14 probe: read ONE global 4 time(s) in a loop, accumulate, return.
 ...
 * so this executes 4 dynamic loads from the SAME cap-table slot.
 ...
 * Pairs with cdif8 (8 loads, 8 DISTINCT slots, straight-line). Expect 260. */
static char g0[2] = { 'A', 0 };
static unsigned clp4_compute(void)
{
  unsigned r = 0, i;
  for (i = 0; i < 4; i++) r += (unsigned)(unsigned char)g0[0];
  return r;
}
#endif
