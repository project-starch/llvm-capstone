#ifndef BIGMANY_KERNEL_H
#define BIGMANY_KERNEL_H
/* MANY globals AT SQLite's SIZE -- the one combination nothing else covers.
     bigwin   1 global    2 MiB   PASSES on silicon
     gpn8..64 many        128 KiB (the board bisection)
     SQLite   1059        2 MiB   HANGS
   Built with DOMAIN_WINDOW=0x140000 so tot_size is 2 MiB (order 9) exactly as SQLite's,
   while the globals stay trivial. If this hangs, it is a count-x-size interaction and
   SQLite's content is irrelevant; if it passes, the remaining suspect is SQLite's own
   78 KB initializer blob (bigwin's is 96 bytes). */
...
static unsigned bigmany_compute(void) {
