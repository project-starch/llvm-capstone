...
static void start_impl(void) {
  ...
  open_req->mode = 0644;
  copy_bytes(open_req->path, HOSTCALL_FILE_HANDLE_WRITE_PROBE_OUTPUT_PATH,
             HOSTCALL_FILE_HANDLE_WRITE_PROBE_OUTPUT_PATH_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_OPEN;
  metadata->offset = HC_FILE_OPEN_REQ_V0_PATH_OFFSET;
  metadata->length = HOSTCALL_FILE_HANDLE_WRITE_PROBE_OUTPUT_PATH_LEN;
  metadata->result = 0;
  metadata->error = 0;
  ...
  copy_bytes((char *)write_req->data, HOSTCALL_FILE_HANDLE_WRITE_PROBE_MESSAGE,
             HOSTCALL_FILE_HANDLE_WRITE_PROBE_MESSAGE_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_WRITE;
  metadata->offset = HC_FILE_WRITE_REQ_V0_DATA_OFFSET;
  metadata->length = HOSTCALL_FILE_HANDLE_WRITE_PROBE_MESSAGE_LEN;
  metadata->result = 0;
  metadata->error = 0;
  dom_return(HC_V0_RET_PENDING);

  if (metadata->phase != HC_V0_PHASE_RESP || metadata->error != 0 ||
      metadata->result != HOSTCALL_FILE_HANDLE_WRITE_PROBE_MESSAGE_LEN) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }
