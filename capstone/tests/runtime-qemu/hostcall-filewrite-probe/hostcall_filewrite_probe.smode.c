...
/* Resolved lazily from the tail of the region list shared by the helper. */
...
static void init_regions(void) {
  /* The helper still shares metadata first and payload second. */
...
static void start_impl(void) {
  ...
  /*
   * Round 1: request the second HostCall opcode while keeping the same metadata
   * layout and the same borrowed payload direction as the stdout proof.
   */
  copy_bytes(payload, HOSTCALL_FILEWRITE_PROBE_MESSAGE,
             HOSTCALL_FILEWRITE_PROBE_MESSAGE_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_WRITE_GUEST_TMPFILE;
  metadata->offset = 0;
  metadata->length = HOSTCALL_FILEWRITE_PROBE_MESSAGE_LEN;
  metadata->result = 0;
  metadata->error = 0;

  dom_return(HC_V0_RET_PENDING);

  /* Round 2: validate the helper-side filewrite result from metadata only. */
  if (metadata->phase != HC_V0_PHASE_RESP || metadata->error != 0 ||
      metadata->result != HOSTCALL_FILEWRITE_PROBE_MESSAGE_LEN) {
