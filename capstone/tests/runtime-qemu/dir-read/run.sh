...
#
# The control is the same test linked against the hostcall.c of 40eefa09420c
# (dev before DIR_READ; pinned, so it stays a control after this lands), which
...
# The control's one difference: 40eefa09420c's hostcall.c, without getdents64.
...
if grep -q SYS_getdents64 "$O/hostcall-control.c"; then
  echo "40eefa09420c's hostcall.c serves getdents64; this control no longer controls anything" >&2
  exit 2
fi
