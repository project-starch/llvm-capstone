#!/usr/bin/env bash
# fstat on the standard descriptors of a musl domain, under QEMU.
#
#   bash run.sh        exit 0 only if the domain built with this tree's runtime
#                      prints "STDIO-FSTAT-DONE failures=0" with a PASS status
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/stdio-fstat}
...
# The control's one difference: origin/dev's hostcall.c, whose fstat knows no stdout.
...
if grep -q "S_IFCHR" "$O/hostcall-control.c"; then
  echo "origin/dev's hostcall.c describes stdout already; this control no longer controls anything" >&2
  exit 2
fi
...
"$CAPSTONE_CLANG" "${CF[@]}" -std=c11 -O2 -c "$HERE/stdio_fstat.c" -o "$O/stdio_fstat.o"
COMMON=("$O/start-musl.o" "$O/tls.o" "$O/set_thread_area.o" "$O/setjmp.o"
        ...
        "$O/level0.o" "${softfloat_objs[@]}" "$O/entry.o" "$O/stdio_fstat.o")
rm -f "$OUT/share"/*.dom
...
verdict=0
if grep -aq "STDIO-FSTAT-DONE failures=0" <(arm_log test) &&
   grep -aq "LT-RESULT fs-test.dom status=0 .*PASS" "$LOG"; then
  echo "  test:    PASS"
else
  echo "  test:    FAIL"; verdict=1
fi
arm_log test | grep -aE "^STDIO-FSTAT|halted|LT-RESULT" | sed 's/^/    /'
if grep -aq "STDIO-FSTAT FAIL fstat-stdout" <(arm_log control) &&
   grep -aq "STDIO-FSTAT-DONE" <(arm_log control); then
...
arm_log control | grep -aE "^STDIO-FSTAT|halted|LT-RESULT" | sed 's/^/    /'
