...
/* Host binding payload (borrower). Round 1: read the borrowed child (a statement
 * value that lives inside the connection) and cache the pointer in a .bss slot.
 * Round 2 (after the engine revokes the parent/connection): re-read the cached
 * child pointer -- the use-after-close. If the parent revoke cascaded to the
 * derived child, the cached capability reloads untagged and this read faults; the
 * monitor terminates the domain and the engine sees the fault sentinel instead of
 * a stale value. The child was shared last, so REGION_COUNT-1 resolves to it. */
...
static unsigned long *borrowed_child_base(void) {
  struct sbiret count = sbi_ecall(SBI_EXT_CAPSTONE,
                                  ...
                                  0);
  /* The derived child was shared last -> highest region index. */
...
static void start_impl(void) {
  /* Round 1: read the child while the connection is open; cache the pointer. */
   ...
   * pointer -> use-after-close. The cached capability reloads untagged; this
   * read faults. */
