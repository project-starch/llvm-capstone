#!/usr/bin/env bash
# readlink() in a musl domain, through PATH_READLINK, and realpath() on top of it, under QEMU.
#
#   bash run.sh        exit 0 only if the domain built with this tree's runtime
#                      prints "PATH-READLINK-DONE failures=0" with a PASS status
#                      line, AND the control fails its first readlink
#
# The control is the same test linked against the hostcall.c of 5e4a9ed08ce0
# (the runtime before this; pinned, so it stays a control after this lands),
# which has no readlinkat: readlink comes back ENOSYS there. It differs from
# the test in that one file only -- the host serves PATH_READLINK in both arms
# -- so a run that passes both ways is not being served by the new opcode at
# all, and the run says so. The share carries rltest/link.txt -> inside.txt.
#
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/path-readlink}
PYTHON=${PYTHON:-python3}
CONTROL=5e4a9ed08ce0
...
# The control's one difference: $CONTROL's hostcall.c, without readlinkat. Its
...
if grep -q 'SYS_readlinkat' "$O/hostcall-control.c"; then
  echo "$CONTROL's hostcall.c serves readlinkat; this control no longer controls anything" >&2
  exit 2
fi
...
"$CAPSTONE_CLANG" "${CF[@]}" -std=c11 -O2 -c "$HERE/path_readlink.c" -o "$O/path_readlink.o"
COMMON=("$O/start-musl.o" "$O/tls.o" "$O/set_thread_area.o" "$O/setjmp.o"
        "${MUSL_OVERRIDE_OBJS[@]}"
        "$O/level0.o" "${softfloat_objs[@]}" "$O/entry.o" "$O/path_readlink.o")
rm -rf "$OUT/share"/*.dom "$OUT/share/rltest"
mkdir -p "$OUT/share/rltest"; printf inside > "$OUT/share/rltest/inside.txt"; ln -s inside.txt "$OUT/share/rltest/link.txt"
for arm in test control; do
  ...
  "$CAPSTONE_LD_LLD" --gc-sections -T "$REPO/capstone/my_first_domain/link.ld" \
    -o "$OUT/share/rl-$arm.dom" "${COMMON[@]}" "$hc" "$ARCHIVE"
done

# The host: libc-test's, built from this tree, so it serves PATH_READLINK in both arms.
...
GUEST='echo __BOOT_OK__; cp /mnt/host/lt.user /tmp/lt.user && chmod 0755 /tmp/lt.user; for d in rl-test rl-control; do echo RUN-BEGIN $d; /tmp/lt.user /mnt/host/$d.dom 60; echo RUN-END $d rc=$?; done; echo __ALL_DONE__'
set +e
...
arm_log() { sed -n "/RUN-BEGIN rl-$1/,/RUN-END rl-$1/p" "$LOG"; }
verdict=0
if grep -aq "PATH-READLINK-DONE failures=0" <(arm_log test) &&
   grep -aq "LT-RESULT rl-test.dom status=0 .*PASS" "$LOG"; then
  echo "  test:    PASS"
else
  echo "  test:    FAIL"; verdict=1
fi
arm_log test | grep -aE "^PATH-READLINK|halted|LT-RESULT" | sed 's/^/    /' || true
if grep -aq "PATH-READLINK FAIL readlink " <(arm_log control) &&
   grep -aq "PATH-READLINK-DONE" <(arm_log control); then
  echo "  control: FAILS readlink, as it must without the opcode"
else
  echo "  control: did NOT fail readlink -- the test cannot tell PATH_READLINK from its absence"; verdict=1
fi
arm_log control | grep -aE "^PATH-READLINK|halted|LT-RESULT" | sed 's/^/    /' || true
