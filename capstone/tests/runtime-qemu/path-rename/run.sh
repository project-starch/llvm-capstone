#!/usr/bin/env bash
# rename() in a musl domain, through the hostcall's PATH_RENAME, under QEMU.
#
#   bash run.sh        exit 0 only if the domain built with this tree's runtime
#                      prints "PATH-RENAME-DONE failures=0" with a PASS status
#                      line, AND the control fails its first rename
#
# The control is the same test linked against the hostcall.c of 089fa1108256
# (the runtime before this; pinned, so it stays a control after this lands),
# which has no renameat2: rename comes back ENOSYS there. It differs from the
# test in that one file only -- the host serves PATH_RENAME in both arms -- so a
# run that passes both ways is not being served by the new opcode at all, and
# the run says so.
#
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/path-rename}
PYTHON=${PYTHON:-python3}
CONTROL=089fa1108256
...
# The control's one difference: $CONTROL's hostcall.c, without renameat2. Its
...
if grep -q 'SYS_renameat2' "$O/hostcall-control.c"; then
  echo "$CONTROL's hostcall.c serves renameat2; this control no longer controls anything" >&2
  exit 2
fi
...
"$CAPSTONE_CLANG" "${CF[@]}" -std=c11 -O2 -c "$HERE/path_rename.c" -o "$O/path_rename.o"
COMMON=("$O/start-musl.o" "$O/tls.o" "$O/set_thread_area.o" "$O/setjmp.o"
        "${MUSL_OVERRIDE_OBJS[@]}"
        "$O/level0.o" "${softfloat_objs[@]}" "$O/entry.o" "$O/path_rename.o")
rm -f "$OUT/share"/*.dom "$OUT/share"/rename-*.dat
for arm in test control; do
  ...
  "$CAPSTONE_LD_LLD" --gc-sections -T "$REPO/capstone/my_first_domain/link.ld" \
    -o "$OUT/share/rn-$arm.dom" "${COMMON[@]}" "$hc" "$ARCHIVE"
done

# The host: libc-test's, built from this tree, so it serves PATH_RENAME in both arms.
...
GUEST='echo __BOOT_OK__; cp /mnt/host/lt.user /tmp/lt.user && chmod 0755 /tmp/lt.user; for d in rn-test rn-control; do echo RUN-BEGIN $d; /tmp/lt.user /mnt/host/$d.dom 60; echo RUN-END $d rc=$?; done; echo __ALL_DONE__'
set +e
...
arm_log() { sed -n "/RUN-BEGIN rn-$1/,/RUN-END rn-$1/p" "$LOG"; }
verdict=0
if grep -aq "PATH-RENAME-DONE failures=0" <(arm_log test) &&
   grep -aq "LT-RESULT rn-test.dom status=0 .*PASS" "$LOG"; then
  echo "  test:    PASS"
else
  echo "  test:    FAIL"; verdict=1
fi
arm_log test | grep -aE "^PATH-RENAME|halted|LT-RESULT" | sed 's/^/    /'
if grep -aq "PATH-RENAME FAIL rename " <(arm_log control) &&
   grep -aq "PATH-RENAME-DONE" <(arm_log control); then
  echo "  control: FAILS rename, as it must without the opcode"
else
  echo "  control: did NOT fail rename -- the test cannot tell PATH_RENAME from its absence"; verdict=1
fi
arm_log control | grep -aE "^PATH-RENAME|halted|LT-RESULT" | sed 's/^/    /'
