...
static void start_impl(void) {
  ...
  copy_bytes(payload, HOSTCALL_SECOND_PENDING_PAYLOAD_STAGE1_TEXT,
             HOSTCALL_SECOND_PENDING_PAYLOAD_STAGE1_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_SECOND_PENDING_PAYLOAD_STAGE1;
  metadata->offset = 0;
  metadata->length = HOSTCALL_SECOND_PENDING_PAYLOAD_STAGE1_LEN;
  metadata->result = 0;
  metadata->error = 0;
  ...
  copy_bytes(payload, HOSTCALL_SECOND_PENDING_PAYLOAD_STAGE2_TEXT,
             HOSTCALL_SECOND_PENDING_PAYLOAD_STAGE2_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_SECOND_PENDING_PAYLOAD_STAGE2;
  metadata->offset = 0;
  metadata->length = HOSTCALL_SECOND_PENDING_PAYLOAD_STAGE2_LEN;
  metadata->result = 0;
  metadata->error = 0;
  dom_return(HC_V0_RET_PENDING);
