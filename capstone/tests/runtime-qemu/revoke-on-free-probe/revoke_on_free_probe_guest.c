...
/* Controller for the Phase-0 revoke-on-free allocator probe. Creates one region
 * and TRANSFERS it (linear, RW) into a single domain, which runs the whole
 * malloc/free/use-after-free lifecycle intra-domain. No host read of the arena
 * (every fault probe revokes part of it; see intra-domain-mrev-revoke-probe's
 * "Gap found").
 */
...
#define TAG "revoke-on-free-probe"
...
int main(int argc, char **argv) {
  ...
  region_id_t region_id = create_region(ROF_PROBE_REGION_SIZE);
  unsigned char *region_bytes =
      (unsigned char *)map_region(region_id, ROF_PROBE_REGION_SIZE);
  if (!region_bytes)
    return fail_cleanup("map_region failed", 0);
  memset(region_bytes, 0, ROF_PROBE_REGION_SIZE);
  print_nobuf("%s: created region ID = %lu\n", TAG, region_id);

  shared_region_annotated(dom_id, region_id, ROF_PROBE_ANNOTATION_PERM_INOUT,
                          ROF_PROBE_ANNOTATION_REV_TRANSFERRED);
