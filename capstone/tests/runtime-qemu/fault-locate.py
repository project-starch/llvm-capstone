...
Line numbers need an image built with line tables (`-gline-tables-only`); a port may
expose that as a build knob. Debug sections are non-alloc, so such an image can be a
twin of the one that actually ran -- CHECK that .text matches rather than assuming it:
...
def text_symbols(nm, image):
    ...
    for line in out.splitlines():
        ...
        # symbol is how a fault was once reported as living in a DATA symbol, and the
        # wrong reading survived a whole session.
...
def show_source(symbolizer, image, addr, what):
    ...
    if not useful:
        print(f"\nno line table for the {what}. Rebuild the image with line tables "
              "(-gline-tables-only) and check .text still matches.")
        return
