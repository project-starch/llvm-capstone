#!/usr/bin/env bash
# Build the M-2 positive control host program (m2-region-overflow/m2_region_overflow_host.c) with
# the guest toolchain, linking the buildroot tree's libcapstone.c by path like every other probe.
set -euo pipefail
...
mkdir -p "$OUT_DIR"
"$GUEST_CC" -O2 -I"$MODCAPSTONE_INCLUDE" -o "$OUT_DIR/m2_region_overflow.user" \
  "$SCRIPT_DIR/m2-region-overflow/m2_region_overflow_host.c" "$LIBCAPSTONE_C"
printf 'Built %s\n' "$OUT_DIR/m2_region_overflow.user"
