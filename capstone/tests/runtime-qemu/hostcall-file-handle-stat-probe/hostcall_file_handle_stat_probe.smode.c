...
static void start_impl(void) {
  ...
  open_req->mode = 0;
  copy_bytes(open_req->path, HOSTCALL_FILE_HANDLE_STAT_PROBE_INPUT_PATH,
             HOSTCALL_FILE_HANDLE_STAT_PROBE_INPUT_PATH_LEN);
  metadata->phase = HC_V0_PHASE_REQ;
  metadata->opcode = HC_V0_OP_FILE_OPEN;
  metadata->offset = HC_FILE_OPEN_REQ_V0_PATH_OFFSET;
  metadata->length = HOSTCALL_FILE_HANDLE_STAT_PROBE_INPUT_PATH_LEN;
  metadata->result = 0;
  metadata->error = 0;
  ...
  if (stat_resp->file_size != HOSTCALL_FILE_HANDLE_STAT_PROBE_MESSAGE_LEN) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }
