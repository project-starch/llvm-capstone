...
static int fail_cleanup(const char *message, unsigned long observed,
  ...
  fprintf(stderr,
          "hostcall-file-handle-stat-probe: %s (observed=%lu)\n",
          message, observed);
  if (metadata) {
    fprintf(stderr,
            "hostcall-file-handle-stat-probe: metadata{phase=%llu opcode=%llu offset=%llu length=%llu result=%lld error=%lld}\n",
...
int main(int argc, char **argv) {
  ...
  if (capstone_init()) {
    fprintf(stderr,
            "hostcall-file-handle-stat-probe: failed to initialize Capstone\n");
    return 1;
  }
  ...
  print_nobuf("hostcall-file-handle-stat-probe: created domain ID = %lu\n",
              dom_id);
  print_nobuf("hostcall-file-handle-stat-probe: metadata region ID = %lu\n",
              metadata_region_id);
  print_nobuf("hostcall-file-handle-stat-probe: payload region ID = %lu\n",
              payload_region_id);
  ...
  print_nobuf(
      "hostcall-file-handle-stat-probe: metadata shared, payload borrowed-out for open request\n");

  ret1 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-stat-probe: first call retval = %lu\n",
              ret1);
  ...
  print_nobuf(
      "hostcall-file-handle-stat-probe: servicing HC_V0_OP_FILE_OPEN for %s\n",
      path_snapshot);
  ...
  print_nobuf(
      "hostcall-file-handle-stat-probe: opened helper handle token = %llu\n",
      handle_token);
  ...
  shared_region_annotated(dom_id, payload_region_id,
                          ...
                          HOSTCALL_STDOUT_PROBE_ANNOTATION_REV_BORROWED);
  print_nobuf(
      "hostcall-file-handle-stat-probe: payload revoked and re-shared for stat request\n");

  ret2 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-stat-probe: second call retval = %lu\n",
              ret2);
  ...
  print_nobuf(
      "hostcall-file-handle-stat-probe: servicing HC_V0_OP_FILE_STAT_BASIC for token %llu\n",
      handle_token);
  ...
  if (fd < 0) {
    ...
    metadata->error = errno;
    metadata->phase = HC_V0_PHASE_ERROR;
    return fail_cleanup("lookup stat handle failed", (unsigned long)errno,
                        metadata);
  }
  ...
  shared_region_annotated(dom_id, payload_region_id,
                          ...
                          HOSTCALL_STDOUT_PROBE_ANNOTATION_REV_BORROWED);
  print_nobuf(
      "hostcall-file-handle-stat-probe: payload revoked and re-shared for stat response plus close request\n");
  ...
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-stat-probe: third call retval = %lu\n",
              ret3);
  ...
  print_nobuf(
      "hostcall-file-handle-stat-probe: servicing HC_V0_OP_FILE_CLOSE for token %llu\n",
      handle_token);
  ...
  print_nobuf("hostcall-file-handle-stat-probe: fourth call retval = %lu\n",
              ret4);
  if (ret4 != HC_V0_RET_DONE)
    return fail_cleanup("unexpected fourth call retval", ret4, metadata);
  ...
  print_nobuf("hostcall-file-handle-stat-probe: success\n");
  print_nobuf("__HOSTCALL_FILE_HANDLE_STAT_OK__\n");

  if (capstone_cleanup()) {
    fprintf(stderr,
            "hostcall-file-handle-stat-probe: failed to clean up Capstone\n");
    return 1;
  }
