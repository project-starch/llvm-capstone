#!/usr/bin/env bash
# 8/16-bit atomics through capabilities, run in a musl domain under QEMU (C-51).
#
#   bash run.sh        build with $CAPSTONE_CLANG at -O0 and -O2, run both in one
#                      boot, exit 0 only if BOTH print
#                      "SUBWORD-ATOMICS-DONE failures=0" and a PASS status line
#
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/subword-atomics}
...
rm -f "$OUT/share"/*.dom
for lvl in O0 O2; do
  "$CAPSTONE_CLANG" "${CF[@]}" -std=c11 "-$lvl" -c "$HERE/subword_atomics.c" -o "$O/test-$lvl.o"
  "$CAPSTONE_LD_LLD" --gc-sections -T "$REPO/capstone/my_first_domain/link.ld" \
    -o "$OUT/share/subword-$lvl.dom" "${RUNTIME[@]}" "$O/test-$lvl.o" "$ARCHIVE"
done
...
GUEST='echo __BOOT_OK__; cp /mnt/host/lt.user /tmp/lt.user && chmod 0755 /tmp/lt.user; for d in subword-O0 subword-O2; do echo RUN-BEGIN $d; /tmp/lt.user /mnt/host/$d.dom 60; echo RUN-END $d rc=$?; done; echo __ALL_DONE__'
set +e
...
verdict=0
for lvl in O0 O2; do
  if grep -aq "SUBWORD-ATOMICS-DONE failures=0" <(sed -n "/RUN-BEGIN subword-$lvl/,/RUN-END subword-$lvl/p" "$LOG") &&
     grep -aq "LT-RESULT subword-$lvl.dom status=0 .*PASS" "$LOG"; then
    echo "  subword-$lvl: PASS"
  else
    echo "  subword-$lvl: FAIL"
    sed -n "/RUN-BEGIN subword-$lvl/,/RUN-END subword-$lvl/p" "$LOG" | grep -aE "^FAIL|DONE|halted|LT-RESULT" | sed 's/^/    /'
    verdict=1
  fi
done
