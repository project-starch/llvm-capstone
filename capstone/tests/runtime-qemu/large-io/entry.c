/* Domain entry for this test: main() under C's exit semantics, so stdio is
   ...
   One entry for both programs; argv[0] only names them in the output.
   __capstone_at_exit is DEFINED here: on a runtime that only declares it weak,
   the address of an undefined weak symbol is not NULL in a domain (C-56), and
   exit() then jumps to the image base. */
#include <stdlib.h>
