...
static int fail_cleanup(const char *message, unsigned long observed,
  ...
  fprintf(stderr, "hostcall-path-access-probe: %s (observed=%lu)\n", message,
          observed);
  if (metadata) {
    fprintf(stderr,
            "hostcall-path-access-probe: metadata{phase=%llu opcode=%llu offset=%llu length=%llu result=%lld error=%lld}\n",
...
static int service_path_access(struct hostcall_v0 *metadata,
  ...
  print_nobuf(
      "hostcall-path-access-probe: servicing HC_V0_OP_PATH_ACCESS for %s\n",
      path_snapshot);
...
int main(int argc, char **argv) {
  ...
  if (capstone_init()) {
    fprintf(stderr,
            "hostcall-path-access-probe: failed to initialize Capstone\n");
    return 1;
  }
  ...
  print_nobuf("hostcall-path-access-probe: created domain ID = %lu\n", dom_id);
  print_nobuf("hostcall-path-access-probe: metadata region ID = %lu\n",
              metadata_region_id);
  print_nobuf("hostcall-path-access-probe: payload region ID = %lu\n",
              payload_region_id);
  ...
  print_nobuf(
      "hostcall-path-access-probe: metadata shared, payload borrowed-out for first path access request\n");

  ret1 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-path-access-probe: first call retval = %lu\n", ret1);
  ...
  if (!request.length)
    return fail_cleanup("unexpected empty first path",
                        (unsigned long)request.length, &request);
  if (!hostcall_payload_range_valid(&request))
    return fail_cleanup("first path exceeds shared region", 0, &request);
  ...
  shared_region_annotated(dom_id, payload_region_id,
                          ...
                          HOSTCALL_STDOUT_PROBE_ANNOTATION_REV_BORROWED);
  print_nobuf(
      "hostcall-path-access-probe: payload revoked and re-shared for second path access request\n");

  ret2 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-path-access-probe: second call retval = %lu\n", ret2);
  ...
  if (request.offset != HC_PATH_ACCESS_REQ_V0_PATH_OFFSET)
    return fail_cleanup("unexpected second path offset",
                        (unsigned long)request.offset, &request);
  if (!request.length)
    return fail_cleanup("unexpected empty second path",
                        (unsigned long)request.length, &request);
  if (!hostcall_payload_range_valid(&request))
    return fail_cleanup("second path exceeds shared region", 0, &request);
  ...
  print_nobuf("hostcall-path-access-probe: third call retval = %lu\n", ret3);
  ...
  print_nobuf("hostcall-path-access-probe: success\n");
  print_nobuf("__HOSTCALL_PATH_ACCESS_OK__\n");

  if (capstone_cleanup()) {
    fprintf(stderr,
            "hostcall-path-access-probe: failed to clean up Capstone\n");
    return 1;
  }
