...
static void start_impl(void) {
  ...
  stage_path_access(HOSTCALL_PATH_ACCESS_PROBE_EXISTING_PATH,
                    HOSTCALL_PATH_ACCESS_PROBE_EXISTING_PATH_LEN);
  dom_return(HC_V0_RET_PENDING);

  if (metadata->phase != HC_V0_PHASE_RESP || metadata->error != 0 ||
      metadata->result != 1 || metadata->offset != 0 || metadata->length != 0) {
    metadata->phase = HC_V0_PHASE_ERROR;
    dom_return(HC_V0_RET_ERROR);
  }

  stage_path_access(HOSTCALL_PATH_ACCESS_PROBE_MISSING_PATH,
                    HOSTCALL_PATH_ACCESS_PROBE_MISSING_PATH_LEN);
  dom_return(HC_V0_RET_PENDING);
