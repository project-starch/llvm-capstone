#!/usr/bin/env bash
# chdir() and getcwd() in a musl domain: a domain-side working directory, under QEMU.
#
#   bash run.sh        exit 0 only if the domain built with this tree's runtime
#                      prints "CWD-DONE failures=0" with a PASS status line,
#                      AND the control fails its chdir
#
# The control is the same test linked against the hostcall.c of 4b78c47dde82
# (the runtime before this; pinned, so it stays a control after this lands),
# which has no chdir or getcwd: both come back ENOSYS there and every relative
# path stays relative to the helper. It differs from the test in that one file
# only -- the host is the same, nothing changed on the wire -- so a run that
# passes both ways is not being served by the cwd at all, and the run says so.
#
...
OUT=${OUT:-$CAPSTONE_TMP_ROOT/cwd}
PYTHON=${PYTHON:-python3}
CONTROL=4b78c47dde82
...
# The control's one difference: $CONTROL's hostcall.c, without chdir/getcwd. Its
...
if grep -qE 'SYS_chdir|SYS_getcwd' "$O/hostcall-control.c"; then
  echo "$CONTROL's hostcall.c serves chdir; this control no longer controls anything" >&2
  exit 2
fi
...
"$CAPSTONE_CLANG" "${CF[@]}" -std=c11 -O2 -c "$HERE/cwd_test.c" -o "$O/cwd_test.o"
COMMON=("$O/start-musl.o" "$O/tls.o" "$O/set_thread_area.o" "$O/setjmp.o"
        "${MUSL_OVERRIDE_OBJS[@]}"
        "$O/level0.o" "${softfloat_objs[@]}" "$O/entry.o" "$O/cwd_test.o")
rm -f "$OUT/share"/*.dom
for arm in test control; do
  ...
  "$CAPSTONE_LD_LLD" --gc-sections -T "$REPO/capstone/my_first_domain/link.ld" \
    -o "$OUT/share/cwd-$arm.dom" "${COMMON[@]}" "$hc" "$ARCHIVE"
done

# The directory the domain changes into, in the share the guest mounts at /mnt/host.
rm -rf "$OUT/share/cwdtest"
mkdir -p "$OUT/share/cwdtest/sub"
printf inside > "$OUT/share/cwdtest/inside.txt"
printf deep > "$OUT/share/cwdtest/sub/deep.txt"

# The host: libc-test's, built from this tree; nothing changed on the wire, so it
# is the same host in both arms.
...
GUEST='echo __BOOT_OK__; cp /mnt/host/lt.user /tmp/lt.user && chmod 0755 /tmp/lt.user; for d in cwd-test cwd-control; do echo RUN-BEGIN $d; /tmp/lt.user /mnt/host/$d.dom 60; echo RUN-END $d rc=$?; done; echo __ALL_DONE__'
set +e
...
arm_log() { sed -n "/RUN-BEGIN cwd-$1/,/RUN-END cwd-$1/p" "$LOG"; }
verdict=0
if grep -aq "CWD-DONE failures=0" <(arm_log test) &&
   grep -aq "LT-RESULT cwd-test.dom status=0 .*PASS" "$LOG"; then
  echo "  test:    PASS"
else
  echo "  test:    FAIL"; verdict=1
fi
arm_log test | grep -aE "^CWD|halted|LT-RESULT" | sed 's/^/    /'
if grep -aq "CWD FAIL chdir " <(arm_log control) &&
   grep -aq "CWD-DONE" <(arm_log control); then
  echo "  control: FAILS chdir, as it must without a cwd"
else
  echo "  control: did NOT fail chdir -- the test cannot tell the cwd from its absence"; verdict=1
fi
arm_log control | grep -aE "^CWD|halted|LT-RESULT" | sed 's/^/    /'
