...
static int fail_cleanup(const char *message, unsigned long observed,
  ...
  fprintf(stderr,
          "hostcall-file-handle-sync-probe: %s (observed=%lu)\n",
          message, observed);
  if (metadata) {
    fprintf(stderr,
            "hostcall-file-handle-sync-probe: metadata{phase=%llu opcode=%llu offset=%llu length=%llu result=%lld error=%lld}\n",
...
int main(int argc, char **argv) {
  ...
  if (capstone_init()) {
    fprintf(stderr,
            "hostcall-file-handle-sync-probe: failed to initialize Capstone\n");
    return 1;
  }
  ...
  print_nobuf("hostcall-file-handle-sync-probe: created domain ID = %lu\n",
              dom_id);
  print_nobuf("hostcall-file-handle-sync-probe: metadata region ID = %lu\n",
              metadata_region_id);
  print_nobuf("hostcall-file-handle-sync-probe: payload region ID = %lu\n",
              payload_region_id);
  ...
  print_nobuf(
      "hostcall-file-handle-sync-probe: metadata shared, payload borrowed-out for open request\n");

  ret1 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-sync-probe: first call retval = %lu\n",
              ret1);
  ...
  print_nobuf(
      "hostcall-file-handle-sync-probe: servicing HC_V0_OP_FILE_OPEN for %s\n",
      path_snapshot);
  ...
  print_nobuf(
      "hostcall-file-handle-sync-probe: opened helper handle token = %llu\n",
      handle_token);
  ...
  shared_region_annotated(dom_id, payload_region_id,
                          ...
                          HOSTCALL_STDOUT_PROBE_ANNOTATION_REV_BORROWED);
  print_nobuf(
      "hostcall-file-handle-sync-probe: payload revoked and re-shared for write request\n");

  ret2 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-sync-probe: second call retval = %lu\n",
              ret2);
  if (ret2 != HC_V0_RET_PENDING)
    return fail_cleanup("unexpected second call retval", ret2, &request);
  if (request.phase != HC_V0_PHASE_REQ)
    return fail_cleanup("unexpected write phase", (unsigned long)request.phase,
                        &request);
  if (request.opcode != HC_V0_OP_FILE_WRITE)
    return fail_cleanup("unexpected write opcode", (unsigned long)request.opcode,
                        &request);
  ...
  print_nobuf(
      "hostcall-file-handle-sync-probe: servicing HC_V0_OP_FILE_WRITE for token %llu\n",
      handle_token);
  ...
  shared_region_annotated(dom_id, payload_region_id,
                          ...
                          HOSTCALL_STDOUT_PROBE_ANNOTATION_REV_BORROWED);
  print_nobuf(
      "hostcall-file-handle-sync-probe: payload revoked and re-shared for sync request\n");

  ret3 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-sync-probe: third call retval = %lu\n",
              ret3);
  if (ret3 != HC_V0_RET_PENDING)
    return fail_cleanup("unexpected third call retval", ret3, &request);
  if (request.phase != HC_V0_PHASE_REQ)
    return fail_cleanup("unexpected sync phase", (unsigned long)request.phase,
                        &request);
  if (request.opcode != HC_V0_OP_FILE_SYNC)
    return fail_cleanup("unexpected sync opcode", (unsigned long)request.opcode,
                        &request);
  if (request.offset != 0 || request.length != 0)
    return fail_cleanup("unexpected sync offset/length",
                        (unsigned long)request.offset, &request);
  if (((struct hc_file_sync_req_v0 *)payload)->handle != handle_token)
    return fail_cleanup("unexpected sync handle token",
  ...
  print_nobuf(
      "hostcall-file-handle-sync-probe: servicing HC_V0_OP_FILE_SYNC for token %llu\n",
      handle_token);
  fd = hostcall_lookup_handle_fd(handle_slots,
                                 HOSTCALL_FILE_SERVICE_PROBE_MAX_HANDLES,
                                 handle_token);
  ...
  if (fsync(fd) < 0) {
    ...
    metadata->error = errno;
    metadata->phase = HC_V0_PHASE_ERROR;
    return fail_cleanup("sync request failed", (unsigned long)errno, metadata);
  }
  ...
  shared_region_annotated(dom_id, payload_region_id,
                          ...
                          HOSTCALL_STDOUT_PROBE_ANNOTATION_REV_BORROWED);
  print_nobuf(
      "hostcall-file-handle-sync-probe: payload revoked and re-shared for close request\n");

  ret4 = call_dom(dom_id);
  hostcall_snapshot_request(&request, metadata);
  print_nobuf("hostcall-file-handle-sync-probe: fourth call retval = %lu\n",
              ret4);
  ...
  print_nobuf(
      "hostcall-file-handle-sync-probe: servicing HC_V0_OP_FILE_CLOSE for token %llu\n",
      handle_token);
  ...
  print_nobuf("hostcall-file-handle-sync-probe: fifth call retval = %lu\n",
              ret5);
  ...
  print_nobuf("hostcall-file-handle-sync-probe: success\n");
  print_nobuf("__HOSTCALL_FILE_HANDLE_SYNC_OK__\n");

  if (capstone_cleanup()) {
    fprintf(stderr,
            "hostcall-file-handle-sync-probe: failed to clean up Capstone\n");
    return 1;
  }
