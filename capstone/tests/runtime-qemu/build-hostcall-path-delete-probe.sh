...
# Build the first SQLite-facing path-delete proof: delete a path, then verify absence.
...
PROBE_DIR="$SCRIPT_DIR/hostcall-path-delete-probe"
...
"$GUEST_CC" \
  ...
  -o "$OUT_DIR/hostcall_path_delete_probe.user" \
  "$PROBE_DIR/hostcall_path_delete_probe_guest.c" \
  "$LIBCAPSTONE_C"

"$GUEST_CC" \
  -static -nostdlib -fPIC \
  -I"$PROBE_DIR" \
  -o "$OUT_DIR/hostcall_path_delete_probe.smode" \
  "$PROBE_DIR/hostcall_path_delete_probe.smode.c"

printf 'Built %s\n' "$OUT_DIR/hostcall_path_delete_probe.user"
printf 'Built %s\n' "$OUT_DIR/hostcall_path_delete_probe.smode"
