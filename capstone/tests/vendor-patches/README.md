...
| `transcapstone-linux-riscv-arch-force-max-order.patch` | `caplifive-buildroot/components/linux` | The `ARCH_FORCE_MAX_ORDER` Kconfig symbol, which riscv does not carry (still absent in 7.2-rc6, and so are x86 and s390: the 9 arches that do carry it use it to make the *default* track a variable page size, which riscv has no need of). **If the kernel is ever bumped past 6.8, change 13 to 12.** `MAX_ORDER` was renamed to `MAX_PAGE_ORDER` and its meaning flipped from exclusive to inclusive, so the same number now means one order more. Nothing warns about this; the build succeeds and the region is twice the intended size. Without it the config line above is silently dropped by `olddefconfig` — the value never takes effect and nothing says so. Both patches are needed; neither works alone. |
