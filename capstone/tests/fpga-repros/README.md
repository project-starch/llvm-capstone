...
> **2026-09-09 — the `status` column above is now the index's own answer**, so a reader does not
> have to open eighteen READMEs to find out what is still live. Each value is sourced from the
> issue's header in `capstone/docs/ref/ISSUES.md` first and the folder's own dated banner second;
> where the two disagree the registry wins and the disagreement is worth reading (`S10-…` is the
> live example: its banner records the route-1 residual as not observed, while the registry records
> S-10b as still present at the resident bitstream). **No folder moved.** `S08-…` carries a report
> written for an external author and their reply, and `R02-…` is a question folder the sweep still
> exercises, so both stay where their links point; see "Three states, not two" below.
...
| dir | issue | status (2026-09-09) | what it shows |
|---|---|---|---|
| `R01-lsu-hazard/` | **R-1** | **resolved-retained** — R-1 GONE at `5097eb166` (2026-09-05); attribution still open | a load through one capability register misses a store through another |
| `R02-delin/` | **R-2** | **resolved-retained** — explained by C-13 (2026-07-29), re-confirmed 2026-09-05 | `delin` in domain code wedges the board — `delin.s` vs `nop.s`, one instruction apart |
| `R16-entry-stall/` | **R-16** | **resolved-retained** — fixed in silicon 2026-08-04; the bitstream acceptance test | the domain never returns from its FIRST entry (`SHA5` stall, no `SQ: G/enter`). **Resolved** — kept as a one-boot bitstream acceptance test |
| `S01-image-perturbation-hang/` | *(open, unexplained)* | **open** — registry R-17, not root-caused; the pair was not run in the 2026-09-05 sweep | a ~1.6 MB domain returns, but **any** perturbation of its image makes it hang — silently, and correctly under QEMU |
| `RTL-store-user-metadata/` | *(observation)* | **observation** — the invariant it tests is correct in silicon | every store routes capability metadata into the dcache write-user sideband; the invariant it tests is **correct** in silicon. Its open question — no traced path from `data_wuser` into software-visible data — is answered by **`R19-...`**, not by `R18-...`, and only on the board: R-19 exhibits a slot returning `0x08000000 + n`. R-18 explicitly **retracts** the claim that it closes this |
| `R18-scalar-store-metadata-clobber/` | **R-18** | **open** — reported; not reproduced at `5097eb166` with its own frozen images | the **zeroing** signature — a plain scalar store in the **upper half** of a 16-byte row silently loses increments (e.g. 567 where 576 was expected), with raw readbacks showing **no** metadata anywhere. 18 frozen images. A dual-bank splash is demonstrated in RTL simulation, though not the same slot the board damages |
| `R19-movc-zero-metadata-in-slot/` | **R-19** | **open** — trigger established on silicon, mechanism unconfirmed | the **metadata-in-slot** signature — the store's own slot comes back holding `compress_cap(NULL) + n`, e.g. `0x08000A31` = `0x08000000` + 2609, where the program only ever wrote an integer. Does **not** reproduce in RTL simulation. Shares a trigger class and a workaround with R-18; whether they are one defect or two is **unknown** |
| `R20-stc-rs1-cursor-forward-x10/` | **R-20** | **resolved-retained** — fixed in silicon 2026-08-10; fix present at `5097eb166` by content | after `stc rX,0(a0)`, a `ld a0,0(a0)` is read by the **next instruction** as the store's BASE ADDRESS, not the loaded value. Only on **x10/a0** (the same shape on `t1` is clean), only with a **capability** store (`sd` is clean), and only while both adjacencies hold — one `nop` either side cures it. Corrupts no memory. Reproduced standalone in a **13 KB** rung; the poisoned value is measured, not inferred |
| `S06-untagged-ldc-stc-high-half/` | **S-06** | **resolved-retained — MEMCPY HALF ONLY** (fixed 2026-08-14; `s06copy`/`s06aggcap`/`s06aggwide` at their oracles at `5097eb166` and again at `66c4e7517`). The struct-assignment half was split out to `R29-…/` on 2026-09-10: it was never fixed, and this folder's acceptance for it cited a different program. | a 16-byte `ldc`/`stc` round trip over **plain, untagged** data keeps only its **low 8 bytes**. Root cause in the D-cache tag path (`wt_dcache_mem.sv:310`, `:140`). |
| `R29-wbuffer-highword-forwarding/` | **R-29** | **open** — reproduced on silicon (boots sw46, sw48 on `66c4e7517`, two firmwares) and in RTL sim on `5097eb166`, `ef5a8eaf2` and `66c4e7517`; mechanism NOT settled, two of three accounts disfavoured by their own arms, no fix candidate | a plain 8-byte `sd` into the **HIGH** word of a 16-byte granule, immediately before a 128-bit `ldc` of that granule, returns the high half **stale**. Adjacency is the trigger: four instructions apart it passes. Sibling of `R-10`'s secondary half. |
| `S07-capability-untagged-on-reload/` | *(open, not root-caused)* | **resolved-retained, caveated** — the fix is exonerated as the cause; residual not observed at `5097eb166` | a **genuine capability** read back from memory comes back **UNTAGGED**, so the next `cincoffset`/dereference raises **mcause 25**. Three instances in three unrelated functions — a `memcpy` stack slot, the shared-region payload capability in `output_text`, and SQLite's lookaside pointer — so it is not specific to any caller or primitive. **SPORADIC**: the same binary passes 5 of 6 genuine executions and wedges once. **NOT S-06** (that is plain data losing its high half, and raises nothing); do not merge them. Four rungs with firing controls exclude the simple round-trip properties, and disassembly excludes a partial overwrite and the write-buffer `.user` clobber |
| `S08-s06fullfix-bitstream-cannot-run-domains/` | **S-08** | **resolved-superseded** — its bitstream is no longer flashed | on `caplifive_s06fullfix.bit` the monitor took an UNHANDLED TRAP just after a domain's first share returned, so that bitstream could run no domains at all. **RESOLVED — fixed in RTL and verified on silicon 2026-08-15** on `caplifive_s06fixs08fix.bit`: `EXCX:0000E002` count 0 against 4 of 4 boots on the broken build, and the S-06 acceptance gate became readable in the same boot and passed |
| `S09-write-buffer-tag-forgery/` | **S-09** | **open** — folded into S-10 in the registry; not exercised as a separate arm | a capability **survives the plain store meant to destroy it** — a scalar store over a capability leaves the tag set, which is a forgery primitive. Carries a **narrowed timing caveat** (2026-08-20): the fix is exonerated as the cause of the timing failure, but the measurement was taken on a bitstream missing setup (WNS −10.629 ns on one clock) |
| `S10-write-buffer-forward-residual/` | **S-10** | **open** — S-10b measured STILL PRESENT at `5097eb166`, and its fix is unsynthesizable | a **scrubbed capability still reads back LIVE** while the store that scrubbed it is in the write buffer — the forwarding path returns the pre-scrub value. Same timing caveat as S-09, and the two share a bitstream |
| `S11-seal-minsize-alignment-inert/` | **S-11** | **open** — instruction-semantics defect; not exercised in the 2026-09-05 sweep | `SEAL` enforces **neither its minimum size nor its base alignment** — an instruction-semantics defect, not a cache or tag-path one. Its README opens by redirecting readers who arrived chasing a tag-loss symptom, because it shares nothing with S-06/S-07/S-09/S-10 |
| `S12-wherecode-notcap-operand-vs-memory/` | **S-12** | **resolved-retained** — fixed in the flashed RTL; "consistent with fixed, not proven" | a capability operand arrives at its consumer as `cnull` (`mcause 25`, `tval 0`) while **memory is intact** — the fault is in what the execution unit *ingested*, not in what was stored. **ROOT-CAUSED 2026-09-03**: a scoreboard WAW clear that ignores `commit_ack`, letting a written-back-but-unretired `stc` entry forward its nulled source to a younger `ldc`. Fixed in RTL, synthesised, flashed. Post-flash verification is **consistent with fixed, not proven** (p = 0.071) — read `01-DECISION-TABLE.md` and `02-REFUTED.md`, which record what the evidence does *not* support |
| `S13-o1-dyn-rev-node-hang/` | **S-13** | **open, not reproducible from this folder** — its two -O1 images are unreconstructable | at `-O1` the domain **hangs in the DYN/rev-node path with no exception**. `OPEN`. The single-entry store syncer is **CLOSED as a cause** — three structural arguments, not absent counters, which is why the missing `req_set` guard that any later reader will reach for is not the answer |
| `RTL-cap-mcause-off-by-one/` | *(spec violation)* | **open** — spec violation, not exercised in the 2026-09-05 sweep | every capability `mcause` from the **data path is one code too high**, and 25 aliases. Verified against the reference model; **not** reproduced as a functional failure — its cost is **misclassification**, which matters because several reports above classify faults by `mcause` |
| `RTL-domain-trap-vector-unset/` | *(root-caused, half-fixed)* | **open** — registry M-1; firmware half confirmed on silicon 2026-09-02; the in-domain vector (`INTERP_DOMAIN_MTVEC=1`) verified on silicon 2026-09-14 (sw69) for a first-entry fault; default builds still wedge (both R-35 probes, 2026-09-24) | a domain enters with **no trap vector** — `create_domain` never writes the trap-vector context slot, so a fault in a domain storms at address 0. Firmware half **confirmed on silicon 2026-09-02**; writing `dom_seal[1]` moves the failure to "the handler's own prologue faults in the wrong capability context". **Not firmware-only** — the remaining half is that a trap does not switch domains |
| `R25-init-rs1-ne-rd/` | **R-25** | **fixed in RTL 2026-09-09** (`42a141c93`), sim-verified, bitstream in synthesis at `66c4e7517`; silicon pre/post readings pending | `INIT rd, rs1` with `rd != rs1` left a **second live LINEAR capability** in `rs1` — same region, same revocation node; the duplicate is usable. Self-checking test FAIL 11 → PASS |
| `R26-ccsrrw-stale-cpmp/` | **R-26** | **fixed in RTL 2026-09-09** (`9d8797560`), sim-verified at two memory latencies, bitstream in synthesis at `66c4e7517` | a `CCSRRW` to `cpmp`/`cscratch`/`cepc` committed **without a flush**, so a younger load's permission check ran against the **old** entry (five cycles early behind a cache miss). One flush statement; `ldmiss` FAIL 11 → PASS |
| `R27-revnode-orphan-response/` | **R-27** | **fixed in RTL 2026-09-09** (`66c4e7517`), sim-verified, audited, bitstream in synthesis; **silicon unconfirmed** | a flush inside a capability op's **revocation-node query window** orphans the node's response; the node blocks in its send and the next capability op **waits forever, no trap**. Realistic trigger: a faulting load immediately followed by an LDC hangs the core instead of trapping. ~30-line drain in `ex_stage.sv` |
| `ARCHIVED/` | — | **archived** | packages whose defect is **fixed in silicon**; see `ARCHIVED/README.md` |
| `R34-lsu-exception-lost-on-immediate-grant/` | **R-34** | **open** — demonstrated in RTL simulation 2026-09-15 at `f6ec6c198` (gate/exception lines identical at the flashed `1bfff7776`); not run on the board | every exception the LSU generates itself — capability causes 24–28 and misaligned 4/6 — is **lost when the access is granted in its request cycle** and delivered only if an exception is still presented one cycle later: raised combinationally, popped the same cycle, sampled the next (`load_unit.sv:423-424`, `:718`; `cva6_mmu.sv:514`); the stock `rv64mi-p-ma_addr` fails the same way |
| `R35-revoked-reference-retains-authority/` | **R-35** | **fixed on the M-mode LSU data path — CLOSED 2026-09-25** (`4ad0df694`, flashed 2026-09-24; the exposing probe no longer reproduces on silicon). Residuals filed: **R-43** (deny-on-miss false denies), **R-44** (the same adopt at the CPMP, S/U mode, still open) | a **revoked capability still read and wrote** the storage its object gave up, because the LSU's single core-wide revnode tracker **re-adopted any unseen id as valid**. Fix: a positive validity cache fed by passive taps on the rev-node unit, fail-closed on a miss |
| `R42-icache-killed-miss-refill/` | **R-42** | **fixed in RTL and synthesized** (`6cbdaeeb4`, 2026-09-24; no new loop, WNS −10.615); reflash authorized 2026-09-25, board acceptance pre-registered in the folder | **performance, not correctness**: a loop whose predicted-taken branch ends a 16-byte I-cache line pays **+1 cycle every iteration** — the speculative fall-through miss is killed and the FSM detours through IDLE. Explains the ladder control's layout-dependent 1.167× |
