...
static unsigned s06agg_d2_compute(void)
{
  ...
  /* R-29 distance ladder: 2 filler instruction(s) between the plain store of y and the
   * capability-grained copy of that granule. The memory clobber keeps the compiler from
   * moving the store past them; the DISASSEMBLY is what verifies the distance. */
