...
set -eu
CO="$1"; T="$2"; DEF="${3:-}"; LIST=testlist_r26.yaml
docker run --rm -v "$CO":/workdir --user "$(id -u):$(id -g)" --entrypoint bash \
  -e HOME=/tmp -e RISCV=/toolchain -e CVA6_REPO_DIR=/workdir -e NUM_JOBS=8 -e VERILATOR_THREADS=1 cva6-build-rv -c '
...
python3 cva6.py --testlist=../tests/'"$LIST"' --test '"$T"' --iss_yaml cva6.yaml --target capstone_cv64a6_imafdc_sv39 \
  ...
  --isscomp_opts="'"$DEF"'" 2>&1 | grep -E "\*\*\* (SUCCESS|FAILED)"'
