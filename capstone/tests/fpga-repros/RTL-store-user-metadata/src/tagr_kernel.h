 ...
 *
 * The verdict is the QEMU-vs-silicon DIFFERENCE, not an absolute: whatever QEMU reports is
 * the architectural reference, and a different value on the board is the defect.
 *
...
static const volatile unsigned long tagr_pad[512] = { 1 };
static char tagr_g[16] = { 1 };
static unsigned tagr_compute(void)
{
  void *cap;
  ...
  *(volatile unsigned long *)(void *)&buf[0] = 0x0123456789abcdefUL;
  ...
  /* CONTROL ARM: deliberately does NOT lcc after the overwrite. Returns unconditionally, so
     if the paired arm produces no result that is the lcc trapping, not the domain wedging. */
  t_int = 7;

  (void)tagr_g; (void)tagr_pad[0];
