...
# Per-primitive breakdown iteration count. Keep BREAKDOWN_ITERS*3 (mrev_only +
# mrd + full accumulate one revocation-tree node per iter, none released -- no
# drop) well under the ~1024-revoke reset ceiling. 64 -> 192 nodes: safe.
BREAKDOWN_ITERS=${BREAKDOWN_ITERS:-64}
...
"$GUEST_CC" \
  ...
  -I"$SCRIPT_DIR" \
  -I"$QEMU_PROBE_DIR" \
  -o "$OUT_DIR/borrow_breakdown_fpga_nogp_ctl" \
  "$SCRIPT_DIR/borrow_breakdown_fpga_nogp_ctl.c"
printf 'Built %s\n' "$OUT_DIR/borrow_breakdown_fpga_nogp_ctl"
...
RAW_S="$ASM_DIR/borrow_breakdown_fpga_nogp.raw.s"
PATCHED_S="$ASM_DIR/borrow_breakdown_fpga_nogp.s"

# -fno-unroll-loops: keep every measured loop (empty, raw, mrev_only, mrd, full)
# a REAL loop so they share identical per-iter loop overhead -> the empty-loop
# subtraction is clean for all. Without it, mrev_only (no loop-carried dep) fully
# unrolls while mrd/full (dep via c=revoke(rev)) stay loops, so empty-subtraction
# would over-remove loop overhead from mint only. Codegen-only; guards unaffected.
"$CAPSTONE_CLANG" -target capstone64-unknown-elf -ffreestanding \
  "$DOMAIN_OPT_LEVEL" -fno-unroll-loops -DBREAKDOWN_ITERS="${BREAKDOWN_ITERS}UL" \
  -I"$SCRIPT_DIR" -I"$QEMU_PROBE_DIR" \
  -S "$SCRIPT_DIR/borrow_breakdown_fpga_nogp.c" -o "$RAW_S"
...
"$CAPSTONE_LD_LLD" -T "$LINKER_SCRIPT" -o "$OUT_DIR/borrow_breakdown_fpga_nogp.dom" \
  "$OBJ_DIR/start.o" "$OBJ_DIR/main.o" "$OBJ_DIR/gct-tail.o"

printf 'Built %s (%s, BREAKDOWN_ITERS=%s)\n' "$OUT_DIR/borrow_breakdown_fpga_nogp.dom" "$DOMAIN_OPT_LEVEL" "$BREAKDOWN_ITERS"
