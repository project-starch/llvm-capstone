#!/bin/bash
# Boot sw8x-b80s-<tag> -- cell 5 (memsys5 + lookaside 1200,40, 2 MiB static heap) run WITH --stats on silicon, to READ its lookaside
# state instead of inferring it: the -O2 Sublet cell ran lookaside-OFF on the board (sw8x-o2stats) because the RTL's movc of an
# untagged source nulls the source and setupLookaside's -O2 code re-reads it; cell 5's stock setupLookaside tests a
# capability-derived pointer at -O2 through the same shape. Parametrised: C5_TAG C5_IMG C5_HASH C5_QEMU C5_QEMU_LOG C5_EXPECT.
# The native -O2 baseline (b36eb3814c3cefce, --stats, 25,122 lookasides on silicon in sw80a) runs last as the positive control
# that the stats block reads on the board. Pre-registered, written before the boot:
#   arm 1  lpc + k800.dom                       RESULT k800 retval=4, or the boot is VOID
#   arm 2  cell 5 variant, --size 1 --stats     112006 38bb59fd, HEAP 2097152 DROPPED 0 RC 0; Successful lookasides / Slots Used per C5_EXPECT
...
C5_TAG=${C5_TAG:?}; C5_IMG=${C5_IMG:?}; C5_HASH=${C5_HASH:?}; C5_QEMU=${C5_QEMU:?}; C5_QEMU_LOG=${C5_QEMU_LOG:?}; C5_EXPECT=${C5_EXPECT:?}; export C5_HASH C5_QEMU
OUT=$U/board-b80s-$C5_TAG; mkdir -p $OUT; LOG=$OUT/log; : > $LOG; rm -f $OUT/marker
...
export FPGA_BITSTREAM=caplifive_r30r31_1bfff7776.bit FPGA_BITSTREAM_UNVERIFIED=1
...
D="$D,/test-domains/speedtest1.dom:--speedtest1 --testset main --size 1 --verify --stats"
...
for pat in (r'RESULT k800 retval=[0-9-]+', r'Verification Hash: \d+ [0-9a-f]{8}', r'SPEEDTEST1-CYCLES \d+', r'BASELINE-WARM CYCLES \d+ INSTRS \d+', r'Successful lookasides:\s+\d+', r'Lookaside Slots Used:\s+\d+ \(max \d+\)', r'Lookaside size faults:\s+\d+', r'Lookaside OOM faults:\s+\d+', r'Schema Heap Usage:\s+\d+', r'HEAP \d+ DROPPED \d+ RC \d+', r'sublet: split=\d+ mrev=\d+ delin=\d+', r'ALEN:[0-9A-F]{8}', r'SQ: released pool rc=\d+', r'RC(LM|PR|SH|RE|EN|CU):[0-9A-F]{8}', r'RR/[a-zA-Z-]+', r'SQ: share-trap=\d+', r'HARD STOP|ENTRY-STALL|ABORT'):
