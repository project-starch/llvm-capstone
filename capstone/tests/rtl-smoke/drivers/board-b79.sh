#!/bin/bash
# Boot sw79 -- E2 of the 2026-09-14 Sublet-paper plan: cell 6 (Sublet, ceeded2533a74bce) at P1's power-of-two
...
OUT=$U/board-b79; mkdir -p $OUT; LOG=$OUT/log; : > $LOG; rm -f $OUT/marker
...
IMG=$HOME/capstone-artifacts/matrix/cell6_sublet/sqlite_silicon.dom
[ "$(sha256sum $IMG|cut -c1-16)" = ceeded2533a74bce ] || fail "cell 6 image is not ceeded2533a74bce"
...
grep -q 'SPEEDTEST1-CYCLES 692392094 HIGHWATER n/a HEAP 1344064' ${QEMU_LOG_CELL_2MIB:?} || fail "the QEMU pre-run at 2 MiB is not on record"
...
say "pieces: cell 6 ceeded2533a74bce (pass record present), rr host 2c9e82d101b48160, lpc 3b93a2b6e2adfa36; QEMU pre-run at 2 MiB on record"
...
L=sys.argv[1]
want={'speedtest1.dom':'ceeded2533a74bce','lpc':'3b93a2b6e2adfa36','sqlite_host_rr.user':'2c9e82d101b48160'}
bad=0
...
say "=== boot sw79 = E2: cell 6 at the 2 MiB arena (P1's matched backing limit), control, cell, control, probe ==="
say "=== pre-registered: retval=4; 112006 38bb59fd + HEAP 1344064 + sublet 5568/37966/32565; cycles 2,803,660,693 +-1 % (2,775.6 M..2,831.7 M); teardown released pool rc=1, RCLM 0->1, no RCPR/RCSH/RCRE; probe ALEN:00200000, RR/done ==="
...
if m: c=int(m[0]); print(f"  cycles {c:,} vs sw74's 2,794,183,730: {c/2794183730:.4f}; vs predicted 2,803,660,693: {c/2803660693:.4f}; in band: {2775600000<=c<=2831700000}")
