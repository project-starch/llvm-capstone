/* Freestanding gp-free-domain controller -- FPGA/CapliFive silicon variant.
 *
 * Pairs with the gp-free proof domain (start-gpfree-cscratch.S + gpfree_app.c):
 * a real integer app with data globals AND a non-inlined call graph, compiled
 * -capstone-gp-free (plain jal/jalr calls within PCC, `scc gp` global addressing,
 * NO cjalr, NO cincoffset-gp). gp is delivered by the MONITOR via the cscratch
 * stack region (create_domain mints an image-covering cap and stores it at the
 * region top slot) -- this run is the SILICON confirmation of that cscratch gp
 * delivery on the real CVA6/Capstone RTL (no QEMU fabrication).
 *
 * Protocol (single REGION_SHARE, no CALL -- same shape as borrow_cost_fpga_nogp):
 *   1. create the domain from the .dom image;
 *   2. create ONE region (REV_SHARED, host retains its mapping);
 *   3. share it once (PERM_INOUT, REV_SHARED) -- this IS the domain entry: the
 *      region cap arrives as domain_main(res, func)'s `res`; the app runs its
 *      globals + call-graph workload and writes *res = 0x2110C000 | (sum & 0xff);
 *   4. read region[0] straight back out of the retained mapping and print it.
 *
 * Expected: retval = 554745961 (0x2110C069), sum = 77 + 28 = 105 = 0x69.
 *
 * Like the sibling controllers this links NO glibc (the board rejects glibc's
 * hard-float fsd): built -nostdlib -static -march=rv64imac -mabi=lp64 (soft-float
 * ABI, zero FP), own _start, raw Linux syscalls via ecall, integer-only output.
 */
...
static void puthex_(ulong v) {       /* 0x hex */
  char buf[16];
  int i = 16;
  if (v == 0) buf[--i] = '0';
...
struct ioctl_dom_create_args {
  ...
  /* Appended by caplifive-buildroot #2/#3 (module d04bd83): the domain's declared requirement,
     ...
     create fails. create_dom() memsets the struct, so these domains declare nothing. */
...
#define EXPECT_RETVAL 554745961u  /* 0x2110C069 */
...
static int run(const char *dom_path) {
  ...
  /* ONE REV_SHARED region: the domain writes its single retval word to the base.
   * Host retains its mapping and reads region[0] straight back after the share. */
  region_id_t region_id = create_region(REGION_SIZE);
  unsigned char *region = map_region(region_id, REGION_SIZE);
  if (!region) { puts_(TAG ": map region failed\n"); return 1; }
  memset_(region, 0, REGION_SIZE);
  puts_(TAG ": region = "); putu_(region_id); puts_(" (REV_SHARED)\n");
  ...
  puts_(TAG ": RESULT retval = "); putu_(retval);
  puts_(" ("); puthex_(retval); puts_(")\n");

  if (retval == EXPECT_RETVAL) {
    puts_(TAG ": PASS -- cscratch gp delivery confirmed on silicon\n");
    return 0;
  }
  puts_(TAG ": FAIL -- expected 554745961 (0x2110c069)\n");
  return 1;
}
...
void _start_c(long *sp) {
  ...
  char **argv = (char **)(sp + 1);
  if (argc < 2) {
    puts_("usage: gpfree_fpga_ctl <gpfree_app.dom>\n");
    sys_exit(2);
  }
  int rc = run(argv[1]);
  sys_exit(rc);
...
__asm__(
  ...
  ".globl _start\n"
  "_start:\n"
  /* Initialise gp (global pointer) for the CONTROLLER itself (ordinary RV64
   * Linux binary; the compiler addresses small globals gp-relative). norelax so
   * `lla gp,__global_pointer$` isn't itself relaxed to gp-relative. */
  "  .option push\n"
  "  .option norelax\n"
