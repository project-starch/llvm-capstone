...
**Upstream fix:** `gh-139210`, *"use-after-free in xml.etree.ElementTree.iterparse()"*, back-patched into the 3.13 branch
**after** our `v3.13.7` tag. The backport is the proof the defect was in 3.13.

**Consumer:** `Modules/_elementtree.c`. **CVE:** `NO VERIFIED CVE`.

## The defect

**What is freed:** event_name, a C string pointing into an item of events_seq.
**What ends its life:** Py_DECREF(events_seq) ran BEFORE the error was formatted.
...
**What distinguishes it:** the entire fix is a REORDERING of two existing lines, and the stale pointer is a char* into a payload rather than any PyObject* a checker could have followed.
...
`shared/defects.c`, case **9**. Shape: payload buffer, error path.
