 ...
 * arm.
 *
 * A SECOND TARGET, SAME CASES
 *
 * With PYMALLOC_POISONCAP the same twenty sequences build as an ordinary
 * CheriBSD purecap program against the port's PoisonCap adapter, and keep the
 * same two modes: mode 0 is request-bounded spatial authority with no
 * per-object invalidation and must COMPLETE, mode 1 adds PoisonCap lifetime
 * invalidation and must FAULT. Only three things differ, and none of them is a
 * case: the probes become capability-base accesses (clbu/csb) so the label sits
 * on an instruction the trap PC can be compared against, the markers become
 * printed lines instead of Capstone marker instructions, and a SIGPROT handler
 * turns the fault into one machine-readable line before re-raising. Every
 * defect() branch and every CHECK is shared, unchanged, between the two.
 *
...
#ifdef PYMALLOC_POISONCAP
/* Nothing references the symbol on this target -- mark() no longer publishes
 * probe addresses -- so keep the definition alive explicitly. */
__attribute__((used))
#endif
...
void pym_replay(const struct pym_header *input, struct pym_header *out,
  ...
  unsigned which = e->id;
#ifdef PYMALLOC_POISONCAP
  /* Before any case executes, and before the input check, so that a fault
   * anywhere in this program is reported rather than merely fatal. */
  install_fault_handler(which);
#endif
