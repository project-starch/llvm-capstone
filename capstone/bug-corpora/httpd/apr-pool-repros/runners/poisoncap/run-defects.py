#!/usr/bin/env python3
"""Run the httpd/APR pool defects on CheriBSD under PoisonCap, as
paired arms: mode 0 publishes exactly bounded aliases and invalidates nothing,
mode 1 poisons and sweeps a node at its release. Both arms run the SAME binary under `supervise`, which
...
def main():
    ...
    p.add_argument("--port", type=int, default=10444)
    ...
    (a.output / "selection.json").write_text(json.dumps(dict(
        schema="httpd-apr-defect-poisoncap-selection-v1", cases=selected, modes=modes,
    ...
    (a.output / "verdicts.json").write_text(json.dumps(dict(
        schema="httpd-apr-defect-poisoncap-v1", negative_control=a.negative_control,
