#!/usr/bin/env python3
"""Run the httpd/APR pool defects on stock CheriBSD, against the platform's own
malloc, with libc revocation on or off.
...
sees a consumer allocating through a pool it destroyed. APR files a destroyed
pool's node on its own free list and pops it straight back for the next pool;
under the default APR_ALLOCATOR_MAX_FREE_UNLIMITED it never calls free() on
that path. So the expected answer, in both configurations, is that the
sequence COMPLETES.
...
def main():
    ...
    (a.output / "selection.json").write_text(
        json.dumps(
            dict(
                schema="httpd-apr-defect-cheribsd-selection-v1",
