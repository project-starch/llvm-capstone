#!/usr/bin/env python3
"""Run the httpd/APR pool defects as paired spatial/sublet domain arms.
...
runtime from the mode argument, so the two arms differ in exactly one thing.
  ...
  sublet  (mode 1)  release and issue each revoke the node and mint a fresh
                    alias, so that handle is dead and the first read through
...
CASES = [
    (
        "9e6be73065",
        "watchdog-destroyed-pool-reused",
        "stale allocator handle / reuse / allocation through the dead handle",
    ),
]

MAGIC = 0x314C4F4F50525041  # "APRPOOL1"
MARKER_BASE = 0xCF1A000000000000
...
p.add_argument(
    "--domain-build",
    type=Path,
    default=tmp / "apr-pool-repros/domain",
