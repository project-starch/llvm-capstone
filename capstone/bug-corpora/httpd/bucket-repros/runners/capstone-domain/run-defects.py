#!/usr/bin/env python3
"""Run the httpd bucket-allocator defects as paired spatial/sublet domain arms.
...
runtime from the mode argument, so the two arms differ in exactly one thing.
  ...
  spatial (mode 0)  a node keeps the alias it was carved with across APR's
                    free list, so a handle held over apr_pool_destroy still
                    names live storage -- the next pool's -- and the run must
                    COMPLETE.
  sublet  (mode 1)  release and issue each revoke the node and mint a fresh
...
CASES = [
    ("1c7a70c9d9", "wrong-bucket-alloc-across-connections", "foreign or dead allocator"),
    ("d2a1cf5f8c", "buckets-outlive-backend-allocator", "foreign or dead allocator"),
    ("106d0761c0", "brigade-on-request-pool-past-eor", "a pool outlived by its holder or its contents"),
    ("d9c2352952", "buckets-from-pool-that-dies-first", "a pool outlived by its holder or its contents"),
    ("c81adad105", "brigade-not-cleaned-before-reuse", "a recycled allocator serving its next user"),
    ("60919177e8", "read-from-destroyed-brigade", "the holder used after it was destroyed"),
...
REPORT_FIELDS = 12  # struct aprp_header: 12 x uint64
...
def classify(serial, report, which, mode, runner_exit):
    ...
    row = {
        ...
        "shape": shape,
        "mode": mode,
        "expected": "fault" if mode == "sublet" and SUBLET_SITE[which] is not None else "complete",
        "runner_exit": runner_exit,
    }
    if mode == "sublet" and SUBLET_SITE[which] is not None:
...
p.add_argument(
    "--domain-build",
    type=Path,
    default=tmp / "apr-bucket-repros/domain",
    help="where shared/build-cases.sh capstone-domain put bin/defect-NN.dom",
)
p.add_argument(
    "--linux-build",
    type=Path,
    default=tmp / "apr-pools/build/linux-guest",
...
status=0
/tmp/apr-host /tmp/apr-defects.dom /tmp/apr-trace.bin /tmp/apr-report.bin {number} || status=$?
...
""")
        ...
        result = run_guest(
            run, "sh /mnt/host/run.sh", "APR_DEFECT_DONE", env=env, timeout_multiplier=1
        )
        serial = (
        ...
        if not serial:
            ...
            sys.exit(75)
        if "APR_DEFECT_DONE" not in serial and not re.search(
