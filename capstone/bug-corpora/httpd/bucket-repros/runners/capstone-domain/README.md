...
The port is `ports/apr/pools` built with `-DAPRP_BUCKETS=ON` (build-cases.sh passes it); its `linux-guest` preset builds the loader.
...
| `sublet` | release and issue each `sublet_give` then `sublet_take` the node; a bucket allocator's pieces are split from the lent block and die with it | the stale access FAULTS at the labelled probe -- except case 4, whose reduced sequence ends no lifetime and which must COMPLETE, a recorded non-detection |
...
`patches/apr-1.7.4-0002-node-lifetime-hooks.patch` and `patches/apr-util/apr-util-1.6.3-0002-bucket-lifetime-hooks.patch`.
