...
MPY=capstone/ports/micropython
CORPUS=capstone/bug-corpora/micropython
...
repro_glue() {
  ...
  # -I for the glue: the reproductions live here, not in the port, so the corpus supplies
  # both the flag and the path to what it switches on.
  MPY_FLOAT_CORE=1 DOMAIN_EXTRA_DEFS="-D$flag -I$REPO/$CORPUS/domain-glue $ROM" DOM_NAME="$name" \
    bash "$BUILD" >"/tmp/capstone/$name-build.log" 2>&1 \
    || { tail -15 "/tmp/capstone/$name-build.log"; die "build failed"; }
