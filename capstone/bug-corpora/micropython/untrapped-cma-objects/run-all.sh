...
M=capstone/bug-corpora/micropython
MODE=${1:-all}

declare -a NAME PATHS
add() { NAME+=("$1"); PATHS+=("$2"); }
[[ $MODE == all || $MODE == glue ]] && {
  add MPY-T07 "$M/temporal/cases/MPY-T07_lexer-source-name-uaf/run.sh"
  add MPY-T16 "$M/temporal/cases/MPY-T16_deinit-after-gc-sweep-all/run.sh"
  add MPY-T29 "$M/temporal/cases/MPY-T29_gc-free-assert-invalid-block/run.sh"; }
[[ $MODE == all || $MODE == scripts ]] && {
  add MPY-T09 "$M/temporal/cases/MPY-T09_bytearray-resize-stale-view/run.sh"
  add MPY-T10 "$M/temporal/cases/MPY-T10_array-resize-stale-view/run.sh"
  add MPY-T13 "$M/temporal/cases/MPY-T13_write-callback-grows-buffer/run.sh"; }
[[ $MODE == all || $MODE == spatial ]] && {
  add MPY-S05 "$M/spatial/cases/MPY-S05_array-append-after-failed-grow/run.sh"; }
[[ $MODE == all || $MODE == backport ]] && {
  add MPY-T02 "$M/temporal/cases/MPY-T02_objarray-bytes-self-copy-uaf/run.sh"
  add MPY-T05 "$M/temporal/cases/MPY-T05_objarray-line509-uaf/run.sh"; }
