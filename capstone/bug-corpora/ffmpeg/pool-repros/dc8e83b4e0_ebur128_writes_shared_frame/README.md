# ebur128: drawing again into a frame it has already shared

`f_ebur128` keeps `pic` across calls and hands a reference on. On the next frame
it draws into the same storage, which a consumer still holds and reads.

    arm=fixed holder=ebur128 shared_when_written=0 consumer_saw=0xA1 consumer_now=0xA1 freed_to_malloc=0
    arm=buggy holder=ebur128 shared_when_written=1 consumer_saw=0xA1 consumer_now=0xB2 freed_to_malloc=0
