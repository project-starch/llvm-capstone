# aphasemeter: drawing again into a frame it has already shared

`avf_aphasemeter` keeps `s->out` across calls and hands a reference on. On the next frame
it draws into the same storage, which a consumer still holds and reads.

    arm=fixed holder=aphasemeter shared_when_written=0 consumer_saw=0xA1 consumer_now=0xA1 freed_to_malloc=0
    arm=buggy holder=aphasemeter shared_when_written=1 consumer_saw=0xA1 consumer_now=0xB2 freed_to_malloc=0
