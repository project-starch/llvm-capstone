...
checker is the authority; this is that checker for the Wireshark corpus.
...
def check(root):
    ...
    for d in dirs:
        ...
        if not source.is_file():
            ...
            continue
        declared = re.search(r"WM_CASE\((\d+)\)", source.read_text())
        if not declared:
            problems.append(f"{d.name}: case.c declares no WM_CASE")
        elif int(declared.group(1)) != case["case"]:
            problems.append(
                f"{d.name}: case.c says WM_CASE({declared.group(1)}), directory says {case['case']}")
