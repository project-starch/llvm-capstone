#!/usr/bin/env python3
"""Run the memcached allocator defects on PoisonCap/CheriBSD, in pairs.
...
thing:
  ...
  mode 0  request-bounded spatial authority, no invalidation at all. The
          freed object stays addressable, the stale access succeeds, and the
          case must reach its end and report a completed run -- with the
          adapter's own line reporting ZERO sweeps, which is what makes this a
          control rather than a second protected arm.
  mode 1  PoisonCap lifetime invalidation: the released unit's granules are
          poisoned, a synchronous sweep runs, and the consumer's retained
          pointer is a revoked alias. The stale access must raise SIGPROT with
          si_code PROT_CHERI_TAG at the labelled `mc_defect_read` instruction
          -- not somewhere, and not some other kind of capability fault.

Before any case arm, the platform's own instruction controls run in the same
boot: live access through a fresh lease, a poisoned read and write, a sweep
that reuses storage, and a retained old alias that must be refused. They are
what a completing spatial arm is read against; if one of them does not behave,
the run exits 75 and records no case verdict.
...
REPORT_BYTES = 96  # struct mcp_header: 12 x uint64
...
# The published platform's own instruction controls, run under the same
# supervisor as the cases and in the same boot, before any of them. Two must
# complete and three must die of a tag fault; between them they show that a
# lease works, that poison marks storage, that a sweep both reclaims it and
# kills the old alias, and that reuse is possible afterwards. A completing
# spatial arm means nothing without these: "nothing happened" looks the same
# as "there is no mechanism".
# si_code, because the two ways this platform refuses an access are not the
# same event and a control that accepted either would not separate them:
#   3  the capability is still tagged and the GRANULE is poisoned -- what a
#      consumer meets between a release and the sweep. The stock header calls 3
#      PROT_CHERI_SEALED; this platform's kernel reports the poison fault with
#      it, which is why the number is written down here rather than a name.
#   2  PROT_CHERI_TAG: the sweep has run and the ALIAS is revoked. This is what
#      a protected case arm must produce, and nothing else.
POISONED_GRANULE = 3
POISONCAP_CONTROLS = (
    ("poisoncap-live", "live", None, "POISONCAP live PASS"),
    ("poisoncap-reuse", "reuse", None, "POISONCAP reuse PASS"),
    ("poisoncap-read", "read", POISONED_GRANULE, "POISONCAP read READY"),
    ("poisoncap-write", "write", POISONED_GRANULE, "POISONCAP write READY"),
    ("poisoncap-reused-read", "reused-read", PROT_CHERI_TAG,
     "POISONCAP reused-read READY"),
)
# The adapter's own line, for the spatial arm only: it must report the mode it
# ran and that it swept nothing at all.
POISON_LINE = re.compile(r"MCP_POISONCAP mode=(\d+) sweeps=(\d+) .*")
...
def build_platform_control(name, argument, code, ready, supervisor, probe,
                           timeout):
    """One poisoncap-probe invocation, observed from outside like a case.

    The two that must complete are checked on their own marker and exit 0. The
    three that must fault are checked on the KERNEL's report -- signal 34 and
    the si_code THIS control is supposed to produce -- and on the probe's ready
    marker, so a program that died before reaching the stale access cannot pass
    as one that was stopped at it. The PC is not compared here: this binary
    carries no corpus label, and what it is being asked is whether the
    mechanism fires, and which of its two refusals fired."""
    case = dict(
        name=name,
        program=str(supervisor),
        args=["./probe", argument],
        inputs={"probe": str(probe)},
        timeout=timeout,
    )
    if code is None:
        case["expect"] = "SUPERVISE exit status=0"
        case["also_expect"] = [ready]
        case["exit"] = 0
    else:
        case["expect_regex"] = (
            rf"SUPERVISE fault signal={SIGPROT} code={code} "
            r"addr=0x[0-9a-f]+ pc=0x[0-9a-f]+"
        )
        case["also_expect"] = [ready]
...
def report_fields(blob):
    ...
    if blob is None or len(blob) != REPORT_BYTES:
        return None
    (magic, count, mode, status, completed, pages, chunk_reuses, chunk_releases,
     object_reuses, object_releases) = struct.unpack("<10Q", blob[:80])
    return dict(
        ...
        completed=completed,
        pages=pages,
        chunk_reuses=chunk_reuses,
        chunk_releases=chunk_releases,
        object_reuses=object_reuses,
        object_releases=object_releases,
...
def evaluate(number, mode, row, stdout, report):
    ...
    verdict = dict(
        case=number,
        mode=mode,
        expected="completed" if mode == 0 else "tag fault at mc_defect_read",
        ran=row is not None,
    ...
    if mode == 0:
        ...
        # The adapter says what it did. A spatial arm that swept anything is
        # not a control, so its own line is part of the oracle rather than
        # decoration: the mode it ran and a sweep count of zero.
        adapter = None
        for line in lines:
            found = POISON_LINE.fullmatch(line)
            if found:
                adapter = dict(mode=int(found[1]), sweeps=int(found[2]))
        verdict["adapter"] = adapter
        verdict["passed"] = bool(
...
def main():
    ...
    p.add_argument("--port", type=int, default=10451)
    p.add_argument(
        "--abi-probe",
        type=Path,
    ...
    p.add_argument(
        "--negative-control",
        action="store_true",
        help="Corrupt every fixture so the program refuses it before any case runs, "
    ...
    poison_probe = bins / "poisoncap-probe"
    for path in [*programs.values(), probe, supervisor, poison_probe]:
        if not path.is_file():
            p.error(f"missing program: {path} (build with shared/build-cases.sh poisoncap)")
    ...
    # The platform's controls first: a case verdict is worth reading only after
    # the mechanism has been shown to fire and to release.
    controls = [
        build_platform_control(name, argument, code, ready, supervisor,
                               poison_probe, a.timeout)
        for name, argument, code, ready in POISONCAP_CONTROLS
    ]
    cases = controls + [case for _, _, case in arms_wanted]
    (a.output / "cases.json").write_text(json.dumps(cases, indent=2) + "\n")
    (a.output / "selection.json").write_text(
        json.dumps(
            dict(
                schema="memcached-defect-poisoncap-selection-v1",
    ...
    for name in PLATFORM_CONTROLS:
        if not rows.get(name, {}).get("passed"):
            ...
            return EXIT_INFRASTRUCTURE
    # The poison controls are required in every run, including the negative
    # control: they take no fixture, so nothing the negative control does to a
    # fixture can excuse one of them failing.
    poison_rows = {}
    for name, _, code, _ in POISONCAP_CONTROLS:
        row = rows.get(name, {})
        poison_rows[name] = row
        expected = "completes" if code is None else f"SIGPROT si_code={code}"
        print(
            f"{'OK  ' if row.get('passed') else 'FAIL'} {name:<22} "
            f"expected={expected:<22} exit={row.get('exit')}",
            flush=True,
        )
    for name, row in poison_rows.items():
    ...
    (a.output / "verdicts.json").write_text(
        json.dumps(
            dict(
                schema="memcached-defect-poisoncap-v1",
                negative_control=a.negative_control,
                complete_suite=sorted(selected) == sorted(claims)
                and sorted(modes) == [0, 1],
                ...
                poisoncap_controls=poison_rows,
                arms=arms,
            ),
            indent=2,
        )
