#!/usr/bin/env python3
"""Run the memcached allocator defects on stock CheriBSD, against the platform's
own malloc, with libc revocation on or off.

This is the arm that asks the shipping temporal mechanism the question. The
port's CheriBSD build takes every slab page and every cache object from the
platform's malloc and returns them through the platform's free, exactly as
upstream memcached does; there is no adapter authority and no protected mode.
What varies is libc revocation -- CheriBSD's own, kernel-defaulted mechanism
-- and the question is whether it sees a consumer reading an object it gave
back to cache.c. cache.c pushes the object on its own STAILQ and pops it back
uncleared for the next request; it calls free() only over a limit the caches
here do not have. So the expected answer, in both configurations, is that the
sequence COMPLETES.
...
PROT_CHERI_TAG, pc == mc_defect_read -- and with revocation off it must
...
def evaluate_revocation_control(row, stdout, revocation_on):
    ...
    verdict = dict(
        ...
        expected="tag fault at mc_defect_read" if revocation_on else "completed",
        ran=row is not None,
        exit=row["exit"] if row else None,
        runner_oracle=bool(row and row["passed"]),
        expect=expect,
...
def main():
    ...
    p.add_argument("--port", type=int, default=10441)
    ...
    p.add_argument(
        "--negative-control",
        action="store_true",
        help="Corrupt every fixture so the program refuses it before any cache is "
    ...
    (a.output / "selection.json").write_text(
        json.dumps(
            dict(
                schema="memcached-allocator-defect-cheribsd-selection-v1",
    ...
    for number, case in arms_wanted:
        ...
        if fields:
            detail = (
                f" completed={fields.get('completed')} object_reuses={fields.get('object_reuses')} "
                f"chunk_reuses={fields.get('chunk_reuses')}"
            )
    ...
    (a.output / "verdicts.json").write_text(
        json.dumps(
            dict(
                schema="memcached-allocator-defect-cheribsd-v1",
                negative_control=a.negative_control,
                complete_suite=sorted(selected) == sorted(claims),
