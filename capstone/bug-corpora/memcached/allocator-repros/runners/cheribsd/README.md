...
port's [stock build](../../../../ports/memcached/allocators/host/cheribsd/README.md):
every slab page and cache object from the platform's own `malloc`, back
through its own `free`, exactly as upstream memcached does. No adapter
authority, no protected mode. The variable is libc revocation — CheriBSD's
own, kernel-defaulted temporal mechanism — and the question is whether it sees
a consumer reading an object it gave back to cache.c.
    ...
    BUILD=/tmp/capstone/memcached-allocator-repros/cheribsd
    bash capstone/bug-corpora/memcached/allocator-repros/shared/build-cases.sh cheribsd "$BUILD"

    python3 capstone/bug-corpora/memcached/allocator-repros/runners/cheribsd/run-defects.py \
      "$BUILD" /tmp/capstone/memcached-defects-cheribsd-on \
...
which must exit 0:
    ...
    python3 .../runners/cheribsd/run-defects.py "$BUILD" /tmp/capstone/memcached-defects-cheribsd-control \
...
|---|---|---|
| `on` (the shipping default) | FAULT at `mc_defect_read`: `SIGPROT`, `si_code == PROT_CHERI_TAG`, pc equal to the address the supervisor resolved | COMPLETE: exit 0, a 96-byte report reading `status=0 completed=1 count=1 mode=0`, no fault observed |
...
active and can fire at this shape. It does not fire for the cases because
cache.c pushes the freed object on its own `STAILQ` (`cache.c:135`) and pops
it straight back for the next request (`cache.c:80`); it calls `free()` only
over a limit the caches here do not have, so libc's quarantine never sees the
object. The same holds one level down for slabs, whose chunks never reach
`free()` at all.
...
`CHECK(700)` refuses it before any cache is created: no completed report. Every
