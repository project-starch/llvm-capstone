...
OBJ_DIR=${OBJ_DIR:-$OUT_DIR/obj-qurt}
...
OUT_DOM=${OUT_DOM:-$OUT_DIR/beebs_qurt_capstone.dom}

QURT_SRC=$BEEBS_SRC_DIR/src/qurt/libqurt.c
SUPPORT_DIR=$BEEBS_SRC_DIR/support
COMPILER_RT=$REPO_ROOT/compiler-rt/lib/builtins
PATCHED_QURT_SRC=$OUT_DIR/libqurt_capstone.c
QURT_TAIL_SRC=$SCRIPT_DIR/adapted/beebs_qurt_capstone_tail.c

if [[ ! -f "$QURT_SRC" || ! -f "$SUPPORT_DIR/support.h" || ! -f "$QURT_TAIL_SRC" ]]; then
  echo "missing required qurt sources" >&2
  exit 1
fi
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_QURT_SRC" -o "$OBJ_DIR/libqurt.o"
objs+=("$OBJ_DIR/libqurt.o")

source "$SCRIPT_DIR/build-beebs-softfloat-common.sh"
objs+=("${softfloat_objs[@]}")

"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$SCRIPT_DIR/beebs_simple_domain.c" -o "$OBJ_DIR/beebs_qurt_domain.o"
objs+=("$OBJ_DIR/beebs_qurt_domain.o")
