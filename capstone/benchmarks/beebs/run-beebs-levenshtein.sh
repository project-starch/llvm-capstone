...
python3 "$CAPSTONE_REPO_ROOT/capstone/tests/runtime-qemu/run-domain-smoke.py" \
  ...
  --guest-command \
    'cp /mnt/host/beebs_levenshtein_host.user /tmp/beebs_levenshtein_host.user && chmod 0755 /tmp/beebs_levenshtein_host.user && /tmp/beebs_levenshtein_host.user /mnt/host/beebs_levenshtein_capstone.dom' \
