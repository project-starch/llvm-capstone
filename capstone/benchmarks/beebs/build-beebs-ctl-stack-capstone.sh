...
CTL_SRC_DIR=$BEEBS_SRC_DIR/src/ctl-stack
PATCHED_INC=$OUT_DIR/include-ctl-stack
PATCHED_SRC=$OUT_DIR/ctl-stack_src.c

mkdir -p "$OUT_DIR" "$OBJ_DIR" "$PATCHED_INC"

# ctl.h includes hosted headers unavailable in the freestanding environment.
# Create a patched version with those includes stripped and type stubs added.
# Copy stack.h into the same dir so its #include "ctl.h" finds the patched copy.
{
...
cp "$CTL_SRC_DIR/stack.h" "$PATCHED_INC/stack.h"
...
COMMON_FLAGS=(
  ...
  "$DOMAIN_OPT_LEVEL"
  -DCTL_STACK
  -I"$PATCHED_INC"   # patched ctl.h + copied stack.h (before CTL_SRC_DIR)
  -I"$SUPPORT_DIR"
)
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_SRC" -o "$OBJ_DIR/ctl-stack.o"
...
"$LD_LLD" -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  "$OBJ_DIR/start.o" "$OBJ_DIR/ctl-stack.o" "$OBJ_DIR/beebs_simple_domain.o"
