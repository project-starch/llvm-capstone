...
SCRATCH_DIR=$OUT_DIR/trio-snprintf-src
...
# Shared trio library sources (trio_test.c is replaced by our adapted oracle tail).
for f in trio.c trio.h triostr.c triostr.h triodef.h triop.h; do
...
"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$ADAPTED_DIR/beebs_trio_snprintf_tail.c" \
  -o "$OBJ_DIR/trio_test.o"
"$CLANG" "${COMMON_FLAGS[@]}" \
