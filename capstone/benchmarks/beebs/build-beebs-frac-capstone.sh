...
OBJ_DIR=${OBJ_DIR:-$OUT_DIR/obj-frac}
...
OUT_DOM=${OUT_DOM:-$OUT_DIR/beebs_frac_capstone.dom}

FRAC_SRC=$BEEBS_SRC_DIR/src/frac/libfrac.c
...
PATCHED_FRAC_SRC=$OUT_DIR/libfrac_capstone.c

if [[ ! -f "$FRAC_SRC" || ! -f "$SUPPORT_DIR/support.h" || ! -f "$LIBM_SRC" ]]; then
  echo "missing required frac sources" >&2
  exit 1
fi

mkdir -p "$OUT_DIR" "$OBJ_DIR"

# Strip the hosted math header; provide a freestanding prototype. fabs is
# supplied by the shared soft-float libm.
{
  printf 'double fabs(double);\n'
  sed -e '/#include <math.h>/d' "$FRAC_SRC"
} > "$PATCHED_FRAC_SRC"
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_FRAC_SRC" -o "$OBJ_DIR/libfrac.o"
objs+=("$OBJ_DIR/libfrac.o")
...
"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$SCRIPT_DIR/beebs_simple_domain.c" -o "$OBJ_DIR/beebs_frac_domain.o"
objs+=("$OBJ_DIR/beebs_frac_domain.o")
