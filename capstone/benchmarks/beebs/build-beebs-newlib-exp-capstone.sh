...
OBJ_DIR=${OBJ_DIR:-$OUT_DIR/obj-newlib-exp}
...
OUT_DOM=${OUT_DOM:-$OUT_DIR/beebs_newlib-exp_capstone.dom}

NEWLIB_SRC=$BEEBS_SRC_DIR/src/newlib-exp/ef_exp.c
SUPPORT_DIR=$BEEBS_SRC_DIR/support
COMPILER_RT=$REPO_ROOT/compiler-rt/lib/builtins
PATCHED_SRC=$OUT_DIR/ef_exp_capstone.c
TAIL_SRC=$SCRIPT_DIR/adapted/beebs_newlib_exp_capstone_tail.c

if [[ ! -f "$NEWLIB_SRC" || ! -f "$SUPPORT_DIR/support.h" || ! -f "$TAIL_SRC" ]]; then
  echo "missing required newlib-exp sources" >&2
  exit 1
fi
...
# five calls; rename both so the adapted tail captures all five __ieee754_expf
# results and checks their bit patterns against a host reference.
{
  printf '#define benchmark exp_orig_benchmark\n'
  printf '#define verify_benchmark exp_orig_verify\n'
  cat "$NEWLIB_SRC"
  cat "$TAIL_SRC"
} > "$PATCHED_SRC"
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_SRC" -o "$OBJ_DIR/ef_exp.o"
objs+=("$OBJ_DIR/ef_exp.o")

source "$SCRIPT_DIR/build-beebs-softfloat-common.sh"
objs+=("${softfloat_objs[@]}")

"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$SCRIPT_DIR/beebs_simple_domain.c" -o "$OBJ_DIR/beebs_newlib_exp_domain.o"
objs+=("$OBJ_DIR/beebs_newlib_exp_domain.o")
