...
JFDCTINT_TAIL_SRC=$SCRIPT_DIR/adapted/beebs_jfdctint_capstone_tail.c
...
if [[ ! -f "$JFDCTINT_TAIL_SRC" ]]; then
  echo "missing adapted jfdctint tail source: $JFDCTINT_TAIL_SRC" >&2
  exit 1
fi
...
cat "$JFDCTINT_TAIL_SRC" >> "$PATCHED_JFDCTINT_SRC"
