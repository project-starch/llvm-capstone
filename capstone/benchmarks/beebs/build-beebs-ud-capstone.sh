...
UD_SRC=$BEEBS_SRC_DIR/src/ud/libud.c
PATCHED_SRC=$OUT_DIR/${BEEBS_BENCHMARK}_src_0.c

mkdir -p "$OUT_DIR" "$OBJ_DIR"

# a[i][j] *= 2.0 — long int *= double triggers soft-float libcall (__muldf3).
# Fix: use integer literal 2 (equivalent, since 2.0 has no fractional part).
sed 's/a\[i\]\[j\] \*= 2\.0/a[i][j] *= 2/' "$UD_SRC" > "$PATCHED_SRC"
...
"$LD_LLD" -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  "$OBJ_DIR/start.o" "$OBJ_DIR/src-0.o" "$OBJ_DIR/beebs_simple_domain.o"
