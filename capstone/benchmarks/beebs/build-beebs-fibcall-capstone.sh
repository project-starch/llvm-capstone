...
OUT_DOM=${OUT_DOM:-$OUT_DIR/beebs_fibcall_capstone.dom}

FIBCALL_SRC=$BEEBS_SRC_DIR/src/fibcall/libfibcall.c
SUPPORT_DIR=$BEEBS_SRC_DIR/support

if [[ ! -f "$FIBCALL_SRC" || ! -f "$SUPPORT_DIR/support.h" ]]; then
  echo "missing BEEBS fibcall source tree: $BEEBS_SRC_DIR" >&2
  exit 1
fi
...
"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$FIBCALL_SRC" \
  -o "$OBJ_DIR/beebs_fibcall.o"

"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$SCRIPT_DIR/beebs_fibcall_domain.c" \
  -o "$OBJ_DIR/beebs_fibcall_domain.o"

"$LD_LLD" -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  "$OBJ_DIR/start.o" \
  "$OBJ_DIR/beebs_fibcall.o" \
  "$OBJ_DIR/beebs_fibcall_domain.o"
