...
OUT_DOM=${OUT_DOM:-$OUT_DIR/beebs_cover_capstone.dom}

COVER_SRC=$BEEBS_SRC_DIR/src/cover/libcover.c
SUPPORT_DIR=$BEEBS_SRC_DIR/support

if [[ ! -f "$COVER_SRC" || ! -f "$SUPPORT_DIR/support.h" ]]; then
  echo "missing BEEBS cover source tree: $BEEBS_SRC_DIR" >&2
  exit 1
fi
...
"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$COVER_SRC" \
  -o "$OBJ_DIR/beebs_cover.o"

"$CLANG" "${COMMON_FLAGS[@]}" \
  -c "$SCRIPT_DIR/beebs_cover_domain.c" \
  -o "$OBJ_DIR/beebs_cover_domain.o"

"$LD_LLD" -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  "$OBJ_DIR/start.o" \
  "$OBJ_DIR/beebs_cover.o" \
  "$OBJ_DIR/beebs_cover_domain.o"
