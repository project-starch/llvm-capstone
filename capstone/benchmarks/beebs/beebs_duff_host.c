...
static int fail_cleanup(const char *msg, unsigned long v) {
  fprintf(stderr, "beebs-duff-host: %s (observed=0x%lx)\n", msg, v);
  capstone_cleanup();
  return 1;
}
...
int main(int argc, char **argv) {
  if (argc != 2) {
    fprintf(stderr, "usage: %s <beebs-duff-domain.dom>\n", argv[0]);
    return 2;
  }

  if (capstone_init()) {
    fprintf(stderr, "beebs-duff-host: failed to initialize Capstone\n");
    return 1;
  }
  ...
  printf("beebs-duff-host: correctness marker validated (retval=0x%lx)\n",
         ret);

  if (capstone_cleanup()) {
    fprintf(stderr, "beebs-duff-host: failed to clean up Capstone\n");
    return 1;
  }
