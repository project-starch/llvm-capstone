...
REWRITE_NDES=$SCRIPT_DIR/adapted/rewrite_ndes_capstone.py

if [[ ! -f "$NDES_SRC" || ! -f "$SUPPORT_DIR/support.h" || ! -f "$REWRITE_NDES" ]]; then
...
python3 "$REWRITE_NDES" "$NDES_SRC" "$PATCHED_NDES_SRC"
