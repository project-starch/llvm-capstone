...
/* VENDOR-NOTES (Capstone PureCap port) -- edits from upstream rhempel/umm_malloc:
 *   1. Freestanding includes: dropped <stdio.h>/<string.h> (no hosted libc in a
 *      Capstone domain); the mem* helpers we use are declared below and resolved
 *      at link from the tag-preserving freestanding string lib.
 *   2. Block header padded to 16 bytes (see the umm_block struct) so the returned
 *      data pointer is 16-byte (capability) aligned. See umm_malloc_cfg.h.
 * No algorithmic changes: umm's index-based (never pointer-based) metadata is
 * already PureCap-safe. */
#include <stdint.h>
#include <stddef.h>
...
/* VENDOR EDIT: header union padded from sizeof(umm_ptr) (4) to 16 bytes so the
 * body -- and thus the returned data pointer -- begins at block_base + 16, which
 * is 16-byte (capability) aligned given a 16-aligned heap array and a 16-multiple
 * block size. The free-list still stores only uint16_t indices (umm_ptr). */
#define UMM_HEADER_SIZE 16
UMM_H_ATTPACKPRE typedef struct umm_block_t {
    union {
        umm_ptr used;
        uint8_t _capstone_pad[UMM_HEADER_SIZE];
    } header;
    union {
        umm_ptr free;
        uint8_t data[UMM_BLOCK_BODY_SIZE - UMM_HEADER_SIZE];
    } body;
