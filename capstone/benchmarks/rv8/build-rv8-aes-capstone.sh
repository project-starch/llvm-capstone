...
SRC=$RV8_SRC_DIR/src/aes.c
PATCHED_SRC=$OUT_DIR/aes_capstone.c
...
# Strip hosted includes (preamble/string lib/rv8_malloc replace them) and drop
# the upstream main() (large malloc'd round-trip + printf; our tail drives a
# known-answer + round-trip check). aes.c uses its own u8/u32 typedefs.
sed -E -e '/^[[:space:]]*#[[:space:]]*include[[:space:]]+<(stdio|stdlib|string)\.h>/d' \
       -e '/^int main\(\)/,$d' \
       "$SRC" > "$PATCHED_SRC"
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_SRC" -o "$OBJ_DIR/aes.o"; objs+=("$OBJ_DIR/aes.o")
"$CLANG" "${COMMON_FLAGS[@]}" -c "$ADAPTED_DIR/rv8_aes_tail.c" -o "$OBJ_DIR/tail.o"; objs+=("$OBJ_DIR/tail.o")
