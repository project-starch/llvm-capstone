...
SRC=$RV8_SRC_DIR/src/dhrystone.c
PATCHED_SRC=$OUT_DIR/dhrystone_capstone.c
...
# Strip hosted includes (the preamble replaces them) and pin LOOPS to a small,
# deterministic count -- the perf metric is irrelevant; we only validate the
# end-state self-check (Array2Glob[8][7] == LOOPS + 10).
sed -E -e '/^[[:space:]]*#[[:space:]]*include[[:space:]]+<(stdio|stdlib|string|sys\/time)\.h>/d' \
       -e 's/#define LOOPS 10000000/#define LOOPS 100000/' \
       "$SRC" > "$PATCHED_SRC"
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_SRC" -o "$OBJ_DIR/dhrystone.o"; objs+=("$OBJ_DIR/dhrystone.o")
"$CLANG" "${COMMON_FLAGS[@]}" -c "$ADAPTED_DIR/rv8_dhrystone_tail.c" -o "$OBJ_DIR/tail.o"; objs+=("$OBJ_DIR/tail.o")
"$CLANG" "${COMMON_FLAGS[@]}" -c "$ADAPTED_DIR/rv8_malloc.c" -o "$OBJ_DIR/malloc.o"; objs+=("$OBJ_DIR/malloc.o")
"$CLANG" "${COMMON_FLAGS[@]}" -c "$ADAPTED_DIR/rv8_stubs.c" -o "$OBJ_DIR/stubs.o"; objs+=("$OBJ_DIR/stubs.o")
