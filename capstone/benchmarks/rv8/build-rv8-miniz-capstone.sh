...
SRC=$RV8_SRC_DIR/src/miniz.c
PATCHED_SRC=$OUT_DIR/miniz_capstone.c
...
# Keep freestanding <stddef.h>/<stdint.h>; strip hosted includes (preamble +
# string lib + arena replace them). Drop the upstream main() (8 MB malloc'd
# round-trip + printf; our tail drives a small round-trip).
sed -E -e '/^[[:space:]]*#[[:space:]]*include[[:space:]]+<(stdio|stdlib|string|assert|time|sys\/stat|sys\/utime)\.h>/d' \
       -e '/^int main\(\)/,$d' \
       "$SRC" > "$PATCHED_SRC"
