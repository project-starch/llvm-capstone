...
| norx | **BLOCKED (root cause found)** | Builds; runtime capability-tag fault. **Diagnosed**: `cf_norx32_encrypt` has 10 args, so its `ciphertext`/`tag` (args 9-10) are **stack-passed capabilities**, which the backend currently delivers **untagged** — a calling-convention ABI bug. Bisected (encrypt-only → `nbytes=0` → minimal repro), confirmed by `capstone/tests/runtime-qemu/stack-cap-arg-repro/`. Same class as the fixed `va_list` bug. **Fix**: pass stack-passed capability args in 16-byte tagged slots (`stc`/`lc`) in `CapstoneISelLowering` `LowerCall`/`LowerFormalArguments`. Once fixed, norx should pass (scaffolding ready). |
