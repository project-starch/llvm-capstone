...
SRC=$RV8_SRC_DIR/src/qsort.c
PATCHED_SRC=$OUT_DIR/qsort_capstone.c
...
# Strip hosted includes (preamble replaces them) and drop the upstream main()
# (200 MB malloc + printf, unused on the domain -- our tail drives the sort).
sed -E -e '/^[[:space:]]*#[[:space:]]*include[[:space:]]+<(stdio|stdlib)\.h>/d' \
       -e '/^int main\(\)/,$d' \
       "$SRC" > "$PATCHED_SRC"
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_SRC" -o "$OBJ_DIR/qsort.o"; objs+=("$OBJ_DIR/qsort.o")
"$CLANG" "${COMMON_FLAGS[@]}" -c "$ADAPTED_DIR/rv8_qsort_tail.c" -o "$OBJ_DIR/tail.o"; objs+=("$OBJ_DIR/tail.o")
