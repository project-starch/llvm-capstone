...
SRC=$RV8_SRC_DIR/src/norx.c
PATCHED_SRC=$OUT_DIR/norx_capstone.c
...
# Keep freestanding <stdint.h>/<stddef.h>; strip hosted ones (preamble + string
# lib + bzero macro replace them). Drop the upstream main() (32 MB malloc'd
# round-trip + printf; our tail drives a small static-buffer round-trip check).
sed -E -e '/^[[:space:]]*#[[:space:]]*include[[:space:]]+<(stdio|stdlib|string|strings|assert)\.h>/d' \
       -e '/^int main\(\)/,$d' \
       "$SRC" > "$PATCHED_SRC"
...
"$CLANG" "${COMMON_FLAGS[@]}" -c "$PATCHED_SRC" -o "$OBJ_DIR/norx.o"; objs+=("$OBJ_DIR/norx.o")
"$CLANG" "${COMMON_FLAGS[@]}" -c "$ADAPTED_DIR/rv8_norx_tail.c" -o "$OBJ_DIR/tail.o"; objs+=("$OBJ_DIR/tail.o")
