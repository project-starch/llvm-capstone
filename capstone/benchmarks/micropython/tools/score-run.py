...
Three outcomes per test, kept distinct on purpose:
...
def main():
    ...
    with open(args.expected) as f:
        for line in f:
            ...
            exp.append((int(idx), name, int(length), int(word, 16), how))
    if not exp:
    ...
    print(f"\n{npass} pass, {nfail} fail, {nabsent} absent, of {len(exp)} tests")
