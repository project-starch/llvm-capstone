...
PLAN = {
    ...
    "MPY-T07": ("lexer-source-name-uaf", "unknown", "no fix commit names the issue and no Python trigger is published"),
    ...
    "MPY-T18": ("gc-collect-frees-live-binding", "unknown", "needs the LVGL C binding; not part of this port"),
    ...
    "MPY-T21": ("rp2-thread-stack-not-scanned", "unknown", "needs the rp2 port and threads"),
    "MPY-T22": ("concurrent-vfs-gc-race", "no", "needs threads and a filesystem, both absent here"),
    "MPY-T23": ("esp32-lwip-freed-buffer", "unknown", "closed upstream as unreproducible; needs the ESP32 network stack"),
    ...
    "MPY-T28": ("gc-realloc-returns-null", "c-level", "the published repro calls gc_alloc/gc_realloc from C; no Python trigger exists"),
    "MPY-T29": ("gc-free-assert-invalid-block", "c-level", "the published repro drives the lexer/compiler from C; no Python trigger exists"),
...
def render_status(rows):
    ...
    heads = [
        ("yes", "Measured in the Capstone domain",
