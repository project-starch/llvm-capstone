...
PLAN = {
    ...
    "MPY-T06": ("btree-reuse-after-close", "parent-measured", "fixed at the pin; also needs the btree module, which this domain does not build"),
        "MPY-T07": ("lexer-source-name-uaf", "yes", "measured twice: at the 2018 parent on the host, where the use-after-free executes and is silent, and in the domain as a reconstruction behind MPY_T07_LEXER_UAF, since mp_parse still frees the lexer at the pin and the caller misuse 4128 fixed is still available"),
    ...
    "MPY-T16": ("deinit-after-gc-sweep-all", "yes", "reconstructed, not the upstream program: the ESP32 port runs gc_sweep_all() before mp_deinit() and this domain owns its own teardown, so the ORDERING is reproduced instead -- a raw pointer to a GC block held across gc_sweep_all(), then read and written"),
...
def main():
    ...
    problems, made = [], 0
    excl_path = os.path.join(S, "EXCLUDED.md")
    excl = [r for r in rows if r["is_temporal"] != "yes"]
    excl_text = ("# Excluded from the corpus\n\n"
    ...
    for r in sorted(excl, key=lambda x: x["id"]):
        excl_text += f"| `{r['id']}` | {r['ref']} | {r['is_temporal']} | {r['temporal_evidence']} |\n"
    if check:
        if not os.path.exists(excl_path) or open(excl_path).read() != excl_text:
            problems.append("EXCLUDED.md stale, rerun gen-cases.py")
    else:
        open(excl_path, "w").write(excl_text)
    ...
    certain = [r for r in rows if r["is_temporal"] == "yes"]
    for r in certain:
        rid = r["id"]
        if rid not in PLAN:
    ...
    if problems:
        ...
        for p in problems:
            print("   ", p)
        return 1
    print(f"{'OK' if check else 'wrote'}: {made or len(certain)} certain temporal cases")
    return 0
