...
## What is actually on record
    ...
    $ grep -rn '1840\|1,840\|85\.05\|84\.15' capstone/docs/
    (no output, exit 1)

    $ grep -rn '84\.13' capstone/docs/
...
The check that did catch it is one line, and it is cheap enough to be routine:
    ...
    grep -rn '<each number about to be asserted>' capstone/docs/
...
prose in `docs/`, proving only that the grep worked on uncompressed text in the searched
