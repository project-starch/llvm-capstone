> **Agent-B private next-step** — this is Agent-B's copy on branch `capstone-bootstrap-b`
> (clone `<REPO-ROOT-B>`). Do NOT edit `current-next-step.md` (Agent-A's
...
- **Longer-term fix (owner = us, in-repo) — fix OUR domain runtime, NOT the RTL** (corrected
  ...
  `<HOME>/.claude-b/plans/curried-crunching-gizmo.md`.
