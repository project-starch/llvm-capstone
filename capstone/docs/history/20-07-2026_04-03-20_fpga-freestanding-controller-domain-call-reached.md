...
- **Monitor** (`create_domain`, patch in `docs/patches/fpga-gpseed-monitor-createdomain.patch`):
...
1. **FP/glibc blocker — FIXED and PROVEN on-board.** The benchmark's `.user` controller was
   ...
   the board owner's steer ("use soft float") = **a freestanding controller that links no glibc**:
...
- ✅ **Fix localized to OUR domain runtime** (`my_first_domain/start.S` + clang codegen),
  **NOT the RTL** — corrected 2026-07-20 after the board owner's answer (see CORRECTION below).
- ❌ No `RESULT` cycle numbers yet — but the fix is in-repo (no bitstream rebuild needed).

## CORRECTION (2026-07-20, the board owner's answer) — the fix is OURS, not the RTL
...
1. **The `mtvec` dumper stayed silent; `$mcause=$mepc=$mtval=0`.** No M-mode trap. So the
   ...
   the board owner confirmed `cscall`/`csreturn` **implicitly flush the icache**, so the stale-icache /
...
out-of-tree bitstream rebuild (Vivado + anvil), so it is an escalation to the board owner, not fixable
here. The FPGA cycle-accurate borrow/revoke numbers are **blocked on this RTL fix.**

**One thing for the board owner to confirm (representability):** QEMU keeps exact bounds in a side
...
- Plan / staged ladder: `<HOME>/.claude-b/plans/curried-crunching-gizmo.md`.
