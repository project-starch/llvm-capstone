# Historical trail split out of `state/current-state.md` (2026-09-04)

`state/current-state.md` had become a 1,298-line append-only log reaching back to June, whose
header said CURRENT while most of it was self-labelled SUPERSEDED. It is the first file every new
session is told to read, so the cost of that was paid on every session.

**Nothing here was deleted or edited** — this is the file's content from `## 2026-08-16` downward,
verbatim. The newest entry below predates two RTL fixes and a reflash. Read it as history: for
what is true now, read `state/current-state.md`, and for the authoritative status of any specific
defect read `ref/ISSUES.md`, which outranks both.
