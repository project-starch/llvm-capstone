...
```
git clone https://github.com/project-starch/llvm-capstone <REPO-ROOT-B>
cd <REPO-ROOT-B>
git checkout -b capstone-bootstrap-b origin/capstone-bootstrap
git submodule update --init --recursive
```
Paths below assume Agent-A = `<REPO-ROOT>`,
Agent-B = `<REPO-ROOT-B>`.
...
cd <REPO-ROOT-B> || exit 1
exec claude "$@"
```
Then just run `claude-b`. (Optionally add `alias claude-a='cd <REPO-ROOT> && claude'` for symmetry.)
...
echo "$CAPSTONE_REPO_ROOT"     # => <REPO-ROOT-B>
...
- `capstone/docs/state/current-state.md` → **but see §5 on per-agent state**
- `capstone/docs/MULTI-AGENT-WORKFLOW.md`
- `capstone/docs/COORDINATION.md`  (who owns what right now)
...
- **B does pure docs/analysis, no compiling → skip the build.** Optionally
  *read-only* share A's clang so smoke checks work:
  ...
  ln -s <REPO-ROOT>/llvm/cmake-build-debug \
        <REPO-ROOT-B>/llvm/cmake-build-debug
  ```
...
```bash
source capstone/tests/capstone-test-env.sh
[ "$CAPSTONE_REPO_ROOT" = <REPO-ROOT-B> ] && echo "repo OK"
