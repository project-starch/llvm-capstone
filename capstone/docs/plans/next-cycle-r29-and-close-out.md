...
| **R-10** | close-out proposed demoting its secondary defect to "a one-line RTL observation" | **Do not.** "Holds a capability" is decided by OR-reducing a user/metadata word and never consults `cap_type` — the same word-vs-granule confusion R-29 is about. **Line numbers corrected 2026-09-15:** the two sites once cited here (`wt_axi_adapter.sv:196`, `wt_dcache_mem.sv:138`) are REPAIRED and now carry explicit opcode-derived flags; the live pair is on the REFILL path, `wt_dcache_mem.sv:358` (`rd_ctag = \|wr_cl_user_i[7:0]`) and `:501` (`cap_tag_q[...] <= wr_vld_bits_i[j] & (\|wr_cl_user_i[7:0])`), present on every tree checked including `66c4e7517` and dev's pin. Citing the repaired sites is how a reader concludes the entry is stale and closes it wrongly — which nearly happened on 2026-09-15. Link it to R-29 as a sibling account; R-10 stays OPEN, and its unrun stage-8 discriminator stays on the list. |
