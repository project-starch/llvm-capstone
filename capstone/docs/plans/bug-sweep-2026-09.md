...
- Pending, infrastructure-blocked: the S-04 -O1 arm boot (image `sqm0o1` b4b453f90cd7edb4 baked in the board overlay; `retry-s04.sh` probes the console until ~18:08 and boots it as sw22 if the host resolves — any such reading is unreviewed and needs a batch-5 line or a rerun). **18:05: handed to the board lane** (every board run goes through it; the probe loop was stopped before it could fire) — sw22 is the first boot of its phase A and it files the batch-5 line.
