...
### Verification of the merged state (2026-09-07 12:50–13:28) — the plan's last check

No `nightly-<TS>` report existed under `/tmp/capstone` since the merge, so the nightly was run from this worktree on the merged code (dev e3dd2619bc9b differs from the branch head only by the board lane's doc/rows commit; toolchain fresh, clang embeds 64d67345b0d2). Report `/tmp/capstone/nightly-20260907_125020/report.md`: lit PASS; coremark, sqlite-memory, rv8, revoke-matrix, revoke-on-free, borrow-cost, tree-cost-O2, static-cap-globals, intra-domain-mrev, hier-revoke, shared-region, linear-uninit-corpus PASS; **sqlite-slt PASS** (the row C-44 predicted would fail — green on the fresh compiler with the pin retired, as the 09-05 select1 run said). Two red rows, both explained and neither a code defect: (1) `lit-generic` FAIL(4): `Generic/bswap.ll` needs `lli`, `Generic/dwarf-md5.ll`/`dwarf-source.ll` need `llvm-dwarfdump`, `RISCV/rvv/debug-info-rvv-dbg-value.mir` likewise — none of those tools is built in this worktree's 30-tool build dir ("command not found", exit 127 in the log); the same four tests PASS with the main checkout's full build on the same source (`llvm-lit -q`, 4 discovered, rc 0). (2) `beebs` FAIL(1): `run-beebs-edn` — the guest's 9p copy stalled before the shell prompt (`RuntimeError: QEMU stopped before the shell prompt while running guest command: cp /mnt/host/beebs_edn_host.user …`), the aha-mont64 shape of 09-05; two fresh-boot reruns of edn alone PASS (`correctness marker validated (retval=0xc171c0de)`, serial logs with boot banners, 13:27:59 and 13:28:04). The plan's verification list is complete: lit green, coverage 0 gaps (09-05), every suite green on the merged code or explained by a worktree tool gap or a guest-share stall that does not repeat first-in-boot.

Gate fix in the same commit: `toolchain-fresh.py` read a false STALE at the nightly's start on the five revision-chain objects (`AsmPrinter`, `LTO`, `IRSymtab`, the two `Version.cpp`), because ninja prints objects as `<dir>/CMakeFiles/<target>.dir/<file>.o` and the suffix match did not allow for the `CMakeFiles` segment — the 09-05 controls never exercised that branch (the tree had zero pending steps right after its full rebuild). Fixed by stripping the segment before matching; controls: the chain-pending worktree reads fresh (3 relink steps), the main checkout reads fresh, a touched `CapstoneSubtarget.cpp` reads STALE naming its object, and after a one-object rebuild the tree reads fresh again.

### C-9 instrumented (2026-09-09) — NOT REPRODUCED at the level it was filed at

C-9 (redundant `mv rd, rd` around an inline-asm `+r` tie) was deferred in the first pass; now instrumented. The filed shape (2026-07-27) is an **optimized** loop: "5-instruction body became 7 — `srai / xor / add / mv / addi / mv / bne`". On the current compiler a reproducer carrying that tie (`c9.c`: `acc = (acc>>1) ^ (acc+i)` with `asm volatile("" : "+r"(acc))` in the loop) emits **zero** self-moves at -O1 and -O2 — the clean 5-instruction body around `#APP/#NO_APP`, identical mv-count to the plain riscv64 target. So the wart does not reproduce at the level it was filed at.

A separate `-O0` observation, **not** C-9: that reproducer emits 9 `mv rd,rd` at -O0, but **deleting the tie leaves all 9** (measured) — they are stack-slot capability address materialization (`cincoffsetimm / mv / mv / shrink / sd`) under FastRegAlloc, unrelated to the inline-asm tie, and they vanish at -O1. The ladder builds at -O0 but measures instret from the optimized comparison (the `ctrsanity_kernel.h` note's "5 instructions" is the optimized body), and the kernel already dropped the in-loop tie, so nothing measured is affected.

**Verdict: NOT REPRODUCED** (not GONE: the 2026-07-27 instrument could not be fired on the artifact it was written against — neither archived old compiler, `llvm-pre-c128` nor `llvm-0819`, has a `clang` or `llc` binary present to re-emit it). No fix: the current compiler does not emit the wart in optimized code, and the -O0 self-moves are a different, correct, cosmetic artifact. The combined plan's §3 listing of C-9 as a "real fix, this lane" is corrected to NOT REPRODUCED / no work.
