...
1. **Global `char *nums[]` pointer array is untagged on the domain load path.**
   ...
   **RESOLVED 2026-06-24** (blocker #1). The backend now auto-materializes
   ...
   `capstone/docs/design/capability-globals-init-decision.md`; test
...
| FP (self-contained newlib routines) | (was FP-blocked) | `newlib-sqrt`, `newlib-exp`, `newlib-log`, `newlib-mod` | **RESOLVED** — each `ef_*.c` ships its own `__ieee754_*f` (integer bit-manip + non-contracted float), so only the soft-float builtins are linked (no libm). Built `-ffp-contract=off`. `newlib-sqrt` keeps the upstream exact `==` verifier with `exp[]` moved to `static const` (Bug #9). `newlib-exp/log/mod` (`verify` was -1) get oracle tails capturing all five calls and exact-bit-comparing vs a `gcc -O0 -ffp-contract=off` host reference. `run-beebs-newlib-{sqrt,exp,log,mod}.sh`. |
| FP (Perlin noise; libm `floor`) | (was FP-blocked) | `stb_perlin` | **RESOLVED** — self-contained oracle (`benchmark()` compares a 10x10 noise plane against a `static const float expected[][]` global and returns 0 on full match; no Bug #9). Adapted tail just checks `res == 0`. Only external dep is `floor`, added to the shared `adapted/beebs_softfloat_libm.c` (bit-exact). Built `-ffp-contract=off`; host and target match the embedded table bit-for-bit. `run-beebs-stb_perlin.sh`. |
| (duplicate of `matmult`) | n/a | `matmult-int` | **N/A — already covered.** `src/matmult-int/matmult.c` is byte-identical to `src/matmult/matmult.c`, which `run-beebs-matmult.sh` already builds with `-DMATMULT_INT` (flat-array Bug #3 source workaround). Adding a separate runner would be a pure duplicate. |
| FP (float matmul; soft-float only) | (was FP-blocked) | `matmult-float` | **RESOLVED** — same source built `-DMATMULT_FLOAT` (UPPERLIMIT 10, `float[10][10]`); soft-float builtins only (no libm). Adapted tail replaces the upstream local-`exp[][]` verifier (Bug #3/#9) with an FNV-1a checksum of the global `ResultArray` (byte stride → no i128 GEP; global → no Bug #9) vs a host reference `0xbdbace3d315e67a4`. Built `-ffunction-sections`/`--gc-sections` to drop the dead `values_match` (else undefined `frexpf`/`fabsf`). `run-beebs-matmult-float.sh`. |
| FP (Whetstone; libm `atan`) | (was FP-blocked) | `whetstone` | **RESOLVED** — added `atan` to the shared libm (fdlibm port, ~1.6e-16). Upstream `verify` is -1 and results flow only through `POUT` (gated on `PRINTOUT`); built `-DPRINTOUT`, the upstream printf `POUT` definition block stripped, and a capturing `POUT` in the tail FNV-folds every module's four doubles, compared **exactly** to a same-libm host reference `0x2f975c4609a1bfbb` (double IEEE ops are bit-identical host↔target at `-ffp-contract=off`, so the libm's absolute accuracy is irrelevant). `run-beebs-whetstone.sh`. |
