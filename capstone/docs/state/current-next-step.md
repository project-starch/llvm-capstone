...
| **R-33** | **NEW, and it is capability soundness rather than instrumentation.** The region allocator hands out sizes that are **not representable** in the compressed bounds encoding, so moving a cursor widens a capability's authority past its own allocation by up to one granule less a byte — `STC`'s check consumes the decompressed (rounded) `end`, and `CINCOFFSET` takes the same path on ordinary linear capabilities. Measured on silicon (sw62 `RCEN`); the over-permissive store itself is **not yet demonstrated**. Contained by the kernel's `PAGE_ALIGN` below 4 MiB, **not** contained at or above it. Cause is the allocator, not the encoder: round region sizes to the granule at creation. |
