...
- `capstone/benchmarks/coremark/` — CoreMark 1.01 PureCap bring-up (complete). All three
  ...
  their root causes are documented in `capstone/docs/plans/backend-compiler-fixes.md`.
  Architecture-specific design decisions (CRC derivation, node count differences) are in
  `capstone/docs/design/coremark-purecap.md`.
...
- if it is project memory or process guidance, update `capstone/docs/`.
