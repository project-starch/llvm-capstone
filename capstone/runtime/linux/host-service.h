 ...
 */
#ifndef CAPSTONE_LINUX_HOST_SERVICE_H
#define CAPSTONE_LINUX_HOST_SERVICE_H
...
#include "../include/capstone/hostcall-file-service.h"

struct hc_host {
  struct hostcall_file_service_handle_slot
      slots[HC_FILE_SERVICE_MAX_HANDLES];
  char path[HC_V0_REGION_SIZE + 1];
...
static char hc_host_bounce_buf[HC_V0_REGION_SIZE];
...
static inline int hc_host_service(struct hc_host *h, const struct hostcall_v0 *req,
  ...
  const hostcall_u64_t max = HC_FILE_SERVICE_MAX_HANDLES;
  switch (req->opcode) {
  ...
  case HC_V0_OP_PATH_READLINK: {
    ...
    memcpy(h->path, payload + req->offset, (size_t)req->length);
    h->path[req->length] = '\0';
    ssize_t n = readlink(h->path, payload, HC_V0_REGION_SIZE);
