...
NAMES = [
    "shared-references-callbacks-deferred-close",
    "buffer-read-after-return",
    "buffer-write-after-return",
    "buffer-read-after-reuse",
    "buffer-write-after-reuse",
    "refstruct-read-after-return",
    "refstruct-write-after-reuse",
    "refstruct-stale-unref",
    "nested-child-read-after-reset",
    "buffer-read-after-backing-free",
    "refstruct-metadata-underflow",
    "buffer-one-past-end",
...
parser.add_argument(
    "--modes",
    default="0,1,2",
    help="Capstone modes: 0=bounds, 1=backing lifetime, 2=Sublet pool leases",
)
...
domain_build = pathlib.Path(
    os.environ.get("FFPOOL_DOMAIN_BUILD_DIR", work / "build/capstone-domain")
)
linux_build = pathlib.Path(
    os.environ.get("FFPOOL_LINUX_BUILD_DIR", work / "build/linux-guest")
)
verdicts = []
for mode in map(int, args.modes.split(",")):
    for case in map(int, args.cases.split(",")):
        ...
        share = run / "share"
        share.mkdir()
        shutil.copy2(domain_build / "bin/pool-security.dom", share / "security.dom")
        shutil.copy2(linux_build / "bin/domain-loader", share / "host.user")
...
"""
        ...
        row = dict(
            ...
            case=case,
            name=NAMES[case],
            expected="fault" if fault else ("rejected" if status else "completed"),
        ...
        if fault:
            ...
            row["passed"] = (
                ...
                and "FF2 return=42044" not in serial
            )
            if case in (10, 11):
        ...
        print(
            f"{'PASS' if row['passed'] else 'FAIL'} mode={mode} {NAMES[case]} expected={row['expected']}",
            flush=True,
        )
        if not row["passed"]:
            raise SystemExit(1)
