...
OUT=${1:?usage: record.sh RESULT-DIR [duration-seconds] [size]}
DURATION=${2:-300}
SIZE=${3:-1280x720}
...
# Require identical decoded frames before deriving the replay command stream.
cmp "$OUT/stock.framemd5" "$OUT/traced.framemd5"
python3 "$HERE/memory/analyze.py" commands "$OUT/recorded.bin" "$OUT/commands.bin"
