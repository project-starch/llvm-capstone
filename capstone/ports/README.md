...
| APR | Pool allocator in a Capstone domain with a Sublet adapter, and a stock CheriBSD build against the platform's own malloc, for the httpd/APR defect corpus; not httpd, not threaded pools | [pools](apr/pools/README.md); shared CMake layout. The [census](apr/README.md) that preceded it stays beside it |
| memcached | Allocator census of 1.6.45: slabs, per-thread object caches, response bundles, bipbuffer, extstore, and the lifetimes around them; no port yet | [Census](memcached/README.md); counted from the source, before any porting |
