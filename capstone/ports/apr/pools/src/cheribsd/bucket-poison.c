/* CheriBSD with PoisonCap: the bucket allocator's seam over the PoisonCap
 * node adapter. The same records and the same LIFO as the other adapters;
 * a piece is published from the manager's authority over its block with
 * exact bounds and no poison authority, and in mode 1 an individual free
 * poisons and sweeps the piece before the freelist can reissue it -- the
 * transition the wmem port's recycler hook exercises, reached here through
 * apr_bucket_free. The block's return to APR invalidates the whole node in
 * the node adapter, pieces included. */
#include "port.h"
#include "poison.h"
...
void aprp_poison_count_piece(void);
...
struct block {
  ...
  void *header;  /* the alias upstream keeps */
  void *manager; /* the node adapter's authority over the node */
  unsigned live;
};
struct piece {
  uintptr_t address;
...
void *aprb_block_lend(void *block) {
  ...
  b->manager = aprp_poison_authority(b->header);
...
void *aprb_carve(void *block, size_t size) {
  ...
  p->live = 1;
  ++carved;
  return aprp_poison_publish((char *)b->manager + (p->address - b->address), size);
}
void aprb_file(void *list, void *node) {
  ...
  if (aprp_mode()) {
    struct block *b = &blocks[p->block];
    aprp_poison_invalidate((char *)b->manager + (p->address - b->address), p->size);
    aprp_poison_count_piece();
  }
  p->filed = 1;
  p->next_free = l->head;
  l->head = (int)(p - pieces);
...
void *aprb_reissue(void *list, apr_memnode_t **memnode) {
  ...
  *memnode = b->header;
  ++reissues;
  return aprp_poison_publish((char *)b->manager + (p->address - b->address), p->size);
}
