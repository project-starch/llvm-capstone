...
FILES = ("memory/unix/apr_pools.c", "include/apr_pools.h", "include/apr_allocator.h")
with tempfile.TemporaryDirectory() as temporary:
    root = Path(temporary)
    with tarfile.open(archive) as source:
        for name in FILES:
            source.extract(f"apr-1.7.4/{name}", root, filter="data")
    tree = root / "apr-1.7.4"
    ...
    result = subprocess.run(
        [sys.executable, port / "cmake/prepare-source.py", archive, "0" * 64, "1.7.4", tree],
        capture_output=True,
    )
