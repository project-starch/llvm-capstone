...
"$LD_LLD" --gc-sections -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  ...
  "$OUT_DIR/set_thread_area.o" "$OUT_DIR/setjmp.o" "${MUSL_OVERRIDE_OBJS[@]}" "$OUT_DIR/level0.o" "${softfloat_objs[@]}" "$OUT_DIR/file_probe.o" "$ARCHIVE"
...
control=$("$LD_LLD" --gc-sections -T "$LINKER_SCRIPT" -o "$OUT_DIR/nohostcall.dom" \
  ...
  "$OUT_DIR/set_thread_area.o" "$OUT_DIR/setjmp.o" "${MUSL_OVERRIDE_OBJS[@]}" "$OUT_DIR/level0.o" "${softfloat_objs[@]}" "$OUT_DIR/file_probe.o" "$ARCHIVE" 2>&1)
set -e
