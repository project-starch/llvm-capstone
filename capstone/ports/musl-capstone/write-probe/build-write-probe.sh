...
"$LD_LLD" --gc-sections -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  ...
  "$OUT_DIR/set_thread_area.o" "$OUT_DIR/setjmp.o" "$OUT_DIR/string_bounds_safe.o" "$OUT_DIR/mbsrtowcs_bounds_safe.o" "$OUT_DIR/fputwc_null_safe.o" "$OUT_DIR/level0.o" "${softfloat_objs[@]}" "$OUT_DIR/write_probe.o" "$ARCHIVE"
...
control=$("$LD_LLD" --gc-sections -T "$LINKER_SCRIPT" -o "$OUT_DIR/nohostcall.dom" \
  ...
  "$OUT_DIR/set_thread_area.o" "$OUT_DIR/setjmp.o" "$OUT_DIR/string_bounds_safe.o" "$OUT_DIR/mbsrtowcs_bounds_safe.o" "$OUT_DIR/fputwc_null_safe.o" "$OUT_DIR/level0.o" "${softfloat_objs[@]}" "$OUT_DIR/write_probe.o" "$ARCHIVE" 2>&1)
set -e
