...
#
# ITS PREMISE IS GONE AS OF 2026-08-24, AND THIS CONTROL IS EXPECTED TO FLIP.
# A capability is MVT::c128 in its own register class now, so a 128-bit shift is
# an ordinary illegal-type expansion and long double compiles: the union
# bit-pattern access every musl long-double routine starts with is pinned in
# clang/test/CodeGen/capstone-long-double.c, and `long double` arithmetic goes to
# __addtf3 like any RV64 without quad hardware.
#
# The control is left in place ON PURPOSE. A flipped control makes this harness
# report ERROR rather than a number, which is exactly the signal wanted: the next
# person to run it against a real musl tree finds out that the premise moved,
# rather than reading a number whose denominator quietly changed. Replacing it
# needs a musl tree to pick and verify a new must-fail file against, which is a
# deliberate act, as the note above says.
