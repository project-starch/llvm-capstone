...
### The musl files on THIS branch are stale; `musl-capstone-port` owns them

`arch-capstone64/syscall_arch.h` here defines `syscall_arg_t` as
`__UINTPTR_TYPE__`, which on this target is the 64-bit **address**, not the
capability. A pointer argument then crosses the boundary as an address, and
`hostcall.c` casts it back and dereferences it -- rebuilding a pointer from an
address yields an UNTAGGED capability, so the load faults with cause 24.

**That is already fixed on `musl-capstone-port`**, which is 93 commits ahead of
this branch and is where the port actually lives. It uses

    typedef void *syscall_arg_t;

so the capability is passed and the tag survives: the argument is a plain `movc`
with no kill marker narrowing it to the address half. Verified against the
current compiler on both spellings.

That branch also carries the rest of the port that is not here at all --
`libc-ext/` (malloc, memcpy, string, tls, locks, printf) and the lua, mruby and
file probes. Do not read this directory as the state of the musl work.
...
bash capstone/ports/musl-capstone/printf-probe/run-printf-probe.sh # one QEMU boot
```
