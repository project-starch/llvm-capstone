#!/usr/bin/env bash
# Reproducers for two runtime defects the tshark port found, run through the port's QEMU runner
# (host/run-qemu.sh, oracle mode, with the program's native build as the reference):
#
#   run.sh c64-fixed   ctors.c with src/tsapp-init-fini.c: stdout MATCH (constructors, main,
#                      destructors in native order)
#   run.sh c64         ctors.c without it: HALTED, cause 24, in musl's exit() walking .fini_array
#                      through uintptr_t (and the constructors never ran)
#   run.sh c65         cond.c: HALTED, cause 24, in musl's __private_cond_signal
#   run.sh i11         closefd1.c: the runtime's "capstone-domain: UNSERVED syscalls" line is
#                      missing; the port's exit hook still reports unserved=160x2 stdout=closed
#
...
CASE=${1:?c64-fixed | c64 | c65 | i11}
case $CASE in c64-fixed|c64) SRC=ctors.c ;; c65) SRC=cond.c ;; i11) SRC=closefd1.c ;; *) echo "unknown case" >&2; exit 2 ;; esac
...
EXTRA=()
if [ "$CASE" = c64-fixed ]; then
  "$CAPSTONE_CLANG" "${RTF[@]}" -c "$APP/src/tsapp-init-fini.c" -o "$W/tsapp-init-fini.o"; EXTRA=("$W/tsapp-init-fini.o")
fi
...
"$CAPSTONE_LD_LLD" --gc-sections -T "$TS_LINKER_SCRIPT" -o "$W/tshark_m5.dom" "${RT[@]}" "$W/level0.o" \
  "$W/tsapp-heap.o" "${EXTRA[@]}" "$W/domreq.o" "$W/prog.o" "$TS_LIBC_ARCHIVE"
