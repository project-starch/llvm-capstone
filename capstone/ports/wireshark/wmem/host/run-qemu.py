"""Replay wmem pool requests under QEMU, retaining every attempted run."""
...
share = run / "share"
record_trace(run, share / "trace.bin", "wireshark.wmem")
(share / "run.sh").write_text(f"""#!/bin/sh
set -e
cp /mnt/host/replay.dom /tmp/wm.dom
cp /mnt/host/host.user /tmp/wm-host
cp /mnt/host/trace.bin /tmp/wm-trace.bin
/tmp/wm-host /tmp/wm.dom /tmp/wm-trace.bin /tmp/wm-report.bin {int(a.protection == 'sublet')}
...
print(f"wmem artifacts: {run}", flush=True)
env = dict(os.environ)
env.setdefault("CAPSTONE_QEMU_LOGIN_TIMEOUT", "60")
env.setdefault("CAPSTONE_GUEST_COMMAND_TIMEOUT", "360")
result = run_guest(run, "sh /mnt/host/run.sh", "WM_DONE", env=env, timeout_multiplier=1)
ok = result.returncode == 0
if ok:
    report = struct.unpack("<16Q", (share / "report.bin").read_bytes())
    ok = report[0] == MAGIC and report[3] == 0 and report[1] == report[4]
