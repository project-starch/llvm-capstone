...
int main(int argc, char **argv) {
  ...
  struct wm_header *staging = malloc(WM_TRACE_BYTES);
  if (!f || !staging)
    return 2;
  size_t bytes = fread(staging, 1, WM_TRACE_BYTES, f);
  if (ferror(f) || fgetc(f) != EOF || bytes < sizeof *staging ||
      staging->magic != WM_MAGIC ||
      staging->count >
          (WM_TRACE_BYTES - sizeof *staging) / sizeof(struct wm_event) ||
      bytes != sizeof *staging + staging->count * sizeof(struct wm_event))
    return 3;
  fclose(f);
  if (capstone_init())
    return 4;
  ...
  if ((long)dom < 0)
    return 5;
  region_id_t ro = create_region(4096), rm = create_region(WM_META_BYTES);
  region_id_t rt = create_region(WM_TRACE_BYTES),
              ra = create_region(WM_PAYLOAD_BYTES);
  if ((long)ro < 0 || (long)rm < 0 || (long)rt < 0 || (long)ra < 0)
    return 6;
  struct wm_header *out = map_region(ro, 4096);
  void *input = map_region(rt, WM_TRACE_BYTES);
  ...
  if (!f || fwrite(out, sizeof *out, 1, f) != 1 || fclose(f))
    return 8;
  int ok = result == 42060 && out->magic == WM_MAGIC && !out->status &&
