...
D=capstone/ports/micropython/temporal-corpus/backport-2024
...
if [[ ! -d $TREE ]]; then
  ...
  for p in capstone/ports/micropython/patches/*.patch; do
    b=$(basename "$p")
...
DOMAIN_EXTRA_DEFS="-DMICROPY_CONFIG_ROM_LEVEL=MICROPY_CONFIG_ROM_LEVEL_CORE_FEATURES \
  ...
  bash capstone/ports/micropython/build-micropython-silicon.sh \
