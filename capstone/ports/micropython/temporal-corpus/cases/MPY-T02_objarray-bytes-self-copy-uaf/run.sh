#!/usr/bin/env bash
# MPY-T02: measured by building the fix commit's PARENT with our port, because the
...
exec bash "$(git rev-parse --show-toplevel)/capstone/ports/micropython/temporal-corpus/backport-2024/run.sh" "$@"
