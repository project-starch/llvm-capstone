...
rather than reporting a result, the same convention as `tests/fpga-repros/*/run.sh`.
    ...
    capstone/ports/micropython/<corpus>/cases/<ID>_*/run.sh
...
## MicroPython in the domain: the common recipe
    ...
    B=capstone/ports/micropython/build-micropython-silicon.sh
...
like results:
    ...
    $GCC -O2 -I capstone/caplifive-buildroot/package/modcapstone/include -I $U \
      -o $SH/mpy-resume-guest capstone/ports/micropython/tools/mpy-resume-guest.c $U/lib/libcapstone.c
...
`$CAPSTONE_TMP_ROOT/micropython/tests/<dir>/`, always with a `00_sanity.py`, and
    ...
    python3 capstone/ports/micropython/tools/run-resumable-suite.py \
      --domain $SH/<name>.dom \
