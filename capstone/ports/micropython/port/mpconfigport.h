...
// uctypes REFERENCES bytearray, so leaving the object type out of a MINIMUM ROM
// level breaks the link, not the run: moductypes.c:268/502/654 use
// mp_type_bytearray and mp_obj_new_bytearray_by_ref, and objarray.c compiles them
// out below CORE_FEATURES. That is what the two undefined symbols in pass 1 were.
// Turning uctypes off instead is not available: the note above records three
// uctypes-off images that each hung on their first domain call.
