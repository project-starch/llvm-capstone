...
/* THE CORPUS REPRODUCTIONS ARE NOT PART OF THE PORT. They live in
   bug-corpora/micropython/domain-glue/ and are pulled in only when a case defines its
   flag; the corpus driver adds the -I. With no flag set this file compiles exactly as
   it did before the split, which is checked by the image md5. */
#if defined(MPY_T07_LEXER_UAF) || defined(MPY_T16_DEINIT_AFTER_SWEEP) || \
    defined(MPY_T29_HIDDEN_ROOT) || defined(MPY_SPATIAL_OVERFLOW)
#define MPY_CORPUS_GLUE 1
#include "mpy_corpus_glue.h"
#endif
void domain_main(unsigned *res, unsigned func) {
#ifdef MPY_CORPUS_GLUE
#include "mpy_corpus_dispatch.h"   /* inside the function on purpose: it is a body */
#endif
