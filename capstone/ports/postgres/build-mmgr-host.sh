...
# the counts add up. On by default here as in the domain builds: this arm is
# the exactness gate, so it should check everything the domain arm checks.
...
mkdir -p "$OUT/obj"
for f in $FILES; do
  $CC -c -O2 -g $PG_CHECK_DATA $INC "$SRC/src/backend/utils/mmgr/$f" -o "$OUT/obj/${f%.c}.o"
done
...
$CC -O2 -g $PG_CHECK_DATA $INC -I"$HERE/tools" "$HERE/tools/replay.c" "$OUT/obj"/*.o -o "$OUT/replay"
