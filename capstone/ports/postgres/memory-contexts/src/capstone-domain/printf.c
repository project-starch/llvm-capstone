...
static void sink_uint(unsigned long v) {
  char digits[24];
  unsigned n = 0;
...
static int render(const char *fmt, va_list ap) {
  fmt_used = 0;
  for (const char *p = fmt; p && *p; p++) {
    ...
    case 'i': {
      ...
      if (v < 0) {
        sink('-');
        sink_uint((unsigned long)-(v + 1) + 1UL);
      } else
        sink_uint((unsigned long)v);
      break;
    }
    case 'u':
      sink_uint(lng ? va_arg(ap, unsigned long)
    ...
    case 'p': {
      ...
      unsigned n = 0;
      do {
        unsigned x = (unsigned)(a & 0xf);
        d[n++] = (char)(x < 10 ? '0' + x : 'a' + x - 10);
        a >>= 4;
      } while (a);
      while (n)
        sink(d[--n]);
