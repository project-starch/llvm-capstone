 ...
 *
 * The loop is src/shared/replay-engine.c, the same one the host driver runs. This
 * file is the domain's half of it: it is handed regions instead of opening a
 * file, and it writes into the payload the host prints after the domain
 * returns, because a domain has no stdout.
 *
 * The regions, in the order the host shares them:
 *
 *   0  the hostcall metadata, whose `length` is how much of the payload is
 * written 1  the payload 2  the arena: the driver's identity tables at the
 * front, the level below the rest 3  the trace, flattened, header and records
 *
 * What the measurement is about is what the manager asks of the level below,
 * and that level is src/allocators/spatial/backing-allocator.c here rather than glibc. The counters it keeps
...
/* The level below for this arm: src/allocators/spatial/backing-allocator.c, a first-fit allocator over one
...
#include "../../capstone-domain/domain-runtime.h"
...
void pg_domain_entry(unsigned *res, unsigned func) {
  ...
  /* The size classes differ from the host's, because a capability does not
     fit in an eight-byte chunk (patches/postgresql-17.0-0001-allocset-capstone-size-classes.patch), so the block
