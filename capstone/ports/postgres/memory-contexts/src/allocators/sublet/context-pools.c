 ...
 *
 * src/allocators/spatial/backing-allocator.c beside this file is the same level for the unprotected arm: a
 ...
 * senior to each sub-pool. Reset revokes the primary pool and each extra
 * pool separately. Everything else here follows from having
 * to keep that capability somewhere a revocation cannot reach.
 *
 * Context headers and allocator metadata stay outside revocable data.
 * Nothing in this file allocates: the three tables are
 ...
 * costs one more revocation at the next nonempty reset. Extra pools form
 * a list hanging off the primary pool. */
struct pg_subpool {
...
unsigned long pg_subpool_arena(void *region, unsigned long bytes) {
  ...
  if (ty != 0) /* not linear: nothing here can work */
    return ty;
  arena_cursor = capstone_cap_base(&arena);
  arena_limit = arena_cursor + bytes;
  if (arena_limit > capstone_cap_end(&arena))
    arena_limit = capstone_cap_end(&arena);
...
static int cut(capstone_cap_slot *slot, capstone_cap_slot *handle,
               unsigned long bytes) {
...
static struct pg_subpool *pool_alloc(unsigned long bytes) {
  ...
  if (capstone_cap_type(&sp->pool) == 7) {
    if (!cut(&sp->pool, &sp->handle, bytes))
      return NULL;
  } else if (capstone_cap_end(&sp->pool) - capstone_cap_base(&sp->pool) <
             bytes) {
  ...
  sp->extra = NULL;
  sp->blocks = NULL;
  sp->cursor = capstone_cap_base(&sp->pool);
  sp->limit = capstone_cap_end(&sp->pool);
  sp->carved = 0;
...
static void pool_drop_blocks(struct pg_subpool *sp) {
...
static void pool_return(struct pg_subpool *sp) {
...
static pg_block *block_from(struct pg_subpool *sp, unsigned long bytes) {
  pg_block *b;
  ...
  b->base = capstone_cap_base(&b->region);
  b->endptr = capstone_cap_end(&b->region);
  b->freeptr = b->base;
  b->prev = b->next = NULL;
  b->aset = NULL;
  b->pool = sp;
...
pg_block *pg_subpool_block_at(unsigned int i) { return &block_tab[i]; }
