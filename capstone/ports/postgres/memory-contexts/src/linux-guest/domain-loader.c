 ...
 *   2  the arena: the driver's identity tables at the front, the level below
 * the rest 3  the trace, read from the file named on the command line
 *
 ...
 *
 * The unprotected arm wants the first: src/allocators/spatial/backing-allocator.c
...
static int fail(const char *what, unsigned long value) {
  mark_u(what, value);
  capstone_cleanup();
  return 1;
}
...
int main(int argc, char **argv) {
  ...
  if ((long)r_arena < 0)
    return fail("PG: create_region(arena) failed, wanted=",
                PG_REPLAY_ARENA_SIZE);
  if ((long)r_trace < 0)
    return fail("PG: create_region(trace) failed, wanted=",
                PG_REPLAY_TRACE_SIZE);
