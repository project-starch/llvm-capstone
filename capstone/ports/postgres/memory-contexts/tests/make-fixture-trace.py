...
class Trace:
    ...
    def write(self, path, blocks):
        ...
        head = HEAD.pack(
            MAGIC, VERSION, REC.size, ENDIAN, 1, 0, 0, len(b"fixture"), 0, b"fixture"
        )
...
def main():
    ...
    if "--blocks" in sys.argv:
        blocks = tuple(
            int(x) for x in sys.argv[sys.argv.index("--blocks") + 1].split(",")
        )
    t = Trace()
    build(t)
    n = t.write(out, blocks)
    print(
        f"{out}: {n} records, {t.nctx} contexts, {t.nptr} objects, "
