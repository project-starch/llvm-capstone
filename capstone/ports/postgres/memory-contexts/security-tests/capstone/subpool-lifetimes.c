 ...
 * dies in one revocation. src/allocators/sublet/context-pools.c is the piece that has
...
static unsigned fill(pg_block *b, unsigned n, unsigned long bytes,
                     unsigned epoch, unsigned int *idx) {
  unsigned bad = 0;
...
void pg_domain_entry(unsigned *res, unsigned func) {
  if (func == CAPSTONE_DPI_REGION_SHARE) {
    ...
    case 2:
      ...
      break;
    default:
      break; /* a fourth share is ignored */
    }
    ++shares;
    return;
  }
  ...
  claim("two blocks out of one sub-pool", 2, (b1 ? 1u : 0u) + (b2 ? 1u : 0u));
  ...
  pg_subpool_reset(a);
  claim("a reset is one revocation", 1, pg_subpool_counts.revocations - before);
  ...
  claim("a node each, so nodes spent", s->split + s->mrev, s->split + s->mrev);
