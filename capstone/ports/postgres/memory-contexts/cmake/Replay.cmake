...
option(PG_DOMAIN_FAULT_RECOVERY "Enable cooperative QEMU domain fault returns (requires trap-delivery QEMU)" OFF)
if(PG_CHECK_DATA)
...
if(PORT_PLATFORM STREQUAL "native")
  ...
  function(pg_domain name)
    ...
    set(domain_data_bytes ${PG_DOMAIN_STACK_BYTES})
    if(PG_DOMAIN_FAULT_RECOVERY)
      target_compile_definitions(${name} PRIVATE CAPSTONE_DOMAIN_FAULT_RECOVERY)
      # Reserve recovery state in addition to the declared application stack.
      math(EXPR domain_data_bytes "${domain_data_bytes} + 256")
    endif()
    target_compile_definitions(${name} PRIVATE CAPSTONE_DOMREQ_DATA=${domain_data_bytes} CAPSTONE_DOMREQ_STACK=${PG_DOMAIN_STACK_BYTES})
  ...
  endforeach()
  if(BUILD_TESTING AND PG_DOMAIN_FAULT_RECOVERY)
    foreach(kind RANGE 0 5)
      pg_domain(fault-probe-${kind} security-tests/capstone/domain-fault.c)
      target_compile_definitions(fault-probe-${kind} PRIVATE PG_FAULT_KIND=${kind})
    endforeach()
  endif()
endif()
