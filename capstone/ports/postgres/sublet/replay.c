 ...
 * of the difference: the loop is shared/replay-engine.c, the same one the host
 * driver and the unprotected domain run, and the manager is the same source
 * with port/aset-sublet.patch applied. Everything the two arms report side by
 ...
 * written 1  the payload 2  the arena, LINEAR, out of which the level below
 * carves every sub-pool 3  the trace, flattened, header and records 4  the
 * scratch, for the driver's identity tables
 *
