...
struct block { struct block *next; size_t size; };
static uint8_t *arena;
static size_t capacity, used;
static struct block *available;
void ff_memory_init(void *base, size_t bytes)
{ arena = base; capacity = bytes; used = 0; available = NULL; }
size_t ff_memory_used(void) { return used; }
void *av_malloc(size_t size)
{
    if (size > capacity || size > SIZE_MAX - 63) return NULL;
    size = (size + 63) & ~(size_t)63;
    struct block **link = &available;
    while (*link) {
        struct block *b = *link;
        if (b->size == size) { *link = b->next; return (uint8_t *)b + 64; }
        link = &b->next;
    }
    if (capacity - used < 64 || size > capacity - used - 64) return NULL;
...
{ void *p = av_malloc(size); if (p) memset(p, 0, size); return p; }
void av_freep(void *slot)
{ void **p = slot; av_free(*p); *p = NULL; }
void *av_realloc(void *p, size_t size)
{
    if (!size) { av_free(p); return NULL; }
