...
def summary(head, events):
    result = dict(events=len(events), status=head[2], metadata_used=head[3], payload_used=head[4], mode=head[11])
    result['primitives'] = dict(zip(['split', 'mrev', 'delin', 'revoke', 'init', 'init_bytes'], head[5:11]))
    for kind, name in [(1, 'buffer'), (2, 'refstruct')]:
        rows = [e for e in events if e[1] == kind]
        gets = [e for e in rows if e[0] == 130]
        result[name] = dict(pools=sum(e[0] == 1 for e in rows), allocations=len(gets),
                            backings=sum(e[0] == 5 for e in rows), reuses=sum(e[10] > 0 for e in gets),
...
if __name__ == '__main__':
    ...
    if action == 'commands':
        with open(args[1], 'wb') as f:
            ...
            for e in rows: f.write(ROW.pack(*e[:8], *([0] * 8)))
    elif action == 'summary':
        ...
        for i, (a, b) in enumerate(zip(rows, other)):
            if a != b:
                raise SystemExit(f'event {i}: ' + str({FIELDS[j]: [a[j], b[j]] for j in range(16) if a[j] != b[j]}))
        print(json.dumps(dict(exact=True, events=len(rows), reference=str(args[0]), replay=str(args[1]))))
    else:
