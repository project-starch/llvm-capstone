...
int main(int argc, char **argv) {
  ...
  struct mcp_header *staging = malloc(MCP_FILE_BYTES);
  if (!f || !staging)
    return 2;
  size_t bytes = fread(staging, 1, MCP_FILE_BYTES, f);
  if (ferror(f) || fgetc(f) != EOF || bytes < sizeof *staging ||
      staging->magic != MCP_MAGIC ||
      staging->count >
          (MCP_FILE_BYTES - sizeof *staging) / sizeof(struct mcp_event) ||
      bytes != sizeof *staging + staging->count * sizeof(struct mcp_event))
    return 3;
  fclose(f);
  if (capstone_init())
    return 4;
  ...
  if ((long)dom < 0)
    return 5;
  region_id_t ro = create_region(4096), rm = create_region(MCP_META_BYTES);
  region_id_t rt = create_region(MCP_FILE_BYTES),
              ra = create_region(MCP_PAYLOAD_BYTES);
  if ((long)ro < 0 || (long)rm < 0 || (long)rt < 0 || (long)ra < 0)
    return 6;
  struct mcp_header *out = map_region(ro, 4096);
  void *input = map_region(rt, MCP_FILE_BYTES);
  ...
  printf("MCP return=%lu status=%llu completed=%llu pages=%llu "
         "chunk_reuses=%llu chunk_releases=%llu object_reuses=%llu\n",
  ...
  if (!f || fwrite(out, sizeof *out, 1, f) != 1 || fclose(f))
    return 8;
  int ok = result == 42047 && out->magic == MCP_MAGIC && !out->status &&
