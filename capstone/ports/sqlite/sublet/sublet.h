 ...
 * UNINIT if a linear child did. init turns the latter into LIN. On the emulator
 * that is one instruction: its revoke leaves the cursor at the end, which is
 * what its init asks for (op_helper.c helper_csrevoke, an experimental choice
 * it says so about). The specification leaves the cursor at the base and lets
 * init succeed only once the region has been written through, so there a
 * merge and a pool's destruction cost a write of the block. The counts here
 * are the same either way; the cycles are not, and are the board's to say.
 *
 * Encodings: opcode 0x5b, funct3 1, funct7 revoke 0 lcc 4 split 6 mrev 8 init 9
 * delin 3; ldc funct3 3, stc funct3 4 (capstone-qemu insn32.decode).
 */
#ifndef SUBLET_H
#define SUBLET_H
