#!/usr/bin/env bash
# Boot Capstone QEMU once and run the file-probe domain.
#
...
LOG_FILE=${LOG_FILE:-$CAPSTONE_TMP_ROOT/capstone-file-probe.log}
...
if ! CAPSTONE_QEMU_LOGIN_TIMEOUT=${CAPSTONE_QEMU_LOGIN_TIMEOUT:-300} \
  ...
  --guest-command \
    'echo __CAPSTONE_QEMU_BOOT_CONTROL_OK__; cp /mnt/host/file_probe.user /tmp/h && chmod 0755 /tmp/h && /tmp/h /mnt/host/file_probe.dom' \
...
then
  echo "run-file-probe.sh FAILED" >&2
  exit 1
fi
