...
OBJ_DIR="$OUT_DIR/obj"
OUT_DOM=${OUT_DOM:-$OUT_DIR/musl_hello.dom}
OUT_HOST=${OUT_HOST:-$OUT_DIR/musl_hello.user}
...
"$CLANG" "${MUSL_CFLAGS[@]}" -c "$SCRIPT_DIR/musl_hello.c" -o "$OBJ_DIR/musl_hello.o"
...
"$LD_LLD" --gc-sections -T "$LINKER_SCRIPT" -o "$OUT_DOM" \
  "$OBJ_DIR/start-musl.o" "$OBJ_DIR/hostcall.o" "$OBJ_DIR/musl_hello.o" \
