//===-- CapstoneTargetParser.cpp - Parser for target features ------*- C++ -*-===//
//
...
constexpr CPUInfo CapstoneCPUInfo[] = {
#define PROC(ENUM, NAME, DEFAULT_MARCH, FAST_SCALAR_UNALIGN,                   \
             FAST_VECTOR_UNALIGN, MVENDORID, MARCHID, MIMPID)                  \
  {                                                                            \
      NAME,                                                                    \
...
void fillValidCPUArchList(SmallVectorImpl<StringRef> &Values, bool IsRV64) {
  for (const auto &C : CapstoneCPUInfo) {
    if (IsRV64 == C.is64Bit())
...
void getFeaturesForCPU(StringRef CPU,
                       ...
                       bool NeedPlus) {
  StringRef MarchFromCPU = llvm::Capstone::getMArchFromMcpu(CPU);
  if (MarchFromCPU == "")
    return;

  EnabledFeatures.clear();
  auto RII = CapstoneISAInfo::parseArchString(
...
void printVType(unsigned VType, raw_ostream &OS) {
  ...
  bool AltFmt = CapstoneVType::isAltFmt(VType);
  if (AltFmt)
    OS << "alt";
...
std::optional<VLMUL> getSameRatioLMUL(unsigned SEW, VLMUL VLMul, unsigned EEW) {
  unsigned Ratio = CapstoneVType::getSEWLMULRatio(SEW, VLMul);
  ...
  if (!isValidLMUL(EMUL, Fractional))
    return std::nullopt;
  return CapstoneVType::encodeLMUL(EMUL, Fractional);
}
