//===-- CapstoneISAInfo.cpp - Capstone Arch String Parser ----------------------===//
//
...
struct CapstoneSupportedExtension {
  const char *Name;
  /// Supported version.
  CapstoneISAUtils::ExtensionVersion Version;

  bool operator<(const CapstoneSupportedExtension &RHS) const {
    return StringRef(Name) < StringRef(RHS.Name);
  }
};

struct CapstoneProfile {
  StringLiteral Name;
  StringLiteral MArch;

  bool operator<(const CapstoneProfile &RHS) const {
    return StringRef(Name) < StringRef(RHS.Name);
  }
};
...
static const char *CapstoneGImplications[] = {"i", "m", "a", "f", "d"};
static const char *CapstoneGImplicationsZi[] = {"zicsr", "zifencei"};

#define GET_SUPPORTED_EXTENSIONS
#include "llvm/TargetParser/CapstoneTargetParserDef.inc"
...
void CapstoneISAInfo::printSupportedExtensions(StringMap<StringRef> &DescMap) {
  outs() << "All available -march extensions for Capstone\n\n";
  PrintExtension("Name", "Version", (DescMap.empty() ? "" : "Description"));

  CapstoneISAUtils::OrderedExtensionMap ExtMap;
...
void CapstoneISAInfo::printEnabledExtensions(
    bool IsRV64, std::set<StringRef> &EnabledFeatureNames,
    StringMap<StringRef> &DescMap) {
  outs() << "Extensions enabled for the given Capstone target\n\n";
  PrintExtension("Name", "Version", (DescMap.empty() ? "" : "Description"));

  CapstoneISAUtils::OrderedExtensionMap FullExtMap;
  CapstoneISAUtils::OrderedExtensionMap ExtMap;
  for (const auto &E : SupportedExtensions)
  ...
  if (auto ISAString = CapstoneISAInfo::createFromExtMap(XLen, FullExtMap))
...
namespace {
struct LessExtName {
  bool operator()(const CapstoneSupportedExtension &LHS, StringRef RHS) {
    return StringRef(LHS.Name) < RHS;
  }
  bool operator()(StringRef LHS, const CapstoneSupportedExtension &RHS) {
    return LHS < StringRef(RHS.Name);
  }
};
} // namespace

static std::optional<CapstoneISAUtils::ExtensionVersion>
...
bool CapstoneISAInfo::isSupportedExtensionFeature(StringRef Ext) {
  bool IsExperimental = stripExperimentalPrefix(Ext);

  ArrayRef<CapstoneSupportedExtension> ExtInfo =
...
bool CapstoneISAInfo::isSupportedExtension(StringRef Ext) {
  verifyTables();
...
bool CapstoneISAInfo::isSupportedExtension(StringRef Ext, unsigned MajorVersion,
...
bool CapstoneISAInfo::hasExtension(StringRef Ext) const {
...
std::vector<std::string> CapstoneISAInfo::toFeatures(bool AddAllExtensions,
  ...
  if (AddAllExtensions) {
    for (const CapstoneSupportedExtension &Ext : SupportedExtensions) {
      if (Exts.count(Ext.Name))
        continue;
      Features.push_back((llvm::Twine("-") + Ext.Name).str());
    }

    for (const CapstoneSupportedExtension &Ext : SupportedExperimentalExtensions) {
      if (Exts.count(Ext.Name))
        continue;
...
static Error getExtensionVersion(StringRef Ext, StringRef In, unsigned &Major,
  ...
  if (CapstoneISAInfo::isSupportedExtension(Ext, Major, Minor))
    return Error::success();

  if (!CapstoneISAInfo::isSupportedExtension(Ext))
...
CapstoneISAInfo::createFromExtMap(unsigned XLen,
                               const CapstoneISAUtils::OrderedExtensionMap &Exts) {
  assert(XLen == 32 || XLen == 64);
  std::unique_ptr<CapstoneISAInfo> ISAInfo(new CapstoneISAInfo(XLen));

  ISAInfo->Exts = Exts;

  return CapstoneISAInfo::postProcessAndChecking(std::move(ISAInfo));
}

llvm::Expected<std::unique_ptr<CapstoneISAInfo>>
CapstoneISAInfo::parseFeatures(unsigned XLen,
...
llvm::Expected<std::unique_ptr<CapstoneISAInfo>>
CapstoneISAInfo::parseNormalizedArchString(StringRef Arch) {
  // Capstone ISA strings must be [a-z0-9_]
  if (!llvm::all_of(
  ...
  while (!Arch.empty()) {
    ...
    if (!ISAInfo->Exts
             .emplace(
                 ExtName.str(),
                 CapstoneISAUtils::ExtensionVersion{MajorVersion, MinorVersion})
             .second)
...
llvm::Expected<std::unique_ptr<CapstoneISAInfo>>
CapstoneISAInfo::parseArchString(StringRef Arch, bool EnableExperimentalExtension,
                              bool ExperimentalExtensionVersionCheck) {
  // Capstone ISA strings must be [a-z0-9_]
  if (!llvm::all_of(
          Arch, [](char C) { return isDigit(C) || isLower(C) || C == '_'; }))
  ...
  if (Arch.consume_front("rv32")) {
    ...
    auto ProfileCmp = [](StringRef Arch, const CapstoneProfile &Profile) {
  ...
  // Ref: Table 22.1 in Capstone User-Level ISA V2.2
  ...
  case 'i':
    ...
    break;
  case 'g':
    // g expands to extensions in CapstoneGImplications.
    ...
    // ISA spec.
    for (const char *Ext : CapstoneGImplications) {
  ...
  while (!Arch.empty()) {
    ...
    do {
      ...
      if (CapstoneISAUtils::AllStdExts.contains(Ext.front())) {
      ...
      if (!CapstoneISAInfo::isSupportedExtension(Name))
        return getErrorForInvalidExt(Name);

      // Insert and error for duplicates.
      if (!ISAInfo->Exts
               .emplace(Name.str(),
                        CapstoneISAUtils::ExtensionVersion{Major, Minor})
               .second)
  ...
  if (Baseline == 'g') {
    for (const char *Ext : CapstoneGImplicationsZi) {
...
Error CapstoneISAInfo::checkDependency() {
...
void CapstoneISAInfo::updateImplication() {
...
void CapstoneISAInfo::updateCombination() {
...
void CapstoneISAInfo::updateImpliedLengths() {
...
std::string CapstoneISAInfo::toString() const {
  std::string Buffer;
...
llvm::Expected<std::unique_ptr<CapstoneISAInfo>>
CapstoneISAInfo::postProcessAndChecking(std::unique_ptr<CapstoneISAInfo> &&ISAInfo) {
...
StringRef CapstoneISAInfo::computeDefaultABI() const {
  if (XLen == 32) {
    if (Exts.count("e"))
      return "ilp32e";
    if (Exts.count("d"))
...
bool CapstoneISAInfo::isSupportedExtensionWithVersion(StringRef Ext) {
  if (Ext.empty())
    return false;
...
std::string CapstoneISAInfo::getTargetFeatureForExtension(StringRef Ext) {
  if (Ext.empty())
    return std::string();
...
struct CapstoneExtBit {
  const StringLiteral ext;
  uint8_t groupid;
  uint8_t bitpos;
};

struct CapstoneExtensionBitmask {
  const char *Name;
  unsigned GroupID;
  unsigned BitPosition;
};

#define GET_CapstoneExtensionBitmaskTable_IMPL
#include "llvm/TargetParser/CapstoneTargetParserDef.inc"

std::pair<int, int> CapstoneISAInfo::getCapstoneFeaturesBitsInfo(StringRef Ext) {
  ...
  // there's only one extension version in the __capstone_feature_bits vector.
