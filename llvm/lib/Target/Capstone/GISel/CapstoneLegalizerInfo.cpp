...
//===----------------------------------------------------------------------===//
/// \file
/// This file implements the targeting of the Machinelegalizer class for Capstone.
/// \todo This should be generated by TableGen.
//===----------------------------------------------------------------------===//

#include "CapstoneLegalizerInfo.h"
...
CapstoneLegalizerInfo::CapstoneLegalizerInfo(const CapstoneSubtarget &ST)
...
bool CapstoneLegalizerInfo::legalizeIntrinsic(LegalizerHelper &Helper,
                                           MachineInstr &MI) const {
  Intrinsic::ID IntrinsicID = cast<GIntrinsic>(MI).getIntrinsicID();

  if (CapstoneVIntrinsicsTable::getCapstoneVIntrinsicInfo(IntrinsicID))
    return true;
  ...
  case Intrinsic::capstone_masked_atomicrmw_add:
  case Intrinsic::capstone_masked_atomicrmw_sub:
  case Intrinsic::capstone_masked_cmpxchg:
    return true;
  }
}

bool CapstoneLegalizerInfo::legalizeVAStart(MachineInstr &MI,
...
bool CapstoneLegalizerInfo::legalizeBRJT(MachineInstr &MI,
                                      MachineIRBuilder &MIRBuilder) const {
  MachineRegisterInfo &MRI = *MIRBuilder.getMRI();
...
bool CapstoneLegalizerInfo::shouldBeInConstantPool(const APInt &APImm,
  ...
  CapstoneMatInt::InstSeq Seq = CapstoneMatInt::generateInstSeq(Imm, STI);
  ...
  CapstoneMatInt::InstSeq SeqLo =
      CapstoneMatInt::generateTwoRegInstSeq(Imm, STI, ShiftAmt, AddOpc);
  return !(!SeqLo.empty() && (SeqLo.size() + 2) <= STI.getMaxBuildIntsCost());
}

bool CapstoneLegalizerInfo::legalizeVScale(MachineInstr &MI,
  ...
  // vscale as VLENB / 8.
  static_assert(Capstone::RVVBitsPerBlock == 64, "Unexpected bits per block!");
  if (STI.getRealMinVLen() < Capstone::RVVBitsPerBlock)
...
bool CapstoneLegalizerInfo::legalizeExt(MachineInstr &MI,
                                     MachineIRBuilder &MIB) const {
...
bool CapstoneLegalizerInfo::legalizeLoadStore(MachineInstr &MI,
...
static MachineInstrBuilder buildAllOnesMask(LLT VecTy, const SrcOp &VL,
  ...
  return MIB.buildInstr(Capstone::G_VMSET_VL, {MaskTy}, {VL});
}
...
buildDefaultVLOps(LLT VecTy, MachineIRBuilder &MIB, MachineRegisterInfo &MRI) {
  ...
  const CapstoneSubtarget &STI = MIB.getMF().getSubtarget<CapstoneSubtarget>();
...
buildSplatPartsS64WithVL(const DstOp &Dst, const SrcOp &Passthru, Register Lo,
  ...
  return MIB.buildInstr(Capstone::G_SPLAT_VECTOR_SPLIT_I64_VL, {Dst},
                        {Passthru, Lo, Hi, VL});
}
...
// VMCLR_VL.
bool CapstoneLegalizerInfo::legalizeSplatVector(MachineInstr &MI,
                                             MachineIRBuilder &MIB) const {
  ...
  if (isAllOnesOrAllOnesSplat(SplatValMI, MRI)) {
    ...
    MIB.buildInstr(Capstone::G_VMSET_VL, {Dst}, {VL});
    MI.eraseFromParent();
    return true;
  }
  if (isNullOrNullSplat(SplatValMI, MRI)) {
    auto VL = buildDefaultVLOps(VecTy, MIB, MRI).second;
    MIB.buildInstr(Capstone::G_VMCLR_VL, {Dst}, {VL});
    MI.eraseFromParent();
    return true;
  }
...
static LLT getLMUL1Ty(LLT VecTy) {
  ...
  return LLT::scalable_vector(Capstone::RVVBitsPerBlock /
                                  VecTy.getElementType().getSizeInBits(),
                              VecTy.getElementType());
}

bool CapstoneLegalizerInfo::legalizeExtractSubvector(MachineInstr &MI,
                                                  MachineIRBuilder &MIB) const {
  ...
  // divide exactly.
  assert(
      CapstoneVType::decodeVLMUL(CapstoneTargetLowering::getLMUL(LitTyMVT)).second ||
      CapstoneTargetLowering::getLMUL(LitTyMVT) == CapstoneVType::LMUL_1);
  ...
  if (TypeSize::isKnownGT(BigTy.getSizeInBits(),
    ...
    assert(Decompose.first != Capstone::NoSubRegister);
  ...
  uint64_t Policy = CapstoneVType::TAIL_AGNOSTIC | CapstoneVType::MASK_AGNOSTIC;
  auto Slidedown = MIB.buildInstr(
      Capstone::G_VSLIDEDOWN_VL, {InterLitTy},
...
bool CapstoneLegalizerInfo::legalizeInsertSubvector(MachineInstr &MI,
                                                 LegalizerHelper &Helper,
                                                 MachineIRBuilder &MIB) const {
  ...
  if (RemIdx == 0) {
    Inserted = MIB.buildInstr(Capstone::G_VMV_V_V_VL, {InsertedDst},
    ...
    Inserted =
        MIB.buildInstr(Capstone::G_VSLIDEUP_VL, {InsertedDst},
...
static unsigned getCapstoneWOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
    llvm_unreachable("Unexpected opcode");
  case TargetOpcode::G_ASHR:
    return Capstone::G_SRAW;
  case TargetOpcode::G_LSHR:
    return Capstone::G_SRLW;
  case TargetOpcode::G_SHL:
    return Capstone::G_SLLW;
  case TargetOpcode::G_SDIV:
    return Capstone::G_DIVW;
  case TargetOpcode::G_UDIV:
    return Capstone::G_DIVUW;
  case TargetOpcode::G_UREM:
    return Capstone::G_REMUW;
  case TargetOpcode::G_ROTL:
    return Capstone::G_ROLW;
  case TargetOpcode::G_ROTR:
    return Capstone::G_RORW;
  case TargetOpcode::G_CTLZ:
    return Capstone::G_CLZW;
  case TargetOpcode::G_CTTZ:
    return Capstone::G_CTZW;
  case TargetOpcode::G_FPTOSI:
    return Capstone::G_FCVT_W_RV64;
  case TargetOpcode::G_FPTOUI:
    return Capstone::G_FCVT_WU_RV64;
  }
}

bool CapstoneLegalizerInfo::legalizeCustom(
  ...
  case TargetOpcode::G_SHL: {
    ...
    Helper.widenScalarSrc(MI, sXLen, 2, TargetOpcode::G_ANYEXT);
    Helper.widenScalarDst(MI, sXLen);
    MI.setDesc(MIRBuilder.getTII().get(getCapstoneWOpcode(MI.getOpcode())));
    Helper.Observer.changedInstr(MI);
    return true;
  }
  case TargetOpcode::G_SDIV:
  ...
  case TargetOpcode::G_FPTOUI: {
    ...
    Helper.widenScalarDst(MI, sXLen);
    MI.setDesc(MIRBuilder.getTII().get(getCapstoneWOpcode(MI.getOpcode())));
    MI.addOperand(MachineOperand::CreateImm(CapstoneFPRndMode::RTZ));
    Helper.Observer.changedInstr(MI);
    return true;
  }
  case TargetOpcode::G_IS_FPCLASS: {
    ...
    // Turn LLVM IR's floating point classes to that in Capstone,
    ...
    auto GFClass = MIB.buildInstr(Capstone::G_FCLASS, {sXLen}, {Src});
