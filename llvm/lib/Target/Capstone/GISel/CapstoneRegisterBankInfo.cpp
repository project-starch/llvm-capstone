//===-- CapstoneRegisterBankInfo.cpp -------------------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
/// \file
/// This file implements the targeting of the RegisterBankInfo class for Capstone.
...
#include "CapstoneGenRegisterBank.inc"
...
CapstoneRegisterBankInfo::CapstoneRegisterBankInfo(unsigned HwMode)
    : CapstoneGenRegisterBankInfo(HwMode) {}
...
static const RegisterBankInfo::ValueMapping *getFPValueMapping(unsigned Size) {
  ...
  case 16:
    Idx = Capstone::FPRB16Idx;
    break;
  case 32:
    Idx = Capstone::FPRB32Idx;
    break;
  case 64:
    Idx = Capstone::FPRB64Idx;
    break;
  }
  return &Capstone::ValueMappings[Idx];
}

// TODO: Make this more like AArch64?
bool CapstoneRegisterBankInfo::hasFPConstraints(
  ...
  return getRegBank(MI.getOperand(0).getReg(), MRI, TRI) == &Capstone::FPRBRegBank;
}

bool CapstoneRegisterBankInfo::onlyUsesFP(const MachineInstr &MI,
                                       const MachineRegisterInfo &MRI,
                                       const TargetRegisterInfo &TRI) const {
  switch (MI.getOpcode()) {
  case Capstone::G_FCVT_W_RV64:
  case Capstone::G_FCVT_WU_RV64:
  case Capstone::G_FCLASS:
...
bool CapstoneRegisterBankInfo::onlyDefinesFP(const MachineInstr &MI,
                                          const MachineRegisterInfo &MRI,
                                          const TargetRegisterInfo &TRI) const {
  switch (MI.getOpcode()) {
...
bool CapstoneRegisterBankInfo::anyUseOnlyUseFP(
...
static const RegisterBankInfo::ValueMapping *getVRBValueMapping(unsigned Size) {
  ...
  if (Size <= 64)
    Idx = Capstone::VRB64Idx;
  else if (Size == 128)
    Idx = Capstone::VRB128Idx;
  else if (Size == 256)
    Idx = Capstone::VRB256Idx;
  else if (Size == 512)
    Idx = Capstone::VRB512Idx;
  else
...
CapstoneRegisterBankInfo::getInstrMapping(const MachineInstr &MI) const {
  ...
  unsigned GPRSize = getMaximumSize(Capstone::GPRBRegBankID);
  assert((GPRSize == 32 || GPRSize == 64) && "Unexpected GPR size");

  unsigned NumOperands = MI.getNumOperands();
  const ValueMapping *GPRValueMapping =
      &Capstone::ValueMappings[GPRSize == 64 ? Capstone::GPRB64Idx
                                          : Capstone::GPRB32Idx];
  ...
  case TargetOpcode::G_LOAD: {
    ...
    if (GPRSize == 32 && Size.getFixedValue() == 64) {
      assert(MF.getSubtarget<CapstoneSubtarget>().hasStdExtD());
  ...
  case TargetOpcode::G_SELECT: {
    ...
    if (GPRSize == 32 && Ty.getSizeInBits() == 64) {
      ...
      for (unsigned Idx = 2; Idx < 4; ++Idx) {
        ...
        if (getRegBank(VReg, MRI, TRI) == &Capstone::FPRBRegBank ||
