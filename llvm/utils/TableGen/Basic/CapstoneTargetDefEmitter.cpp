//===- CapstoneTargetDefEmitter.cpp - Generate lists of Capstone CPUs ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This tablegen backend emits the include file needed by CapstoneTargetParser.cpp
// and CapstoneISAInfo.cpp to parse the Capstone CPUs and extensions.
//
//===----------------------------------------------------------------------===//
...
static void printExtensionTable(raw_ostream &OS,
                                ...
                                bool Experimental) {
  OS << "static const CapstoneSupportedExtension Supported";
  if (Experimental)
    OS << "Experimental";
  OS << "Extensions[] = {\n";
...
static void emitCapstoneExtensions(const RecordKeeper &Records, raw_ostream &OS) {
  OS << "#ifdef GET_SUPPORTED_EXTENSIONS\n";
  OS << "#undef GET_SUPPORTED_EXTENSIONS\n\n";

  std::vector<const Record *> Extensions =
      Records.getAllDerivedDefinitionsIfDefined("CapstoneExtension");
  ...
  if (!Extensions.empty()) {
    ...
    for (const Record *Ext : Extensions) {
      ...
      for (auto *ImpliedExt : ImpliesList) {
        if (!ImpliedExt->isSubClassOf("CapstoneExtension"))
          continue;
...
// in Capstone ISA specification (version 20191213) 'Chapter 27. ISA Extension
// Naming Conventions'.
//
// This is almost the same as CapstoneFeatures::parseFeatureBits, except that we
// get feature name from feature records instead of feature bits.
static void printMArch(raw_ostream &OS, ArrayRef<const Record *> Features) {
  CapstoneISAUtils::OrderedExtensionMap Extensions;
  unsigned XLen = 0;

  // Convert features to FeatureVector.
  for (const Record *Feature : Features) {
    StringRef FeatureName = getExtensionName(Feature);
    if (Feature->isSubClassOf("CapstoneExtension")) {
...
static void printProfileTable(raw_ostream &OS,
                              ...
                              bool Experimental) {
  OS << "static constexpr CapstoneProfile Supported";
  if (Experimental)
    OS << "Experimental";
  OS << "Profiles[] = {\n";
...
static void emitCapstoneProfiles(const RecordKeeper &Records, raw_ostream &OS) {
  OS << "#ifdef GET_SUPPORTED_PROFILES\n";
  OS << "#undef GET_SUPPORTED_PROFILES\n\n";

  auto Profiles = Records.getAllDerivedDefinitionsIfDefined("CapstoneProfile");
...
static void emitCapstoneProcs(const RecordKeeper &RK, raw_ostream &OS) {
  OS << "#ifndef PROC\n"
  ...
  for (const Record *Rec :
       RK.getAllDerivedDefinitionsIfDefined("CapstoneProcessorModel")) {
  ...
  for (const Record *Rec :
       RK.getAllDerivedDefinitionsIfDefined("CapstoneTuneProcessorModel")) {
...
static void emitCapstoneExtensionBitmask(const RecordKeeper &RK, raw_ostream &OS) {
  std::vector<const Record *> Extensions =
      RK.getAllDerivedDefinitionsIfDefined("CapstoneExtensionBitmask");
  llvm::sort(Extensions, [](const Record *Rec1, const Record *Rec2) {
  ...
  OS << "#ifdef GET_CapstoneExtensionBitmaskTable_IMPL\n";
  OS << "static const CapstoneExtensionBitmask ExtensionBitmask[]={\n";
...
static void emitCapstoneTargetDef(const RecordKeeper &RK, raw_ostream &OS) {
  emitCapstoneExtensions(RK, OS);
  emitCapstoneProfiles(RK, OS);
  emitCapstoneProcs(RK, OS);
  emitCapstoneExtensionBitmask(RK, OS);
}

static TableGen::Emitter::Opt X("gen-capstone-target-def", emitCapstoneTargetDef,
