//===-- CapstoneVEmitter.cpp - Generate capstone_vector.h for use with clang ----===//
//
...
//===----------------------------------------------------------------------===//
//
// This tablegen backend is responsible for emitting capstone_vector.h which
// includes a declaration and definition of each intrinsic functions specified
// in https://github.com/capstone/rvv-intrinsic-doc.
//
// See also the documentation in include/clang/Basic/capstone_vector.td.
//
//===----------------------------------------------------------------------===//

#include "clang/Support/CapstoneVIntrinsicUtils.h"
...
using namespace clang::Capstone;
...
public:
  ...
  /// Emit all the information needed by SemaCapstoneVectorLookup.cpp.
...
void emitCodeGenSwitchBody(const RVVIntrinsic *RVVI, raw_ostream &OS) {
  if (!RVVI->getIRName().empty())
    OS << "  ID = Intrinsic::capstone_" + RVVI->getIRName() + ";\n";
...
void RVVEmitter::createHeader(raw_ostream &OS) {
  ...
  OS << "/*===---- capstone_vector.h - Capstone V-extension RVVIntrinsics "
        "-------------------===\n"
        " *\n"
        " *\n"
  ...
  OS << "#ifndef __Capstone_VECTOR_H\n";
  OS << "#define __Capstone_VECTOR_H\n\n";
  ...
  OS << "#pragma clang capstone intrinsic vector\n\n";
...
void RVVEmitter::createBuiltins(raw_ostream &OS) {
  ...
  OS << "// Capstone Vector builtin enumerators\n";
  OS << "#ifdef GET_CapstoneV_BUILTIN_ENUMERATORS\n";
  for (RVVIntrinsic *Def : UniqueDefs)
    OS << "  BI__builtin_rvv_" << Def->getBuiltinName() << ",\n";
  OS << "#endif // GET_CapstoneV_BUILTIN_ENUMERATORS\n\n";

  // Emit the string table for the RVV builtins.
  OS << "// Capstone Vector builtin enumerators\n";
  OS << "#ifdef GET_CapstoneV_BUILTIN_STR_TABLE\n";
  Table.EmitStringTableDef(OS, "BuiltinStrings");
  OS << "#endif // GET_CapstoneV_BUILTIN_STR_TABLE\n\n";

  // Emit the info structs of RVV builtins. Note that these are emitted without
  // any outer context to enable concatenating them.
  OS << "// Capstone Vector builtin infos\n";
  OS << "#ifdef GET_CapstoneV_BUILTIN_INFOS\n";
  for (RVVIntrinsic *Def : UniqueDefs) {
  ...
  OS << "#endif // GET_CapstoneV_BUILTIN_INFOS\n\n";
}
...
void RVVEmitter::createCodeGen(raw_ostream &OS) {
  ...
  for (auto &Def : Defs) {
    ...
    if (P.second) {
      OS << "case CapstoneVector::BI__builtin_rvv_" << Def->getBuiltinName()
         << ":\n";
      continue;
    }
...
void RVVEmitter::createRVVIntrinsics(
  ...
  for (const Record *R : Records.getAllDerivedDefinitions("RVVBuiltin")) {
    ...
    // They are written in capstone_vector.td and will emit those marco define in
    // capstone_vector.h
...
void RVVEmitter::createSema(raw_ostream &OS) {
  ...
  // Emit signature table for SemaCapstoneVectorLookup.cpp.
  OS << "#ifdef DECL_SIGNATURE_TABLE\n";
  SST.print(OS);
  OS << "#endif\n";

  // Emit RVVIntrinsicRecords for SemaCapstoneVectorLookup.cpp.
...
namespace clang {
void EmitCapstoneRVVHeader(const RecordKeeper &Records, raw_ostream &OS) {
  RVVEmitter(Records).createHeader(OS);
}

void EmitCapstoneRVVBuiltins(const RecordKeeper &Records, raw_ostream &OS) {
  RVVEmitter(Records).createBuiltins(OS);
}

void EmitCapstoneRVVBuiltinCG(const RecordKeeper &Records, raw_ostream &OS) {
  RVVEmitter(Records).createCodeGen(OS);
}

void EmitCapstoneRVVBuiltinSema(const RecordKeeper &Records, raw_ostream &OS) {
