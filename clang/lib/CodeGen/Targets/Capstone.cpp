...
public:
  CapstoneABIInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen, unsigned FLen,
               bool EABI)
...
void CapstoneABIInfo::appendAttributeMangling(TargetClonesAttr *Attr,
                                           unsigned Index,
                                           raw_ostream &Out) const {
  appendAttributeMangling(Attr->getFeatureStr(Index), Out);
}

void CapstoneABIInfo::appendAttributeMangling(StringRef AttrStr,
                                           raw_ostream &Out) const {
  if (AttrStr == "default") {
    Out << ".default";
    return;
...
void CapstoneABIInfo::computeInfo(CGFunctionInfo &FI) const {
  unsigned ABIVLen;
...
#define CC_VLS_CASE(ABI_VLEN)                                                  \
  case CallingConv::CC_CapstoneVLSCall_##ABI_VLEN:                                \
    ABIVLen = ABI_VLEN;                                                        \
    break;
    CC_VLS_CASE(32)
    CC_VLS_CASE(64)
...
// field is a float.
bool CapstoneABIInfo::detectFPCCEligibleStructHelper(QualType Ty, CharUnits CurOff,
                                                  llvm::Type *&Field1Ty,
                                                  CharUnits &Field1Off,
                                                  llvm::Type *&Field2Ty,
                                                  CharUnits &Field2Off) const {
...
bool CapstoneABIInfo::detectFPCCEligibleStruct(QualType Ty, llvm::Type *&Field1Ty,
                                            CharUnits &Field1Off,
                                            llvm::Type *&Field2Ty,
                                            CharUnits &Field2Off,
                                            int &NeededArgGPRs,
...
ABIArgInfo CapstoneABIInfo::coerceAndExpandFPCCEligibleStruct(
...
bool CapstoneABIInfo::detectVLSCCEligibleStruct(QualType Ty, unsigned ABIVLen,
                                             llvm::Type *&VLSType) const {
  // No capstone_vls_cc attribute.
  if (ABIVLen == 0)
    return false;
  ...
  VLSType = llvm::TargetExtType::get(
      getVMContext(), "capstone.vector.tuple",
...
ABIArgInfo CapstoneABIInfo::coerceVLSVector(QualType Ty, unsigned ABIVLen) const {
  ...
  if (ABIVLen == 0) {
    ...
    ResType = llvm::ScalableVectorType::get(
        EltType,
        llvm::divideCeil(NumElts * llvm::Capstone::RVVBitsPerBlock, ABIVLen));
...
ABIArgInfo CapstoneABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
                                              int &ArgGPRsLeft,
                                              int &ArgFPRsLeft,
  ...
  // ABI is also not merged yet in Capstone:
  // https://github.com/capstone-non-isa/capstone-elf-psabi-doc/pull/419
  if (const VectorType *VT = Ty->getAs<VectorType>();
    ...
    if (VT->getVectorKind() == VectorKind::Generic && ABIVLen != 0)
      // Generic vector without capstone_vls_cc should fall through and pass by
      // reference.
...
ABIArgInfo CapstoneABIInfo::classifyReturnType(QualType RetTy,
                                            unsigned ABIVLen) const {
  if (RetTy->isVoidType())
    return ABIArgInfo::getIgnore();
...
RValue CapstoneABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
...
ABIArgInfo CapstoneABIInfo::extendType(QualType Ty, llvm::Type *CoerceTy) const {
...
llvm::Value *CapstoneABIInfo::createCoercedLoad(Address Src, const ABIArgInfo &AI,
  ...
  if (llvm::TargetExtType *TupTy = dyn_cast<llvm::TargetExtType>(Ty)) {
    // In Capstone VLS calling convention, struct of fixed vectors or struct of
    ...
    // };
    // is lowered to target("capstone.vector.tuple", <vscale x 8 x i8>, 2)
    // when ABI_VLEN = 128 bits, please checkout
    // clang/test/CodeGen/Capstone/capstone-vector-callingconv-llvm-ir.c
    // for more information.
    assert(TupTy->getName() == "capstone.vector.tuple");
    ...
    for (unsigned i = 0; i < NumElts; ++i) {
      ...
      TupleVal =
          CGF.Builder.CreateIntrinsic(llvm::Intrinsic::capstone_tuple_insert,
                                      {Ty, EltTy}, {TupleVal, VectorVal, Idx});
    }
    return TupleVal;
  }

  // In Capstone VLS calling convention, struct of fixed vector or struct of
...
void CapstoneABIInfo::createCoercedStore(llvm::Value *Val, Address Dst,
                                      const ABIArgInfo &AI, bool DestIsVolatile,
                                      CodeGenFunction &CGF) const {
  ...
  if (llvm::TargetExtType *TupTy = dyn_cast<llvm::TargetExtType>(SrcTy)) {
    // In Capstone VLS calling convention, struct of fixed vectors or struct
    ...
    for (unsigned i = 0; i < NumElts; ++i) {
      ...
      auto *TupleElement = CGF.Builder.CreateIntrinsic(
          llvm::Intrinsic::capstone_tuple_extract, {EltTy, TupTy}, {Val, Idx});
...
namespace {
class CapstoneTargetCodeGenInfo : public TargetCodeGenInfo {
public:
  CapstoneTargetCodeGenInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen,
                         unsigned FLen, bool EABI)
      : TargetCodeGenInfo(
            std::make_unique<CapstoneABIInfo>(CGT, XLen, FLen, EABI)) {
    SwiftInfo =
  ...
  void setTargetAttributes(const Decl *D, llvm::GlobalValue *GV,
    ...
    const auto *Attr = FD->getAttr<CapstoneInterruptAttr>();
    if (!Attr)
      return;
    ...
    for (CapstoneInterruptAttr::InterruptType type : Attr->interrupt()) {
      switch (type) {
      case CapstoneInterruptAttr::machine:
        // Do not update `Kind` because `Kind` is already "machine", or the
        // kinds also contains SiFive types which need to be applied.
        break;
      case CapstoneInterruptAttr::supervisor:
        Kind = "supervisor";
        break;
      case CapstoneInterruptAttr::rnmi:
        Kind = "rnmi";
        break;
      case CapstoneInterruptAttr::qcinest:
        Kind = "qci-nest";
        break;
      case CapstoneInterruptAttr::qcinonest:
        Kind = "qci-nonest";
        break;
...
CodeGen::createCapstoneTargetCodeGenInfo(CodeGenModule &CGM, unsigned XLen,
                                      unsigned FLen, bool EABI) {
  return std::make_unique<CapstoneTargetCodeGenInfo>(CGM.getTypes(), XLen, FLen,
                                                  EABI);
}
