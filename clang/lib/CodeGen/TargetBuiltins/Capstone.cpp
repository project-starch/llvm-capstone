//===-------- Capstone.cpp - Emit LLVM Code for builtins ---------------------===//
//
...
// Capstone Vector builtin helper functions are marked NOINLINE to prevent
// excessive inlining in CodeGenFunction::EmitCapstoneBuiltinExpr's large switch
...
emitRVVGetBuiltin(CodeGenFunction *CGF, const CallExpr *E,
  ...
  return Builder.CreateIntrinsic(
      Intrinsic::capstone_tuple_extract, {ResultType, Ops[0]->getType()},
...
emitRVVSetBuiltin(CodeGenFunction *CGF, const CallExpr *E,
  ...
  return Builder.CreateIntrinsic(
      Intrinsic::capstone_tuple_insert, {ResultType, Ops[2]->getType()},
...
emitRVVCreateBuiltin(CodeGenFunction *CGF, const CallExpr *E,
  ...
  for (unsigned I = 0, N = Ops.size(); I < N; ++I) {
    if (isa<ScalableVectorType>(ResultType)) {
      ...
      ReturnVector = Builder.CreateIntrinsic(Intrinsic::capstone_tuple_insert,
...
Value *CodeGenFunction::EmitCapstoneCpuInit() {
  llvm::FunctionType *FTy = llvm::FunctionType::get(VoidTy, {VoidPtrTy}, false);
  llvm::FunctionCallee Func =
      CGM.CreateRuntimeFunction(FTy, "__init_capstone_feature_bits");
...
Value *CodeGenFunction::EmitCapstoneCpuSupports(const CallExpr *E) {
  ...
  return EmitCapstoneCpuSupports(ArrayRef<StringRef>(FeatureStr));
}

static Value *loadCapstoneFeatureBits(unsigned Index, CGBuilderTy &Builder,
                                   CodeGenModule &CGM) {
  llvm::Type *Int32Ty = Builder.getInt32Ty();
  llvm::Type *Int64Ty = Builder.getInt64Ty();
  llvm::ArrayType *ArrayOfInt64Ty =
      llvm::ArrayType::get(Int64Ty, llvm::CapstoneISAInfo::FeatureBitSize);
  llvm::Type *StructTy = llvm::StructType::get(Int32Ty, ArrayOfInt64Ty);
  llvm::Constant *CapstoneFeaturesBits =
      CGM.CreateRuntimeVariable(StructTy, "__capstone_feature_bits");
  cast<llvm::GlobalValue>(CapstoneFeaturesBits)->setDSOLocal(true);
  Value *IndexVal = llvm::ConstantInt::get(Int32Ty, Index);
  llvm::Value *GEPIndices[] = {Builder.getInt32(0), Builder.getInt32(1),
                               IndexVal};
  Value *Ptr =
      Builder.CreateInBoundsGEP(StructTy, CapstoneFeaturesBits, GEPIndices);
  Value *FeaturesBit =
      Builder.CreateAlignedLoad(Int64Ty, Ptr, CharUnits::fromQuantity(8));
  return FeaturesBit;
}

Value *CodeGenFunction::EmitCapstoneCpuSupports(ArrayRef<StringRef> FeaturesStrs) {
  const unsigned CapstoneFeatureLength = llvm::CapstoneISAInfo::FeatureBitSize;
  uint64_t RequireBitMasks[CapstoneFeatureLength] = {0};

  for (auto Feat : FeaturesStrs) {
    auto [GroupID, BitPos] = CapstoneISAInfo::getCapstoneFeaturesBitsInfo(Feat);
  ...
  for (unsigned Idx = 0; Idx < CapstoneFeatureLength; Idx++) {
    if (RequireBitMasks[Idx] == 0)
      continue;

    Value *Mask = Builder.getInt64(RequireBitMasks[Idx]);
    Value *Bitset =
        Builder.CreateAnd(loadCapstoneFeatureBits(Idx, Builder, CGM), Mask);
...
Value *CodeGenFunction::EmitCapstoneCpuIs(const CallExpr *E) {
  const Expr *CPUExpr = E->getArg(0)->IgnoreParenCasts();
  StringRef CPUStr = cast<clang::StringLiteral>(CPUExpr)->getString();
  return EmitCapstoneCpuIs(CPUStr);
}

Value *CodeGenFunction::EmitCapstoneCpuIs(StringRef CPUStr) {
  llvm::Type *Int32Ty = Builder.getInt32Ty();
  llvm::Type *Int64Ty = Builder.getInt64Ty();
  llvm::StructType *StructTy = llvm::StructType::get(Int32Ty, Int64Ty, Int64Ty);
  llvm::Constant *CapstoneCPUModel =
      CGM.CreateRuntimeVariable(StructTy, "__capstone_cpu_model");
  cast<llvm::GlobalValue>(CapstoneCPUModel)->setDSOLocal(true);

  auto loadCapstoneCPUID = [&](unsigned Index) {
    Value *Ptr = Builder.CreateStructGEP(StructTy, CapstoneCPUModel, Index);
    Value *CPUID = Builder.CreateAlignedLoad(StructTy->getTypeAtIndex(Index),
                                             Ptr, llvm::MaybeAlign());
    return CPUID;
  };

  const llvm::Capstone::CPUModel Model = llvm::Capstone::getCPUModel(CPUStr);

  // Compare mvendorid.
  Value *VendorID = loadCapstoneCPUID(0);
  Value *Result =
      Builder.CreateICmpEQ(VendorID, Builder.getInt32(Model.MVendorID));

  // Compare marchid.
  Value *ArchID = loadCapstoneCPUID(1);
  Result = Builder.CreateAnd(
      Result, Builder.CreateICmpEQ(ArchID, Builder.getInt64(Model.MArchID)));

  // Compare mimpid.
  Value *ImpID = loadCapstoneCPUID(2);
  Result = Builder.CreateAnd(
...
Value *CodeGenFunction::EmitCapstoneBuiltinExpr(unsigned BuiltinID,
                                             const CallExpr *E,
                                             ReturnValueSlot ReturnValue) {

  if (BuiltinID == Builtin::BI__builtin_cpu_supports)
    return EmitCapstoneCpuSupports(E);
  if (BuiltinID == Builtin::BI__builtin_cpu_init)
    return EmitCapstoneCpuInit();
  if (BuiltinID == Builtin::BI__builtin_cpu_is)
    return EmitCapstoneCpuIs(E);
  ...
  if (Error == ASTContext::GE_Missing_type) {
    ...
    assert(BuiltinID >= clang::Capstone::FirstRVVBuiltin &&
           BuiltinID <= clang::Capstone::LastRVVBuiltin);
    ICEArguments = 0;
    if (BuiltinID == CapstoneVector::BI__builtin_rvv_vget_v ||
        BuiltinID == CapstoneVector::BI__builtin_rvv_vset_v)
      ICEArguments = 1 << 1;
  } else {
    assert(Error == ASTContext::GE_None && "Unexpected error");
  }

  if (BuiltinID == Capstone::BI__builtin_capstone_ntl_load)
    ICEArguments |= (1 << 1);
  if (BuiltinID == Capstone::BI__builtin_capstone_ntl_store)
    ICEArguments |= (1 << 2);
  ...
  case Capstone::BI__builtin_capstone_orc_b_32:
  case Capstone::BI__builtin_capstone_orc_b_64:
  case Capstone::BI__builtin_capstone_clmul_32:
  case Capstone::BI__builtin_capstone_clmul_64:
  case Capstone::BI__builtin_capstone_clmulh_32:
  case Capstone::BI__builtin_capstone_clmulh_64:
  case Capstone::BI__builtin_capstone_clmulr_32:
  case Capstone::BI__builtin_capstone_clmulr_64:
  case Capstone::BI__builtin_capstone_xperm4_32:
  case Capstone::BI__builtin_capstone_xperm4_64:
  case Capstone::BI__builtin_capstone_xperm8_32:
  case Capstone::BI__builtin_capstone_xperm8_64:
  case Capstone::BI__builtin_capstone_brev8_32:
  case Capstone::BI__builtin_capstone_brev8_64:
  case Capstone::BI__builtin_capstone_zip_32:
  case Capstone::BI__builtin_capstone_unzip_32: {
    switch (BuiltinID) {
    default: llvm_unreachable("unexpected builtin ID");
    // Zbb
    case Capstone::BI__builtin_capstone_orc_b_32:
    case Capstone::BI__builtin_capstone_orc_b_64:
      ID = Intrinsic::capstone_orc_b;
      break;

    // Zbc
    case Capstone::BI__builtin_capstone_clmul_32:
    case Capstone::BI__builtin_capstone_clmul_64:
      ID = Intrinsic::capstone_clmul;
      break;
    case Capstone::BI__builtin_capstone_clmulh_32:
    case Capstone::BI__builtin_capstone_clmulh_64:
      ID = Intrinsic::capstone_clmulh;
      break;
    case Capstone::BI__builtin_capstone_clmulr_32:
    case Capstone::BI__builtin_capstone_clmulr_64:
      ID = Intrinsic::capstone_clmulr;
      break;

    // Zbkx
    case Capstone::BI__builtin_capstone_xperm8_32:
    case Capstone::BI__builtin_capstone_xperm8_64:
      ID = Intrinsic::capstone_xperm8;
      break;
    case Capstone::BI__builtin_capstone_xperm4_32:
    case Capstone::BI__builtin_capstone_xperm4_64:
      ID = Intrinsic::capstone_xperm4;
      break;

    // Zbkb
    case Capstone::BI__builtin_capstone_brev8_32:
    case Capstone::BI__builtin_capstone_brev8_64:
      ID = Intrinsic::capstone_brev8;
      break;
    case Capstone::BI__builtin_capstone_zip_32:
      ID = Intrinsic::capstone_zip;
      break;
    case Capstone::BI__builtin_capstone_unzip_32:
      ID = Intrinsic::capstone_unzip;
      break;
    }
  ...
  // Zknh
  case Capstone::BI__builtin_capstone_sha256sig0:
    ID = Intrinsic::capstone_sha256sig0;
    break;
  case Capstone::BI__builtin_capstone_sha256sig1:
    ID = Intrinsic::capstone_sha256sig1;
    break;
  case Capstone::BI__builtin_capstone_sha256sum0:
    ID = Intrinsic::capstone_sha256sum0;
    break;
  case Capstone::BI__builtin_capstone_sha256sum1:
    ID = Intrinsic::capstone_sha256sum1;
    break;

  // Zksed
  case Capstone::BI__builtin_capstone_sm4ks:
    ID = Intrinsic::capstone_sm4ks;
    break;
  case Capstone::BI__builtin_capstone_sm4ed:
    ID = Intrinsic::capstone_sm4ed;
    break;

  // Zksh
  case Capstone::BI__builtin_capstone_sm3p0:
    ID = Intrinsic::capstone_sm3p0;
    break;
  case Capstone::BI__builtin_capstone_sm3p1:
    ID = Intrinsic::capstone_sm3p1;
    break;

  case Capstone::BI__builtin_capstone_clz_32:
  case Capstone::BI__builtin_capstone_clz_64: {
  ...
  case Capstone::BI__builtin_capstone_ctz_32:
  case Capstone::BI__builtin_capstone_ctz_64: {
  ...
  // Zihintntl
  case Capstone::BI__builtin_capstone_ntl_load: {
    llvm::Type *ResTy = ConvertType(E->getType());
    unsigned DomainVal = 5; // Default __Capstone_NTLH_ALL
    if (Ops.size() == 2)
      DomainVal = cast<ConstantInt>(Ops[1])->getZExtValue();

    llvm::MDNode *CapstoneDomainNode = llvm::MDNode::get(
        getLLVMContext(),
    ...
    Load->setMetadata(CGM.getModule().getMDKindID("capstone-nontemporal-domain"),
                      CapstoneDomainNode);

    return Load;
  }
  case Capstone::BI__builtin_capstone_ntl_store: {
    unsigned DomainVal = 5; // Default __Capstone_NTLH_ALL
    if (Ops.size() == 3)
    ...
    Store->setMetadata(CGM.getModule().getMDKindID("capstone-nontemporal-domain"),
                       CapstoneDomainNode);

    return Store;
  }
  // Zihintpause
  case Capstone::BI__builtin_capstone_pause: {
    llvm::Function *Fn = CGM.getIntrinsic(llvm::Intrinsic::capstone_pause);
    return Builder.CreateCall(Fn, {});
  }

  // XCValu
  case Capstone::BI__builtin_capstone_cv_alu_addN:
    ID = Intrinsic::capstone_cv_alu_addN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_addRN:
    ID = Intrinsic::capstone_cv_alu_addRN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_adduN:
    ID = Intrinsic::capstone_cv_alu_adduN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_adduRN:
    ID = Intrinsic::capstone_cv_alu_adduRN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_clip:
    ID = Intrinsic::capstone_cv_alu_clip;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_clipu:
    ID = Intrinsic::capstone_cv_alu_clipu;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_extbs:
    return Builder.CreateSExt(Builder.CreateTrunc(Ops[0], Int8Ty), Int32Ty,
                              "extbs");
  case Capstone::BI__builtin_capstone_cv_alu_extbz:
    return Builder.CreateZExt(Builder.CreateTrunc(Ops[0], Int8Ty), Int32Ty,
                              "extbz");
  case Capstone::BI__builtin_capstone_cv_alu_exths:
    return Builder.CreateSExt(Builder.CreateTrunc(Ops[0], Int16Ty), Int32Ty,
                              "exths");
  case Capstone::BI__builtin_capstone_cv_alu_exthz:
    return Builder.CreateZExt(Builder.CreateTrunc(Ops[0], Int16Ty), Int32Ty,
                              "exthz");
  case Capstone::BI__builtin_capstone_cv_alu_sle:
    return Builder.CreateZExt(Builder.CreateICmpSLE(Ops[0], Ops[1]), Int32Ty,
                              "sle");
  case Capstone::BI__builtin_capstone_cv_alu_sleu:
    return Builder.CreateZExt(Builder.CreateICmpULE(Ops[0], Ops[1]), Int32Ty,
                              "sleu");
  case Capstone::BI__builtin_capstone_cv_alu_subN:
    ID = Intrinsic::capstone_cv_alu_subN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_subRN:
    ID = Intrinsic::capstone_cv_alu_subRN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_subuN:
    ID = Intrinsic::capstone_cv_alu_subuN;
    break;
  case Capstone::BI__builtin_capstone_cv_alu_subuRN:
    ID = Intrinsic::capstone_cv_alu_subuRN;
    break;

  // XAndesBFHCvt
  case Capstone::BI__builtin_capstone_nds_fcvt_s_bf16:
    return Builder.CreateFPExt(Ops[0], FloatTy);
  case Capstone::BI__builtin_capstone_nds_fcvt_bf16_s:
    return Builder.CreateFPTrunc(Ops[0], BFloatTy);

    // Vector builtins are handled from here.
#include "clang/Basic/capstone_vector_builtin_cg.inc"

    // SiFive Vector builtins are handled from here.
#include "clang/Basic/capstone_sifive_vector_builtin_cg.inc"
