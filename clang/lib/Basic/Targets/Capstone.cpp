...
#include "../../../include/clang/Basic/TargetBuiltins.h"
...
ArrayRef<const char *> CapstoneTargetInfo::getGCCRegNames() const {
  // clang-format off
...
ArrayRef<TargetInfo::GCCRegAlias> CapstoneTargetInfo::getGCCRegAliases() const {
...
bool CapstoneTargetInfo::validateAsmConstraint(
...
std::string CapstoneTargetInfo::convertConstraint(const char *&Constraint) const {
  std::string R;
  switch (*Constraint) {
  // c* and v* are two-letter constraints on Capstone.
  case 'c':
  case 'v':
...
void CapstoneTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (CodeModel == "small")
    Builder.defineMacro("__capstone_cmodel_medlow");
  else if (CodeModel == "medium")
    Builder.defineMacro("__capstone_cmodel_medany");
  else if (CodeModel == "large")
    Builder.defineMacro("__capstone_cmodel_large");

  StringRef ABIName = getABI();
  if (ABIName == "ilp32f" || ABIName == "lp64f")
    Builder.defineMacro("__capstone_float_abi_single");
  else if (ABIName == "ilp32d" || ABIName == "lp64d")
    Builder.defineMacro("__capstone_float_abi_double");
  else
    Builder.defineMacro("__capstone_float_abi_soft");

  if (ABIName == "ilp32e" || ABIName == "lp64e")
    Builder.defineMacro("__capstone_abi_rve");

  Builder.defineMacro("__capstone_arch_test");

  for (auto &Extension : ISAInfo->getExtensions()) {
    auto ExtName = Extension.first;
    auto ExtInfo = Extension.second;

    Builder.defineMacro(Twine("__capstone_", ExtName),
                        Twine(getVersionValue(ExtInfo.Major, ExtInfo.Minor)));
  }

  if (ISAInfo->hasExtension("zmmul"))
    Builder.defineMacro("__capstone_mul");

  if (ISAInfo->hasExtension("m")) {
    Builder.defineMacro("__capstone_div");
    Builder.defineMacro("__capstone_muldiv");
  }

  if (ISAInfo->hasExtension("a")) {
    Builder.defineMacro("__capstone_atomic");
  ...
  if (FLen) {
    Builder.defineMacro("__capstone_flen", Twine(FLen));
    Builder.defineMacro("__capstone_fdiv");
    Builder.defineMacro("__capstone_fsqrt");
  }

  if (MinVLen) {
    Builder.defineMacro("__capstone_v_min_vlen", Twine(MinVLen));
    Builder.defineMacro("__capstone_v_elen", Twine(MaxELen));
    Builder.defineMacro("__capstone_v_elen_fp", Twine(MaxELenFp));
  }

  if (ISAInfo->hasExtension("c"))
    Builder.defineMacro("__capstone_compressed");

  if (ISAInfo->hasExtension("zve32x"))
    Builder.defineMacro("__capstone_vector");

  // Currently we support the v1.0 Capstone V intrinsics.
  Builder.defineMacro("__capstone_v_intrinsic", Twine(getVersionValue(1, 0)));

  auto VScale = getVScaleRange(Opts, ArmStreamingKind::NotStreaming);
  if (VScale && VScale->first && VScale->first == VScale->second)
    Builder.defineMacro("__capstone_v_fixed_vlen",
                        Twine(VScale->first * llvm::Capstone::RVVBitsPerBlock));

  if (FastScalarUnalignedAccess)
    Builder.defineMacro("__capstone_misaligned_fast");
  else
    Builder.defineMacro("__capstone_misaligned_avoid");

  if (ISAInfo->hasExtension("e")) {
    if (Is64Bit)
      Builder.defineMacro("__capstone_64e");
    else
      Builder.defineMacro("__capstone_32e");
  }

  if (Opts.CFProtectionReturn && ISAInfo->hasExtension("zicfiss"))
    Builder.defineMacro("__capstone_shadow_stack");
  ...
  if (Opts.CFProtectionBranch) {
    ...
    Builder.defineMacro("__capstone_landing_pad");
    switch (Scheme) {
    case CFBranchLabelSchemeKind::Unlabeled:
      Builder.defineMacro("__capstone_landing_pad_unlabeled");
      break;
...
bool CapstoneTargetInfo::initFeatureMap(
  ...
  auto ParseResult = llvm::CapstoneISAInfo::parseFeatures(XLen, FeaturesVec);
  if (!ParseResult) {
    std::string Buffer;
...
CapstoneTargetInfo::getVScaleRange(const LangOptions &LangOpts,
                                ArmStreamingKind IsArmStreamingFunction,
                                llvm::StringMap<bool> *FeatureMap) const {
  // Capstone::RVVBitsPerBlock is 64.
  unsigned VScaleMin = ISAInfo->getMinVLen() / llvm::Capstone::RVVBitsPerBlock;
  ...
  if (VScaleMin > 0) {
    unsigned VScaleMax = ISAInfo->getMaxVLen() / llvm::Capstone::RVVBitsPerBlock;
...
bool CapstoneTargetInfo::hasFeature(StringRef Feature) const {
  bool Is64Bit = getTriple().isCapstone64();
  auto Result = llvm::StringSwitch<std::optional<bool>>(Feature)
                    .Case("capstone", true)
                    .Case("capstone32", !Is64Bit)
                    .Case("capstone64", Is64Bit)
                    .Case("32bit", !Is64Bit)
                    .Case("64bit", Is64Bit)
...
bool CapstoneTargetInfo::handleTargetFeatures(std::vector<std::string> &Features,
                                           DiagnosticsEngine &Diags) {
  unsigned XLen = getTriple().isArch64Bit() ? 64 : 32;
  auto ParseResult = llvm::CapstoneISAInfo::parseFeatures(XLen, Features);
  if (!ParseResult) {
    std::string Buffer;
    llvm::raw_string_ostream OutputErrMsg(Buffer);
    handleAllErrors(ParseResult.takeError(), [&](llvm::StringError &ErrMsg) {
...
bool CapstoneTargetInfo::isValidCPUName(StringRef Name) const {
  bool Is64Bit = getTriple().isArch64Bit();
  return llvm::Capstone::parseCPU(Name, Is64Bit);
}

void CapstoneTargetInfo::fillValidCPUList(
    SmallVectorImpl<StringRef> &Values) const {
  bool Is64Bit = getTriple().isArch64Bit();
  llvm::Capstone::fillValidCPUArchList(Values, Is64Bit);
}

bool CapstoneTargetInfo::isValidTuneCPUName(StringRef Name) const {
  bool Is64Bit = getTriple().isArch64Bit();
  return llvm::Capstone::parseTuneCPU(Name, Is64Bit);
}

void CapstoneTargetInfo::fillValidTuneCPUList(
    SmallVectorImpl<StringRef> &Values) const {
  bool Is64Bit = getTriple().isArch64Bit();
  llvm::Capstone::fillValidTuneCPUArchList(Values, Is64Bit);
}

static void populateNegativeCapstoneFeatures(std::vector<std::string> &Features) {
  auto RII = llvm::CapstoneISAInfo::parseArchString(
...
static void handleFullArchString(StringRef FullArchStr,
  ...
  if (llvm::errorToBool(RII.takeError())) {
    ...
    populateNegativeCapstoneFeatures(Features);
    std::vector<std::string> FeatStrings =
        (*RII)->toFeatures(/* AddAllExtensions */ true);
    llvm::append_range(Features, FeatStrings);
  }
...
ParsedTargetAttr CapstoneTargetInfo::parseTargetAttr(StringRef Features) const {
  ...
  auto handleArchExtension = [](StringRef AttrString,
    ...
    for (auto Ext : Exts) {
      ...
      std::string TargetFeature =
          llvm::CapstoneISAInfo::getTargetFeatureForExtension(ExtName);
  ...
  for (auto &Feature : AttrFeatures) {
    ...
    if (Feature.starts_with("arch=")) {
      ...
      if (!FoundArch) {
        ...
        StringRef MarchFromCPU = llvm::Capstone::getMArchFromMcpu(Ret.CPU);
        if (MarchFromCPU != "") {
          Ret.Features.clear();
...
llvm::APInt
CapstoneTargetInfo::getFMVPriority(ArrayRef<StringRef> Features) const {
  // Priority is explicitly specified on Capstone unlike on other targets, where
...
CapstoneTargetInfo::checkCallingConvention(CallingConv CC) const {
  switch (CC) {
  default:
    return CCCR_Warning;
  case CC_C:
...
bool CapstoneTargetInfo::validateCpuSupports(StringRef Feature) const {
  // Only allow extensions we have a known bit position for in the
  // __capstone_feature_bits structure.
  return -1 != llvm::CapstoneISAInfo::getCapstoneFeaturesBitsInfo(Feature).second;
}

bool CapstoneTargetInfo::isValidFeatureName(StringRef Name) const {
  return llvm::CapstoneISAInfo::isSupportedExtensionFeature(Name);
}

bool CapstoneTargetInfo::validateGlobalRegisterVariable(
...
bool CapstoneTargetInfo::validateCpuIs(StringRef CPUName) const {
  assert(getTriple().isOSLinux() &&
         "__builtin_cpu_is() is only supported for Linux.");

  return llvm::Capstone::hasValidCPUModel(CPUName);
}
